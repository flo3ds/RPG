...
final class Types {
	...
	static ParameterizedType newParameterizedTypeWithOwner(@Nullable Type ownerType, Class<?> rawType,
		...
		// ParameterizedTypeImpl constructor already checks, but we want to
		// throw NPE before IAE
	...
	private static final class ParameterizedTypeImpl implements ParameterizedType, Serializable {
		...
		public String toString() {
			...
			return builder.append(rawType.getName()).append('<')
					.append(COMMA_JOINER.join(transform(argumentsList, TYPE_NAME))).append('>').toString();
		}
	...
	/**
	 * Invocation handler to work around a compatibility problem between Java 7
	 * and Java 8.
	 *
	 * <p>
	 * Java 8 introduced a new method {@code getAnnotatedBounds()} in the
	 * {@link TypeVariable} interface, whose return type {@code AnnotatedType[]}
	 * is also new in Java 8. That means that we cannot implement that interface
	 * in source code in a way that will compile on both Java 7 and Java 8. If
	 * we include the {@code getAnnotatedBounds()} method then its return type
	 * means it won't compile on Java 7, while if we don't include the method
	 * then the compiler will complain that an abstract method is unimplemented.
	 * So instead we use a dynamic proxy to get an implementation. If the method
	 * being called on the {@code TypeVariable} instance has the same name as
	 * one of the public methods of {@link TypeVariableImpl}, the proxy calls
	 * the same method on its instance of {@code TypeVariableImpl}. Otherwise it
	 * throws {@link UnsupportedOperationException}; this should only apply to
	 * {@code getAnnotatedBounds()}. This does mean that users on Java 8 who
	 * obtain an instance of {@code TypeVariable} from
	 * {@link TypeResolver#resolveType} will not be able to call
	 * {@code getAnnotatedBounds()} on it, but that should hopefully be rare.
	 *
	 * <p>
	 * This workaround should be removed at a distant future time when we no
			...
			for (Method method : TypeVariableImpl.class.getMethods()) {
				if (method.getDeclaringClass().equals(TypeVariableImpl.class)) {
					try {
						...
						// OK: the method is accessible to us anyway. The
						// setAccessible call is only for
						// unusual execution environments where that might not
						// be true.
					}
	...
	private static final class TypeVariableImpl<D extends GenericDeclaration> {
		...
		public boolean equals(Object obj) {
			if (NativeTypeVariableEquals.NATIVE_TYPE_VARIABLE_ONLY) {
				// equal only to our TypeVariable implementation with identical
				// bounds
	...
	// TODO(benyu): Once behavior is the same for all Java versions we support,
	// delete this.
	enum JavaVersion {
		JAVA6 {
			@Override
	...
	/**
	 * Per <a href=
	 * "https://code.google.com/p/guava-libraries/issues/detail?id=1635">issue
	 * 1635</a>, In JDK 1.7.0_51-b13, {@link TypeVariableImpl#equals(Object)} is
	 * changed to no longer be equal to custom TypeVariable implementations. As
	 * a result, we need to make sure our TypeVariable implementation respects
	 * symmetry. Moreover, we don't want to reconstruct a native type variable
	 * {@code <A>} using our implementation unless some of its bounds have
	 * changed in resolution. This avoids creating unequal TypeVariable
	 * implementation unnecessarily. When the bounds do change, however, it's
	 * fine for the synthetic TypeVariable to be unequal to any native
	 * TypeVariable anyway.
	 */
