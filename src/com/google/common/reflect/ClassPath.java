 ...
 *
 * <p>
 * <b>Warning:</b> Currently only {@link URLClassLoader} and only
 * {@code file://} urls are supported.
 *
 * <p>
 * In the case of directory classloaders, symlinks are supported but cycles are
 * not traversed. This guarantees discovery of each <em>unique</em> loadable
 * resource. However, not all possible aliases for resources on cyclic paths
 * will be listed.
 *
 * @author Ben Yu
 * @since 14.0
...
public final class ClassPath {
	 ...
	 *
	 * @throws IOException
	 *             if the attempt to read class path resources (jar files or
	 *             directories) failed.
	 */
	...
	public static class ResourceInfo {
		 ...
		 *             if the resource cannot be loaded through the class
		 *             loader, despite physically existing in the class path.
		 */
		...
		/**
		 * Returns a {@link ByteSource} view of the resource from which its
		 * bytes can be read.
		 *
		 * @throws NoSuchElementException
		 *             if the resource cannot be loaded through the class
		...
		/**
		 * Returns a {@link CharSource} view of the resource from which its
		 * bytes can be read as characters decoded with the given
		 * {@code charset}.
		 *
		 * @throws NoSuchElementException
		 *             if the resource cannot be loaded through the class
	 ...
	 * {@link ClassLoader} and calls {@link #scanDirectory} and
	 * {@link #scanJarFile} for directories and jar files on the class path
	 * respectively.
	 */
	abstract static class Scanner {

		// We only scan each file once independent of the classloader that
		// resource might be associated
		// with.
		...
		private void scanFrom(File file, ClassLoader classloader) throws IOException {
			try {
				...
				// TODO(emcmanus): consider whether to log other failure cases
				// too.
				return;
			}
			if (file.isDirectory()) {
	...
	static final class DefaultScanner extends Scanner {
		private final SetMultimap<ClassLoader, String> resources = MultimapBuilder.hashKeys().linkedHashSetValues()
				.build();
		 ...
		 * encountered. Symlinks which have already been traversed in the
		 * current tree path will be skipped to eliminate cycles; otherwise
		 * symlinks are traversed.
		 *
		 * @param directory
		 *            the root of the directory to scan
		 * @param classloader
		 *            the classloader that includes resources found in
		 *            {@code directory}
		 * @param packagePrefix
		 *            resource path prefix inside {@code classloader} for any
		 *            files found under {@code directory}
		 * @param currentPath
		 *            canonical files already visited in the current directory
	...
	// TODO(benyu): Try java.nio.file.Paths#get() when Guava drops JDK 6
	// support.
	@VisibleForTesting
	static File toFile(URL url) {
		checkArgument(url.getProtocol().equals("file"));
		try {
			return new File(url.toURI()); // Accepts escaped characters like
											// %20.
		} catch (URISyntaxException e) { // URL.toURI() doesn't escape chars.
			return new File(url.getPath()); // Accepts non-escaped chars like
											// space.
		}
	}
}
