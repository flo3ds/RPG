...
/**
 * A {@link RangeSet} whose contents will never change, with many other
...
public final class ImmutableRangeSet<C extends Comparable> extends AbstractRangeSet<C> implements Serializable {
	...
	/**
	 * Returns an {@code ImmutableRangeSet} representing the union of the
	 * specified ranges.
	 *
	 * <p>
	 * This is the smallest {@code RangeSet} which encloses each of the
	 * specified ranges. Duplicate or connected ranges are permitted, and will
	 * be coalesced in the result.
	 *
	 * @since 21.0
	 */
	...
	/**
	 * Returns an {@code ImmutableRangeSet} containing each of the specified
	 * disjoint ranges. Overlapping ranges and empty ranges are forbidden,
	 * though adjacent ranges are permitted and will be merged.
	 *
	...
	public boolean intersects(Range<C> otherRange) {
		int ceilingIndex = SortedLists.binarySearch(ranges, Range.<C>lowerBoundFn(), otherRange.lowerBound,
				Ordering.natural(), ANY_PRESENT, NEXT_HIGHER);
	...
	/**
	 * Guaranteed to throw an exception and leave the {@code RangeSet}
	 * unmodified.
	 *
	...
	public ImmutableSet<Range<C>> asDescendingSetOfRanges() {
		...
		return new RegularImmutableSortedSet<Range<C>>(ranges.reverse(), Range.<C>rangeLexOrdering().reverse());
	}
	 ...
	 * {@code TreeRangeSet.create(this).addAll(other)} except it returns an
	 * {@code ImmutableRangeSet}.
	 *
	 * @since 21.0
	 */
	...
	/**
	 * Returns a new range set consisting of the intersection of this range set
	 * and {@code other}.
	 *
	 * <p>
	 * This is essentially the same as {@code
	 * TreeRangeSet.create(this).removeAll(other.complement())} except it
	 * returns an {@code
	 * ImmutableRangeSet}.
	 *
	 * @since 21.0
	...
	/**
	 * Returns a new range set consisting of the difference of this range set
	 * and {@code other}.
	 *
	 * <p>
	 * This is essentially the same as
	 * {@code TreeRangeSet.create(this).removeAll(other)} except it returns an
	 * {@code ImmutableRangeSet}.
	 *
	 * @since 21.0
	 */
	...
	/**
	 * Returns a view of the intersection of this range set with the given
	 * range.
	 */
	@Override
	 ...
	 * given domain {@linkplain RangeSet#contains contained} by this range set.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * <b>Warning:</b> Be extremely careful what you do with the {@code asSet}
	 * view of a large range set (such as
	 * {@code ImmutableRangeSet.of(Range.greaterThan(0))}). Certain operations
	 * on such a set can be performed efficiently, but others (such as
	 ...
	 * form of the set's contents, such as {@code "[1..100]}"}.
	 *
	 * @throws IllegalArgumentException
	...
	public ImmutableSortedSet<C> asSet(DiscreteDomain<C> domain) {
		...
		if (!span.hasLowerBound()) {
			// according to the spec of canonical, neither this
			// ImmutableRangeSet nor
	...
	private final class AsSet extends ImmutableSortedSet<C> {
		...
		int indexOf(Object target) {
			if (contains(target)) {
				@SuppressWarnings("unchecked") // if it's contained, it's
												// definitely a C
				C c = (C) target;
				long total = 0;
				for (Range<C> range : ranges) {
	...
	/**
	 * Returns {@code true} if this immutable range set's implementation
	 * contains references to user-created objects that aren't accessible via
	 * this range set's methods. This is generally used to determine whether
	...
	public static class Builder<C extends Comparable<?>> {
		...
		// TODO(lowasser): consider adding union, in addition to add, that does
		// allow overlap

		/**
		 * Add the specified range to this builder. Adjacent ranges are
		 * permitted and will be merged, but overlapping ranges will cause an
		 * exception when {@link #build()} is called.
		 *
		 * @throws IllegalArgumentException
		 *             if {@code range} is empty
		 */
		...
		/**
		 * Add all ranges from the specified range set to this builder. Adjacent
		 * ranges are permitted and will be merged, but overlapping ranges will
		 * cause an exception when {@link #build()} is called.
		 */
		@CanIgnoreReturnValue
		...
		/**
		 * Add all of the specified ranges to this builder. Adjacent ranges are
		 * permitted and will be merged, but overlapping ranges will cause an
		 * exception when {@link #build()} is called.
		 *
		 * @throws IllegalArgumentException
		...
		/**
		 * Returns an {@code ImmutableRangeSet} containing the ranges added to
		 * this builder.
		 *
		 * @throws IllegalArgumentException
		...
		public ImmutableRangeSet<C> build() {
			ImmutableList.Builder<Range<C>> mergedRangesBuilder = new ImmutableList.Builder<Range<C>>(ranges.size());
			...
			while (peekingItr.hasNext()) {
				...
				while (peekingItr.hasNext()) {
					...
					if (range.isConnected(nextRange)) {
						checkArgument(range.intersection(nextRange).isEmpty(),
