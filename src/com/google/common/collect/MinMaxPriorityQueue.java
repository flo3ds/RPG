 ...
 * specified comparator. If no comparator is given at creation time, the natural
 * order of elements is used. If no maximum size is given at creation time, the
 * queue is unbounded.
 *
 * <p>
 * Usage example:
 ...
 *
 * <p>
 * As a {@link Queue} it functions exactly as a {@link PriorityQueue}: its head
 * element -- the implicit target of the methods {@link #peek()},
 * {@link #poll()} and {@link #remove()} -- is defined as the <i>least</i>
 * element in the queue according to the queue's comparator. But unlike a
 * regular priority queue, the methods {@link #peekLast}, {@link #pollLast} and
 * {@link #removeLast} are also provided, to act on the <i>greatest</i> element
 * in the queue instead.
 *
 * <p>
 * A min-max priority queue can be configured with a maximum size. If so, each
 * time the size of the queue exceeds that value, the queue automatically
 ...
 *
 * <ul>
 * <li>If you only access one end of the queue, and do use a maximum size, this
 * class will perform significantly worse than a {@code PriorityQueue} with
 * manual eviction above the maximum size. In many cases
 * {@link Ordering#leastOf} may work for your use case with significantly
 * improved (and asymptotically superior) performance.
 * <li>The retrieval operations {@link #peek}, {@link #peekFirst},
 * {@link #peekLast}, {@link #element}, and {@link #size} are constant-time.
...
public final class MinMaxPriorityQueue<E> extends AbstractQueue<E> {
	...
	/**
	 * Creates a new min-max priority queue with default settings: natural
	 * order, no maximum size, no initial contents, and an initial expected size
	 * of 11.
	 */
	...
	/**
	 * Creates a new min-max priority queue using natural order, no maximum
	 * size, and initially containing the given elements.
	 */
	 ...
	 * MinMaxPriorityQueue} instances that use {@code comparator} to determine
	 ...
	 * removes its greatest element (according to its comparator), which might
	 * be the element that was just added.
	 */
	 ...
	 *            this builder (for example, a {@code Builder<Number>} can
	 *            produce a {@code Queue<Number>} or {@code Queue<Integer>} but
	 *            not a {@code
	 *     Queue<Object>}).
	 * @since 8.0
	 */
	...
	public static final class Builder<B> {
		...
		/**
		 * Configures this builder to build min-max priority queues with an
		 * initial expected size of {@code expectedSize}.
		 */
		...
		/**
		 * Configures this builder to build {@code MinMaxPriorityQueue}
		 * instances that are limited to {@code maximumSize} elements. Each time
		 * a queue grows beyond this bound, it immediately removes its greatest
		 * element (according to its comparator), which might be the element
		 * that was just added.
		 */
		@CanIgnoreReturnValue
	...
	public boolean offer(E element) {
		...
		// Adds the element to the end of the heap and bubbles it up to the
		// correct
		// position.
	...
	/**
	 * Retrieves, but does not remove, the least element of this queue, or
	 ...
	 * first one is the element that was previously at the end of the heap and
	 * is now at some position before {@code index}. The second element is the
	 * one that was swapped down to replace the element at {@code index}. This
	 * fact is used by iterator.remove so as to visit elements during a
	 * traversal once and only once.
	 */
	@VisibleForTesting
	@CanIgnoreReturnValue
	MoveDesc<E> removeAt(int index) {
		...
		if (lastElementAt == index) {
			// 'actualLastElement' is now at 'lastElementAt', and the element
			// that was at 'lastElementAt'
			// is now at the end of queue. If that's the element we wanted to
			// remove in the first place,
			// don't try to (incorrectly) trickle it. Instead, just delete it
			// and we're done.
			queue[size] = null;
			return null;
		}
		...
		if (lastElementAt < index) {
			// Last element is moved to before index, swapped with trickled
			// element.
			if (changes == null) {
				// The trickled element is still after index.
				return new MoveDesc<E>(actualLastElement, toTrickle);
			} else {
				// The trickled element is back before index, but the replaced
				// element
	...
	private MoveDesc<E> fillHole(int index, E toTrickle) {
		...
		// comparisons with toTrickle, but in some cases we will need to bubble
		// it
		...
		if (bubbledTo == vacated) {
			...
			// it from min level to max level (or max to min level) and bubble
			// up
			// there.
	...
	/**
	 * Returns {@code true} if the MinMax heap structure holds. This is only
	 * used in testing.
	 *
	 ...
	 * min-heap and a max-heap. Conceptually, these might each have their own
	 * array for storage, but for efficiency's sake they are stored interleaved
	 * on alternate heap levels in the same array (MMPQ.queue).
	 */
	@WeakOuter
	private class Heap {
		...
		/**
		 * Tries to move {@code toTrickle} from a min to a max level and bubble
		 * up there. If it moved before {@code removeIndex} this method returns
		 * a pair as described in {@link #removeAt}.
		 */
		MoveDesc<E> tryCrossOverAndBubbleUp(int removeIndex, int vacated, E toTrickle) {
			...
			E parent;
			// If toTrickle is moved up to a parent of removeIndex, the parent
			// is
			// placed in removeIndex position. We must return that to the
			// iterator so
			// that it knows to skip it.
			if (crossOver < removeIndex) {
				// We crossed over to the parent level in crossOver, so the
				// parent
		...
		/**
		 * Bubbles a value from {@code index} up the appropriate heap if
		 * required.
		 */
		...
		/**
		 * Moves an element one level up from a min level to a max level (or
		 * vice versa). Returns the new position of the element.
		 */
		int crossOverUp(int index, E x) {
			...
			if (parentIndex != 0) {
				...
				// Since the end of the array is actually the middle of the
				// heap,
		...
		/**
		 * Swap {@code actualLastElement} with the conceptually correct last
		 * element of the heap. Returns the index that {@code actualLastElement}
		 * now resides in.
		 *
		 * <p>
		 * Since the last element of the array is actually in the middle of the
		 * sorted structure, a childless uncle node could be smaller, which
		 * would corrupt the invariant if this element becomes the new parent of
		 * the uncle. In that case, we first switch the last element with its
		...
		/**
		 * Crosses an element over to the opposite heap by moving it one level
		 * down (or up if there are no elements below it).
		 *
		 * Returns the new position of the element.
		 */
		int crossOver(int index, E x) {
			int minChildIndex = findMinChild(index);
			// TODO(kevinb): split the && into two if's and move crossOverUp so
			// it's
		 ...
		 * created.
		 *
		 * @return the position of the new hole (where the lowest grandchild
		 *         moved from, that had no grandchild to replace it)
		 */
	...
	private class QueueIterator implements Iterator<E> {
		...
		// The same element is not allowed in both forgetMeNot and skipMe, but
		// duplicates are allowed in
		...
		/**
		 * Returns true if an exact reference (==) was found and removed from
		...
		/**
		 * Advances nextCursor to the index of the first element after {@code c}
		 * that is not in {@code skipMe} and returns {@code size()} if there is
		 * no such element.
		 */
	...
	/**
	 * Returns an iterator over the elements contained in this collection, <i>in
	 * no particular order</i>.
	 *
	 * <p>
	 * The iterator is <i>fail-fast</i>: If the MinMaxPriorityQueue is modified
	...
	/**
	 * Returns the comparator used to order the elements in this queue. Obeys
	 * the general contract of {@link PriorityQueue#comparator}, but returns
	...
	static int initialQueueSize(int configuredExpectedSize, int maximumSize, Iterable<?> initialContents) {
		// Start with what they said, if they said it, otherwise
		// DEFAULT_CAPACITY
