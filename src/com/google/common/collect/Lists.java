...
public final class Lists {
	 ...
	 * provides only a tiny bit of syntactic sugar for
	 * {@code newArrayList(}{@link Arrays#asList asList}{@code (...))}, or for
	 * creating an empty list then calling {@link Collections#addAll}. This
	 * method is not actually very useful and will likely be deprecated in the
	 * future.
	 */
	@SafeVarargs
	 ...
	 * <p>
	 * <b>Note for Java 7 and later:</b> this method is now unnecessary and
	 * should be treated as deprecated. Instead, use
	 * {@code new }{@link ArrayList#ArrayList(int) ArrayList}{@code <>(int)}
	 * directly, taking advantage of the new
	 * <a href="http://goo.gl/iz2Wi">"diamond" syntax</a>. (Unlike here, there
	 * is no risk of overload ambiguity, since the {@code
	 ...
	 *
	 * <p>
	 * <b>Performance note:</b> {@link ArrayList} and
	 * {@link java.util.ArrayDeque} consistently outperform {@code LinkedList}
	 * except in certain rare and specific situations. Unless you have spent a
	 * lot of time benchmarking your specific needs, use one of those instead.
	 *
	 * <p>
	 * <b>Note for Java 7 and later:</b> this method is now unnecessary and
	 * should be treated as deprecated. Instead, use the {@code LinkedList}
	 * {@linkplain LinkedList#LinkedList() constructor} directly, taking
	 * advantage of the new <a href="http://goo.gl/iz2Wi">"diamond" syntax</a>.
	 */
	@GwtCompatible(serializable = true)
	 ...
	 * <p>
	 * <b>Note for Java 7 and later:</b> if {@code elements} is a
	 * {@link Collection}, you don't need this method. Use the
	 * {@code LinkedList} {@linkplain LinkedList#LinkedList(Collection)
	...
	/**
	 * Creates a {@code CopyOnWriteArrayList} instance containing the given
	 * elements.
	 *
	 * @param elements
	 ...
	 * backed by the specified array of additional elements. Changes to the
	 * {@code
	 ...
	 * Changes to the {@code rest} array will be reflected in the returned list.
	 * Unlike {@link Arrays#asList}, the returned list is unmodifiable.
	 *
	 * <p>
	 * This is useful when a varargs method needs to use a signature such as
	 ...
	 * consumption is much smaller. When the cartesian product is constructed,
	 * the input lists are merely copied. Only as the resulting list is iterated
	 * are the individual lists created, and these are not retained after
	 * iteration.
	 *
	 * @param lists
	 *            the lists to choose elements from, in the order that the
	 *            elements chosen from those lists should appear in the
	 *            resulting lists
	 * @param <B>
	 ...
	 *         lists
	 * @throws IllegalArgumentException
	 *             if the size of the cartesian product would be greater than
	 *             {@link Integer#MAX_VALUE}
	 * @throws NullPointerException
	 *             if {@code lists}, any one of the {@code lists}, or any
	 ...
	 * changes to {@code fromList} will be reflected in the returned list and
	 * vice versa.
	 *
	 * <p>
	 ...
	 * for the returned list to be a view, but it means that the function will
	 * be applied many times for bulk operations like {@link List#contains} and
	 * {@link List#hashCode}. For this to perform well, {@code function} should
	 * be fast. To avoid lazy evaluation when the returned list doesn't need to
	 * be a view, copy the returned list into a new list of your choosing.
	 *
	 * <p>
	 * If {@code fromList} implements {@link RandomAccess}, so will the returned
	 * list. The returned list is threadsafe if the supplied list and function
	 * are.
	 *
	 * <p>
	 ...
	 * so serializing the returned list is <b>not recommended</b>. Instead, copy
	 * the list using {@link ImmutableList#copyOf(Collection)} (for example),
	 * then serialize the copy. Other methods similar to this do not implement
	 * serialization at all for this reason.
	 *
	 * <p>
	 * <b>Java 8 users:</b> many use cases for this method are better addressed
	...
	/**
	 * Implementation of a transforming random access list. We try to make as
	 * many of these methods pass-through to the source list as possible so that
	 * the performance characteristics of the source list and transformed list
	 * are similar.
	 *
	 * @see Lists#transform
	 */
	 ...
	 * units. The view does not support any modification operations, but
	 * reflects any changes to the underlying character sequence.
	 *
	 * @param sequence
	 *            the character sequence to view as a {@code List} of characters
	 ...
	 * {@code 3,
	 * 2, 1}. The returned list is backed by this list, so changes in the
