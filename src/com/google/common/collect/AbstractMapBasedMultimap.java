...
abstract class AbstractMapBasedMultimap<K, V> extends AbstractMultimap<K, V> implements Serializable {
	 ...
	 * from the map.
	 *
	 * The get method returns a WrappedCollection, which decorates the
	 * collection in the map (if the key is present) or an empty collection (if
	 * the key is not present). When the collection delegate in the
	 * WrappedCollection is empty, the multimap may contain subsequently added
	 * values for that key. To handle that situation, the WrappedCollection
	 * checks whether map contains an entry for the provided key, and if so
	 ...
	 * default, it simply calls {@link #createCollection()}, which is the
	 * correct behavior for most implementations. The {@link LinkedHashMultimap}
	 * class overrides it.
	 *
	 * @param key
	 ...
	 * {@link AbstractMapBasedMultimap#map} whenever the delegate is non-empty.
	 * The {@code
	 * refreshIfEmpty}, {@code removeIfEmpty}, and {@code addToMap} methods
	 * ensure that the {@code WrappedCollection} and map remain consistent.
	 *
	 * <p>
	...
	private class WrappedSet extends WrappedCollection implements Set<V> {
		...
		public boolean removeAll(Collection<?> c) {
			...
			// Guava issue 1013: AbstractSet and most JDK set implementations
			// are
	...
	/**
	 * SortedSet decorator that stays in sync with the multimap values for a
	 * key.
	 */
	@WeakOuter
	 ...
	 * key, and so on, suffices for most {@code AbstractMapBasedMultimap}
	 * implementations.
	 *
	...
	private class SortedAsMap extends AsMap implements SortedMap<K, Collection<V>> {
		...
		// returns a SortedSet, even though returning a Set would be sufficient
		// to
