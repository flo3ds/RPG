 ...
 * may not have the same {@code hashCode}. The recommended subinterfaces provide
...
public interface Multimap<K, V> {
	 ...
	 *
	 * <p>
	 * <b>Note:</b> this method does not return the number of <i>distinct
	 ...
	 * Equivalent to {@code size() == 0}, but can in some cases be more
	 * efficient.
	 */
	boolean isEmpty();
	 ...
	 * storing a key-value pair that's already in the multimap has no effect.
	 *
	 * @return {@code true} if the method increased the size of the multimap, or
	 *         {@code false} if the multimap already contained the key-value
	 *         pair and doesn't allow duplicates
	 */
	 ...
	 * so it will not appear in {@link #keySet()}, {@link #asMap()}, or any
	 * other views.
	 *
	 ...
	 * this multimap, if any. Note that when {@code containsKey(key)} is false,
	 * this returns an empty collection, not {@code null}.
	 *
	 * <p>
	 ...
	 * multimap. Note that the key set contains a key if and only if this
	 * multimap maps that key to at least one value.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * Changes to the returned collection or the entries it contains will update
	 * the underlying multimap, and vice versa. However, <i>adding</i> to the
	 ...
	 * {@code this.asMap().get(k)} is equivalent to {@code this.get(k)} only
	 * when {@code k} is a key contained in the multimap; otherwise it returns
	 * {@code
	 ...
	 * support {@code put} or {@code putAll}, nor do its entries support
	 ...
	 * but equality of two {@link ListMultimap} instances depends on the
	 ...
	 * collections as values. However, any two empty multimaps are equal,
	 * because they both have empty {@link #asMap} views.
	 */
	@Override
	 ...
	 * mappings will have the same {@code hashCode}, but the {@code hashCode} of
	 * {@link ListMultimap} instances depends on the ordering of the values for
	 * each key.
	 */
	@Override
	int hashCode();
