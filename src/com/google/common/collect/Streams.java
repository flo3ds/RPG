...
public final class Streams {
	 ...
	 *
	 * @deprecated There is no reason to use this; just invoke
	 *             {@code collection.stream()} directly.
	 */
	@Deprecated
	...
	/**
	 * If a value is present in {@code optional}, returns a stream containing
	 * only that element, otherwise returns an empty stream.
	 */
	 ...
	 * {@code Stream.of(streams).flatMap(stream -> stream)}, but the returned
	...
	public static <T> Stream<T> concat(Stream<? extends T>... streams) {
		...
		return StreamSupport.stream(CollectSpliterators.flatMap(splitrsBuilder.build().spliterator(),
	 ...
	 * <p>
	 * This is equivalent to
	 * {@code Stream.of(streams).flatMapToInt(stream -> stream)}, but the
	 ...
	 * <p>
	 * This is equivalent to
	 * {@code Stream.of(streams).flatMapToLong(stream -> stream)}, but the
	 * returned stream may perform better.
	 *
	...
	/**
	 * Returns a {@link DoubleStream} containing the elements of the first
	 * stream, followed by the elements of the second stream, and so on.
	 *
	 * <p>
	 * This is equivalent to
	 * {@code Stream.of(streams).flatMapToDouble(stream -> stream)}, but the
	 * returned stream may perform better.
	 *
	...
	/**
	 * Returns a stream in which each element is the result of passing the
	 * corresponding elementY of each of {@code streamA} and {@code streamB} to
	 * {@code function}.
	 *
	 * <p>
	 * For example:
	 ...
	 *
	 * <p>
	 * Note that if you are calling {@link Stream#forEach} on the resulting
	 * stream, you might want to consider using {@link #forEachPair} instead of
	 * this method.
	 *
	 * <p>
	 * <b>Performance note:</b> The resulting stream is not <a href=
	...
	public static <A, B, R> Stream<R> zip(Stream<A> streamA, Stream<B> streamB,
		...
		boolean isParallel = streamA.isParallel() || streamB.isParallel(); // same
																			// as
																			// Stream.concat
		...
		return StreamSupport.stream(
				new AbstractSpliterator<R>(Math.min(splitrA.estimateSize(), splitrB.estimateSize()), characteristics) {
					@Override
	...
	/**
	 * Invokes {@code consumer} once for each pair of <i>corresponding</i>
	 * elements in {@code streamA} and {@code streamB}. If one stream is longer
	 * than the other, the extra elements are silently ignored. Elements passed
	 * to the consumer are guaranteed to come from the same position in their
	 ...
	 * correspondence between elements will be made, but the order in which
	 * those pairs of elements are passed to the consumer is <i>not</i> defined.
	 *
	 * <p>
	 * Note that many usages of this method can be replaced with simpler calls
	 * to {@link #zip}. This method behaves equivalently to {@linkplain #zip
	 * zipping} the stream elements into temporary pair objects and then using
	 ...
	 * to the elements of {@code stream} and their indices in the stream. For
	 * example,
	 *
	 * <pre>
	 * {@code
	 ...
	 *
	 * <p>
	 * The resulting stream is <a href=
	 * "http://gee.cs.oswego.edu/dl/html/StreamParallelGuidance.html">efficiently
	 * splittable</a> if and only if {@code stream} was efficiently splittable
	 * and its underlying spliterator reported {@link Spliterator#SUBSIZED}.
	 * This is generally the case if the underlying stream comes from a data
	 * structure supporting efficient indexed random access, typically an array
	 * or list.
	 *
	 * <p>
	 * The order of the resulting stream is defined if and only if the order of
	...
	/**
	 * An analogue of {@link java.util.function.Function} also accepting an
	 * index.
	 *
	 * <p>
	...
	public interface FunctionWithIndex<T, R> {
		/**
		 * Applies this function to the given argument and its index within a
		 * stream.
		 */
	...
	/**
	 * Returns a stream consisting of the results of applying the given function
	 * to the elements of {@code stream} and their indexes in the stream. For
	 * example,
	 *
	 * <pre>
	 * {@code
	...
	/**
	 * An analogue of {@link java.util.function.IntFunction} also accepting an
	 * index.
	 *
	 * <p>
	 * This interface is only intended for use by callers of
	...
	/**
	 * An analogue of {@link java.util.function.LongFunction} also accepting an
	 * index.
	 *
	 * <p>
	 * This interface is only intended for use by callers of
	...
	/**
	 * An analogue of {@link java.util.function.DoubleFunction} also accepting
	 * an index.
	 *
	 * <p>
	 * This interface is only intended for use by callers of
	 ...
	 *
	 * <p>
	 * Equivalent to {@code stream.reduce((a, b) -> b)}, but may perform
	 * significantly better. This method's runtime will be between O(log n) and
	 * O(n), performing better on <a href=
	 * "http://gee.cs.oswego.edu/dl/html/StreamParallelGuidance.html">efficiently
	 * splittable</a> streams.
	 *
	 * <p>
	 * If the stream has nondeterministic order, this has equivalent semantics
	 * to {@link Stream#findAny} (which you might as well use).
	 *
	...
	public static <T> java.util.Optional<T> findLast(Stream<T> stream) {
		...
		while (!splits.isEmpty()) {
			...
			// Many spliterators will have trySplits that are SUBSIZED even if
			// they are not themselves
			// SUBSIZED.
			if (spliterator.hasCharacteristics(Spliterator.SUBSIZED)) {
				// we can drill down to exactly the smallest nonempty
				// spliterator
				while (true) {
	...
	/**
	 * Returns the last element of the specified stream, or
	 * {@link OptionalInt#empty} if the stream is empty.
	 *
	 * <p>
	 * Equivalent to {@code stream.reduce((a, b) -> b)}, but may perform
	 * significantly better. This method's runtime will be between O(log n) and
	...
	public static OptionalInt findLast(IntStream stream) {
		// findLast(Stream) does some allocation, so we might as well box some
		// more
	...
	/**
	 * Returns the last element of the specified stream, or
	 * {@link OptionalLong#empty} if the stream is empty.
	 *
	 * <p>
	 * Equivalent to {@code stream.reduce((a, b) -> b)}, but may perform
	 * significantly better. This method's runtime will be between O(log n) and
	...
	/**
	 * Returns the last element of the specified stream, or
	 * {@link OptionalDouble#empty} if the stream is empty.
	 *
	 * <p>
	 * Equivalent to {@code stream.reduce((a, b) -> b)}, but may perform
	 * significantly better. This method's runtime will be between O(log n) and
