...
abstract class RegularImmutableTable<R, C, V> extends ImmutableTable<R, C, V> {
	...
	static <R, C, V> RegularImmutableTable<R, C, V> forCells(List<Cell<R, C, V>> cells,
		...
		if (rowComparator != null || columnComparator != null) {
			 ...
			 * expected and that isn't documented in the Javadoc. If a row
			 * Comparator is provided, cellSet() iterates across the columns in
			 * the first row, the columns in the second row, etc. If a column
			 * Comparator is provided but a row Comparator isn't, cellSet()
			 * iterates across the rows in the first column, the rows in the
			 * second column, etc.
			 */
	...
	private static final <R, C, V> RegularImmutableTable<R, C, V> forCellsInternal(Iterable<Cell<R, C, V>> cells,
		...
		ImmutableSet<R> rowSpace = (rowComparator == null) ? ImmutableSet.copyOf(rowSpaceBuilder)
				: ImmutableSet.copyOf(ImmutableList.sortedCopyOf(rowComparator, rowSpaceBuilder));
		ImmutableSet<C> columnSpace = (columnComparator == null) ? ImmutableSet.copyOf(columnSpaceBuilder)
