...
public final class Maps {
	 ...
	 *
	 * <p>
	 * The iteration order of the returned map follows the enum's iteration
	 ...
	 * provided mapping functions to the input elements. The resulting
	 * implementation is specialized for enum key types. The returned map and
	 * its views will iterate over keys in their enum definition order, not
	 * encounter order.
	 *
	 * <p>
	 * If the mapped keys contain duplicates, an
	 * {@code IllegalArgumentException} is thrown when the collection operation
	 * is performed. (This differs from the {@code Collector} returned by
	...
	public static <T, K extends Enum<K>, V> Collector<T, ?, ImmutableMap<K, V>> toImmutableEnumMap(
		...
		return Collector.of(() -> new Accumulator<K, V>((v1, v2) -> {
	 ...
	 *
	 * <p>
	 * If the mapped keys contain duplicates, the values are merged using the
	 * specified merging function.
	 *
	 * @since 21.0
	 */
	@Beta
	public static <T, K extends Enum<K>, V> Collector<T, ?, ImmutableMap<K, V>> toImmutableEnumMap(
		...
		return Collector.of(() -> new Accumulator<K, V>(mergeFunction), (accum, t) -> {
			K key = checkNotNull(keyFunction.apply(t), "Null key for input %s", t);
			V newValue = checkNotNull(valueFunction.apply(t), "Null value for input %s", t);
			accum.put(key, newValue);
		}, Accumulator::combine, Accumulator::toImmutableMap);
	}
	 ...
	 * capacity" that it <i>should</i> hold {@code expectedSize} elements
	 * without growth. This behavior cannot be broadly guaranteed, but it is
	 * observed to be true for OpenJDK 1.7. It also can't be guaranteed that the
	 ...
	 *            the equivalence relationship to use to compare values
	 * @return the difference between the two maps
	 * @since 10.0
	 */
	public static <K, V> MapDifference<K, V> difference(Map<? extends K, ? extends V> left,
		...
		Map<K, V> onlyOnRight = new LinkedHashMap<K, V>(right); // will whittle
																// it down
	 ...
	 * natural ordering of its elements. This difference is an immutable
	 * snapshot of the state of the maps at the time this method is called. It
	 * will never change, even if the maps change at a later time.
	 *
	 * <p>
	 * Since this method uses {@code TreeMap} instances internally, the keys of
	 * the right map must all compare as distinct according to the comparator of
	 * the left map.
	 *
	 * <p>
	 * <b>Note:</b>If you only need to know whether two sorted maps have the
	...
	/**
	 * Returns a live {@link Map} view whose keys are the contents of
	 * {@code set} and whose values are computed on demand using
	 * {@code function}. To get an immutable <i>copy</i> instead, use
	 * {@link #toMap(Iterable, Function)}.
	 *
	 * <p>
	 * Specifically, for each {@code k} in the backing set, the returned map has
	 * an entry mapping {@code k} to {@code function.apply(k)}. The {@code
	 ...
	 *
	 * <p>
	 * <b>Warning:</b> If the function rejects {@code null}, caution is required
	 * to make sure the set does not contain {@code null}, because the view
	 * cannot stop {@code null} from being added to the set.
	 *
	 * <p>
	...
	private static final class NavigableAsMapView<K, V> extends AbstractNavigableMap<K, V> {
		/*
		 * Using AbstractNavigableMap is simpler than extending SortedAsMapView
		 * and rewriting all the NavigableMap methods.
		 */
	 ...
	 * The map's iteration order is the order of the first appearance of each
	 * key in {@code keys}.
	 *
	 * <p>
	 ...
	 *             if any element of {@code keys} is {@code null}, or if
	 *             {@code valueFunction} produces {@code null} for any key
	 * @since 14.0
	 */
	 ...
	 * those values. In other words, each input value produces an entry in the
	 * map whose key is the result of applying {@code keyFunction} to that
	 * value. These entries appear in the same order as the input values.
	 * Example usage:
	 * 
	 * <pre>
	 *    {@code
	 ...
	 * {@link Multimaps#index(Iterable, Function) Multimaps.index}.
	 *
	 * @param values
	 ...
	 * @throws NullPointerException
	 *             if any element of {@code values} is {@code
	 *         null}, or if {@code keyFunction} produces {@code null} for any
	 *             value
	 */
	
	public static <K, V> ImmutableMap<K, V> uniqueIndex(Iterable<V> values, Function<? super V, K> keyFunction) {
		// TODO(lowasser): consider presizing the builder if values is a
		// Collection
	 ...
	 * <p>
	 * If your index may associate multiple values with each key, use
	 * {@link Multimaps#index(Iterator, Function) Multimaps.index}.
	 *
	 * @param values
	 *            the values to use when constructing the {@code Map}
	 * @param keyFunction
	 ...
	 *             if any key in {@code Properties} is not a {@code
	 *         String}
	 * @throws NullPointerException
	 ...
	 *
	 * <p>
	 * To use a plain {@link Map} as a {@link Function}, see
	...
	private static final class BiMapConverter<A, B> extends Converter<A, B> implements Serializable {
		...
		// There's really no good way to implement toString() without printing
		// the entire BiMap, right?
		@Override
		public String toString() {
	 ...
	 * bimap, and attempts to modify the returned map, whether direct or via its
	...
	private static class UnmodifiableBiMap<K, V> extends ForwardingMap<K, V> implements BiMap<K, V>, Serializable {
		...
		public BiMap<V, K> inverse() {
			...
			return (result == null) ? inverse = new UnmodifiableBiMap<V, K>(delegate.inverse(), this) : result;
		}
	 ...
	 * original navigable map's entries. In contrast to
	 * {@link #transformValues}, this method's entry-transformation logic may
	 ...
	 * 		}
	 * 	};
	 * 	NavigableMap<String, String> transformed = LabsMaps.transformNavigableEntries(options, flagPrefixer);
	 * 	System.out.println(transformed);
	 * }
	 * </pre>
	 *
	...
	static <K, V1, V2> Function<V1, V2> asValueToValueFunction(final EntryTransformer<? super K, V1, V2> transformer,
			final K key) {
	...
	static class TransformedEntriesMap<K, V1, V2> extends IteratorBasedAbstractMap<K, V2> {
		...
		public V2 getOrDefault( Object key,  V2 defaultValue) {
			...
			return (value != null || fromMap.containsKey(key)) ? transformer.transformEntry((K) key, value)
					: defaultValue;
		}
		...
		public V2 remove(Object key) {
			return fromMap.containsKey(key) ? transformer.transformEntry((K) key, fromMap.remove(key)) : null;
		}
		...
		Spliterator<Entry<K, V2>> entrySpliterator() {
			return CollectSpliterators.map(fromMap.entrySet().spliterator(),
					Maps.<K, V1, V2>asEntryToEntryFunction(transformer));
		}
	...
	@GwtIncompatible // NavigableMap
	private static class TransformedEntriesNavigableMap<K, V1, V2> extends TransformedEntriesSortedMap<K, V1, V2>
	 ...
	 *
	 * <p>
	 * The resulting map's {@code keySet()}, {@code entrySet()}, and {@code
	 * values()} views have iterators that don't support {@code remove()}, but
	 * all other methods are supported by the map and its views. When given a
	 * key that doesn't satisfy the predicate, the map's {@code put()} and
	 ...
	 *
	 * <p>
	 * The returned map isn't threadsafe or serializable, even if {@code
	 * unfiltered} is.
	 *
	 * <p>
	...
	public static <K, V> SortedMap<K, V> filterKeys(SortedMap<K, V> unfiltered,
		...
		// TODO(lowasser): Return a subclass of Maps.FilteredKeyMap for slightly
		// better
		// performance.
	 ...
	 * whose keys satisfy a predicate. The returned map is a live view of {@code
	 * unfiltered}; changes to one affect the other.
	 *
	 * <p>
	 * The resulting map's {@code keySet()}, {@code entrySet()}, and {@code
	...
	/**
	 * Returns a bimap containing the mappings in {@code unfiltered} whose keys
	 * satisfy a predicate. The returned bimap is a live view of
	 * {@code unfiltered}; changes to one affect the other.
	 *
	 * <p>
	 * The resulting bimap's {@code keySet()}, {@code entrySet()}, and
	 * {@code values()} views have iterators that don't support
	 * {@code remove()}, but all other methods are supported by the bimap and
	 * its views. When given a key that doesn't satisfy the predicate, the
	 * bimap's {@code
	 ...
	 * <p>
	 * When methods such as {@code removeAll()} and {@code clear()} are called
	 * on the filtered bimap or its views, only mappings that satisfy the filter
	 * will be removed from the underlying bimap.
	 *
	 * <p>
	 * The returned bimap isn't threadsafe or serializable, even if
	 * {@code unfiltered} is.
	 *
	 * <p>
	 * Many of the filtered bimap's methods, such as {@code size()}, iterate
	 * across every key in the underlying bimap and determine which satisfy the
	 * filter. When a live view is <i>not</i> needed, it may be faster to copy
	 * the filtered bimap and use the copy.
	 *
	 * <p>
	 * <b>Warning:</b> {@code entryPredicate} must be <i>consistent with equals
	 ...
	 * values()} views have iterators that don't support {@code remove()}, but
	 * all other methods are supported by the map and its views. When given a
	 * value that doesn't satisfy the predicate, the map's {@code put()}, {@code
	 * putAll()}, and {@link Entry#setValue} methods throw an
	 * {@link IllegalArgumentException}.
	 *
	 * <p>
	 * When methods such as {@code removeAll()} and {@code clear()} are called
	...
	/**
	 * Returns a navigable map containing the mappings in {@code unfiltered}
	 * whose values satisfy a predicate. The returned map is a live view of
	 * {@code
	 * unfiltered}; changes to one affect the other.
	 *
	 * <p>
	...
	/**
	 * Returns a bimap containing the mappings in {@code unfiltered} whose
	 * values satisfy a predicate. The returned bimap is a live view of
	 * {@code unfiltered}; changes to one affect the other.
	 *
	 * <p>
	 * The resulting bimap's {@code keySet()}, {@code entrySet()}, and
	 * {@code values()} views have iterators that don't support
	 * {@code remove()}, but all other methods are supported by the bimap and
	 * its views. When given a value that doesn't satisfy the predicate, the
	 * bimap's {@code put()}, {@code forcePut()} and {@code putAll()} methods
	 * throw an {@link IllegalArgumentException}. Similarly, the map's entries
	 * have a {@link Entry#setValue} method that throws an
	 * {@link IllegalArgumentException} when the provided value doesn't satisfy
	 * the predicate.
	 *
	 * <p>
	 * When methods such as {@code removeAll()} and {@code clear()} are called
	 ...
	 * <p>
	 * Many of the filtered bimap's methods, such as {@code size()}, iterate
	 * across every value in the underlying bimap and determine which satisfy
	 * the filter. When a live view is <i>not</i> needed, it may be faster to
	 * copy the filtered bimap and use the copy.
	 *
	 * <p>
	 * <b>Warning:</b> {@code entryPredicate} must be <i>consistent with equals
	 * </i>, as documented at {@link Predicate#apply}.
	 ...
	 * values()} views have iterators that don't support {@code remove()}, but
	 * all other methods are supported by the map and its views. When given a
	 * key/value pair that doesn't satisfy the predicate, the map's
	...
	/**
	 * Returns a bimap containing the mappings in {@code unfiltered} that
	 * satisfy a predicate. The returned bimap is a live view of
	 * {@code unfiltered}; changes to one affect the other.
	 *
	 * <p>
	 ...
	 * {@code values()} views have iterators that don't support
	 * {@code remove()}, but all other methods are supported by the bimap and
	 * its views. When given a key/value pair that doesn't satisfy the
	 * predicate, the bimap's {@code put()}, {@code forcePut()} and
	 * {@code putAll()} methods throw an {@link IllegalArgumentException}.
	 * Similarly, the map's entries have an {@link Entry#setValue} method that
	 ...
	 * <p>
	 * Many of the filtered bimap's methods, such as {@code size()}, iterate
	 * across every key/value mapping in the underlying bimap and determine
	 * which satisfy the filter. When a live view is <i>not</i> needed, it may
	 * be faster to copy the filtered bimap and use the copy.
	 *
	 * <p>
	 * <b>Warning:</b> {@code entryPredicate} must be <i>consistent with equals
	 * </i>, as documented at {@link Predicate#apply}.
	...
	static class FilteredEntryMap<K, V> extends AbstractFilteredMap<K, V> {
		...
		static <K, V> boolean removeAllKeys(Map<K, V> map, Predicate<? super Entry<K, V>> entryPredicate,
		...
		static <K, V> boolean retainAllKeys(Map<K, V> map, Predicate<? super Entry<K, V>> entryPredicate,
				Collection<?> keyCollection) {
			Iterator<Entry<K, V>> entryItr = map.entrySet().iterator();
			boolean result = false;
			while (entryItr.hasNext()) {
	...
	private static class FilteredEntrySortedMap<K, V> extends FilteredEntryMap<K, V> implements SortedMap<K, V> {
		...
		@Override
		public K firstKey() {
			// correctly throws NoSuchElementException when filtered map is
			// empty.
	...
	private static class FilteredEntryNavigableMap<K, V> extends AbstractNavigableMap<K, V> {
		/*
		 * It's less code to extend AbstractNavigableMap and forward the
		 * filtering logic to FilteredEntryMap than to extend
		 * FilteredEntrySortedMap and reimplement all the NavigableMap methods.
		 */
	...
	static final class FilteredEntryBiMap<K, V> extends FilteredEntryMap<K, V> implements BiMap<K, V> {
		...
		FilteredEntryBiMap(BiMap<K, V> delegate, Predicate<? super Entry<K, V>> predicate) {
			...
			this.inverse = new FilteredEntryBiMap<V, K>(delegate.inverse(), inversePredicate(predicate), this);
		}
	...
	/**
	 * Returns an unmodifiable view of the specified navigable map. Query
	 * operations on the returned map read through to the specified map, and
	 * attempts to modify the returned map, whether direct or via its views,
	 ...
	 * V>} to a {@code NavigableMap<K, V>}. If it permitted this, the returned
	 * map's {@code
	 * comparator()} method might return a {@code Comparator<? extends K>},
	 * which works only on a particular subtype of {@code K}, but promise that
	 * it's a {@code Comparator<? super K>}, which must work on any type of
	 * {@code K}.
	 *
	 * @param map
	...
	private static <K, V> Entry<K, V> unmodifiableOrNull( Entry<K, ? extends V> entry) {
	...
	static class UnmodifiableNavigableMap<K, V> extends ForwardingSortedMap<K, V>
		...
		public NavigableMap<K, V> descendingMap() {
			...
			return (result == null) ? descendingMap = new UnmodifiableNavigableMap<K, V>(delegate.descendingMap(), this)
					: result;
		}
	 ...
	 * critical that <b>all</b> access to the backing navigable map is
	 ...
	 * {@code headMap} or {@code tailMap} views.
	 * 
	 * <pre>
	 * {
	 * 	&#64;code
	 ...
	 * 		Iterator<K> it = set2.iterator(); // Must be in synchronized
	 * 											// block
	 * 		while (it.hasNext()) {
	 * 			foo(it.next());
	 * 		}
	...
	/**
	 * {@code AbstractMap} extension that makes it easy to cache customized
	...
	static class KeySet<K, V> extends Sets.ImprovedAbstractSet<K> {
		...
		public void forEach(Consumer<? super K> action) {
			checkNotNull(action);
			// avoids entry allocation for those maps that allocate entries on
			// iteration
	...
	static class Values<K, V> extends AbstractCollection<V> {
		...
		public void forEach(Consumer<? super V> action) {
			checkNotNull(action);
			// avoids allocation of entries for those maps that generate fresh
	 ...
	 *
	 * <p>
	 * This method delegates to the appropriate methods of {@link NavigableMap}
	 * (namely {@link NavigableMap#subMap(Object, boolean, Object, boolean)
	 * subMap()}, {@link NavigableMap#tailMap(Object, boolean) tailMap()}, and
	 * {@link NavigableMap#headMap(Object, boolean) headMap()}) to actually
	 * construct the view. Consult these methods for a full description of the
	 * returned view's behavior.
	 *
	 * <p>
	 * <b>Warning:</b> {@code Range}s always represent a range of values using
	 * the values' natural ordering. {@code NavigableMap} on the other hand can
	 * specify a custom ordering via a {@link Comparator}, which can violate the
	 * natural ordering. Using this method (or in general using {@code Range})
