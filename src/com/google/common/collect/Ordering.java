 ...
 * an "enriched" version of {@code Comparator} for pre-Java-8 users, in the same
 * sense that {@link FluentIterable} is an enriched {@link Iterable} for
 * pre-Java-8 users.
 *
 * <h3>Three types of methods</h3>
 *
 * Like other fluent types, there are three types of methods present: methods
 * for <i>acquiring</i>, <i>chaining</i>, and <i>using</i>.
 *
 * <h4>Acquiring</h4>
 *
 * <p>
 ...
 *
 * <p>
 * Complex chained orderings like the following example can be challenging to
 * understand.
 *
 * <pre>
 * {
 * 	&#64;code
 * 	Ordering<Foo> ordering = Ordering.natural().nullsFirst().onResultOf(getBarFunction).nullsLast();
 * }
 * </pre>
 *
 * Note that each chaining method returns a new ordering instance which is
 * backed by the previous instance, but has the chance to act on values
 * <i>before</i> handing off to that backing instance. As a result, it usually
 * helps to read chained ordering expressions <i>backwards</i>. For example,
 * when {@code compare} is called on the above ordering:
 *
 * <ol>
 * <li>First, if only one {@code Foo} is null, that null value is treated as
 * <i>greater</i>
 * <li>Next, non-null {@code Foo} values are passed to {@code getBarFunction}
 * (we will be comparing {@code Bar} values from now on)
 * <li>Next, if only one {@code Bar} is null, that null value is treated as
 * <i>lesser</i>
 * <li>Finally, natural ordering is used (i.e. the result of
 * {@code Bar.compareTo(Bar)} is returned)
 * </ol>
 *
 * <p>
 * Alas, {@link #reverse} is a little different. As you read backwards through a
 * chain and encounter a call to {@code reverse}, continue working backwards
 ...
 *
 * <p>
 * If you are using Java 8, this class is now obsolete <i>(pending a few August
 * 2016 updates)</i>. Most of its functionality is now provided by
 * {@link java.util.stream.Stream Stream} and by {@link Comparator} itself, and
 * the rest can now be found as static methods in our new {@link Comparators}
 * class. See each method below for further instructions. Whenever possible, you
 * should change any references of type {@code Ordering} to be of type
 * {@code Comparator} instead. However, at this time we have no plan to
 * <i>deprecate</i> this class.
 *
 * <p>
 * Many replacements involve adopting {@code Stream}, and these changes can
 * sometimes make your code verbose. Whenever following this advice, you should
 * check whether {@code Stream} could be adopted more comprehensively in your
 * code; the end result may be quite a bit simpler.
 *
 * <h3>See also</h3>
 *
 * <p>
...
public abstract class Ordering<T> implements Comparator<T> {
	 ...
	 * results in no change to the order of elements. Note especially that
	 * {@link #sortedCopy} and {@link #immutableSortedCopy} are stable, and in
	 * the returned instance these are implemented by simply copying the source
	 * list.
	 *
	 * <p>
	 * Example:
	 ...
	 * {@code [t, e, s, t,
	 * null, null, null]} regardless of the true comparison order of those three
	 * values (which might not even implement {@link Comparable} at all).
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * <b>Java 8 users:</b> Use the lambda expression {@code (a, b) -> 0}
	 * instead (in certain cases you may need to cast that to
	 * {@code Comparator<YourType>}).
	 *
	 * @since 13.0
	 */
	@GwtCompatible(serializable = true)
	 ...
	 *
	 * <p>
	 * <b>Java 8 users:</b> Use {@code Comparator.comparing(Object::toString)}
	 * instead.
	 */
	@GwtCompatible(serializable = true)
	 ...
	 * {@code compare(a, b) == 0} implies {@code a == b} (identity equality).
	 * There is no meaning whatsoever to the order imposed, but it is constant
	...
	static class ArbitraryOrdering extends Ordering<Object> {
		...
		private Integer getUid(Object obj) {
			...
			if (uid == null) {
				// One or more integer values could be skipped in the event of a
				// race
				// to generate a UID for the same object from multiple threads,
				// but
	 ...
	 * values and uses {@code
	 * this} to compare non-null values.
	 *
	 * <p>
	 * <b>Java 8 users:</b> Use {@code Comparator.nullsFirst(thisComparator)}
	 * instead.
	 */
	// type parameter <S> lets us avoid the extra <String> in statements like:
	 ...
	 *
	 * <p>
	 * <b>Java 8 users:</b> Use {@code Comparator.nullsLast(thisComparator)}
	 * instead.
	 */
	// type parameter <S> lets us avoid the extra <String> in statements like:
	 ...
	 * {@code Comparator.comparing(function, thisComparator)} instead (you can
	 * omit the comparator if it is the natural order).
	 */
	@GwtCompatible(serializable = true)
	 ...
	 * <p>
	 * <b>Java 8 users:</b> Use
	 * {@code thisComparator.thenComparing(secondaryComparator)} instead.
	 * Depending on what {@code secondaryComparator} is, one of the other
	 * overloads of {@code
	 ...
	 * {@code comparatorCollection.stream().reduce(Comparator::thenComparing).get()}
	 * (if the collection might be empty, also provide a default comparator as
	 * the {@code identity} parameter to {@code reduce}).
	 *
	 * @param comparators
	 ...
	 * iterator will be left exhausted: its {@code
	 * hasNext()} method will return {@code false}.
	 *
	 * <p>
	 * <b>Java 8 users:</b> Continue to use this method for now. After the next
	 * release of Guava, use
	 * {@code Streams.stream(iterator).min(thisComparator).get()} instead (but
	 ...
	 *
	 * <p>
	 * <b>Java 8 users:</b> If {@code iterable} is a {@link Collection}, use
	 * {@code
	 * Collections.min(collection, thisComparator)} instead. Otherwise, continue
	 * to use this method for now. After the next release of Guava, use {@code
	 * Streams.stream(iterable).min(thisComparator).get()} instead. Note that
	 * these alternatives do not guarantee which tied minimum element is
	 * returned)
	 *
	 * @param iterable
	 ...
	 * <p>
	 * <b>Java 8 users:</b> Use
	 * {@code Collections.min(Arrays.asList(a, b), thisComparator)} instead (but
	 * note that it does not guarantee which tied minimum element is returned).
	 *
	 * @param a
	 ...
	 * <p>
	 * <b>Java 8 users:</b> Use
	 * {@code Collections.min(Arrays.asList(a, b, c...), thisComparator)}
	 ...
	 * The iterator will be left exhausted: its {@code hasNext()} method will
	 * return {@code false}.
	 *
	 * <p>
	 * <b>Java 8 users:</b> Continue to use this method for now. After the next
	 * release of Guava, use
	 * {@code Streams.stream(iterator).max(thisComparator).get()} instead (but
	 ...
	 * <b>Java 8 users:</b> If {@code iterable} is a {@link Collection}, use
	 * {@code
	 * Collections.max(collection, thisComparator)} instead. Otherwise, continue
	 * to use this method for now. After the next release of Guava, use {@code
	 * Streams.stream(iterable).max(thisComparator).get()} instead. Note that
	 * these alternatives do not guarantee which tied maximum element is
	 * returned)
	 *
	 * @param iterable
	 ...
	 * <p>
	 * <b>Java 8 users:</b> Use
	 * {@code Collections.max(Arrays.asList(a, b), thisComparator)} instead (but
	 * note that it does not guarantee which tied maximum element is returned).
	 *
	 * @param a
	 ...
	 * <p>
	 * <b>Java 8 users:</b> Use
	 * {@code Collections.max(Arrays.asList(a, b, c...), thisComparator)}
	 * instead (but note that it does not guarantee which tied maximum element
	 * is returned).
	 *
	 * @param a
	...
	public <E extends T> List<E> leastOf(Iterable<E> iterable, int k) {
		if (iterable instanceof Collection) {
			...
			if (collection.size() <= 2L * k) {
				// In this case, just dumping the collection to an array and
				// sorting is
				// faster than using the implementation for Iterator, which is
				// specialized for k much smaller than n.

				@SuppressWarnings("unchecked") // c only contains E's and
												// doesn't escape
	 ...
	 * <b>Java 8 users:</b> Continue to use this method for now. After the next
	 * release of Guava, use
	 * {@code Streams.stream(iterator).collect(Comparators.least(k, thisComparator))}
	 * instead.
	 *
	 * @return an immutable {@code RandomAccess} list of the {@code k} least
	 *         elements in ascending order
	...
	public <E extends T> List<E> leastOf(Iterator<E> iterator, int k) {
		...
		if (k == 0 || !iterator.hasNext()) {
			...
			// k is really large; just do a straightforward
	 ...
	 * <b>Java 8 users:</b> Continue to use this method for now. After the next
	 * release of Guava, use
	 * {@code Streams.stream(iterator).collect(Comparators.greatest(k, thisComparator))}
	 * instead.
	 *
	 * @return an immutable {@code RandomAccess} list of the {@code k} greatest
	 *         elements in <i>descending order</i>
	 ...
	 * space) than this method, and this method in turn generally performs
	 * better than copying the list and calling {@link Collections#sort(List)}.
	 */
	...
	/**
	 * Returns an <b>immutable</b> list containing {@code elements} sorted by
	 * this ordering. The input is not modified.
	 *
	 * <p>
	 * Unlike {@link Sets#newTreeSet(Iterable)}, this method does not discard
	 * elements that are duplicates according to the comparator. The sort
	 ...
	 * <p>
	 * <b>Performance note:</b> According to our benchmarking on Open JDK 7,
	 * this method is the most efficient way to make a sorted copy of a
	 * collection.
	 *
	 * @throws NullPointerException
	 *             if any element of {@code elements} is {@code null}
	 * @since 3.0
	 */
	// TODO(kevinb): rerun benchmarks including new options
	@CanIgnoreReturnValue // TODO(kak): Consider removing this before internal
							// migration
	 ...
	 * {@link Comparators#isInOrder(Iterable, Comparator)} instead, since the
	 * rest of {@code Ordering} is mostly obsolete (as explained in the class
	 * documentation).
	 */
	 ...
	 *             directly. This method is scheduled for deletion in June 2018.
	 */
	@Deprecated
