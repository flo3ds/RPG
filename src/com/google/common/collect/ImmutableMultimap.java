...
/**
 * A {@link Multimap} whose contents will never change, with many other
 * important properties detailed at {@link ImmutableCollection}.
 *
 * <p>
 * <b>Warning:</b> avoid <i>direct</i> usage of {@link ImmutableMultimap} as a
 * type (as with {@link Multimap} itself). Prefer subtypes such as
 * {@link ImmutableSetMultimap} or {@link ImmutableListMultimap}, which have
 * well-defined {@link #equals} semantics, thus avoiding a common source of bugs
 * and confusion.
 *
 * <p>
 * <b>Note:</b> every {@link ImmutableMultimap} offers an {@link #inverse} view,
 * so there is no need for a distinct {@code ImmutableBiMultimap} type.
 *
 * <a name="iteration"></a>
 * <p>
 * <b>Key-grouped iteration.</b> All view collections follow the same iteration
 * order. In all current implementations, the iteration order always keeps
 * multiple entries with the same key together. Any creation method that would
 * customarily respect insertion order (such as {@link #copyOf(Multimap)})
...
public abstract class ImmutableMultimap<K, V> extends AbstractMultimap<K, V> implements Serializable {
	 ...
	 * "key-grouped" insertion order described in the <a href="#iteration">class
	 * documentation</a>.
	 */
	...
	public static class Builder<K, V> {
		 ...
		 * builder generated by {@link ImmutableMultimap#builder}.
		 */
		public Builder() {
		...
		public ImmutableMultimap<K, V> build() {
			...
			if (keyComparator != null) {
				...
				List<Map.Entry<K, Collection<V>>> entries = Ordering.from(keyComparator).<K>onKeys()
	...
	public static <K, V> ImmutableMultimap<K, V> copyOf(Multimap<? extends K, ? extends V> multimap) {
		if (multimap instanceof ImmutableMultimap) {
			@SuppressWarnings("unchecked") // safe since multimap is not
											// writable
	 ...
	 * collection is returned. The values are in the same order as the
	 ...
	 * multimap's methods. This is generally used to determine whether
	...
	/**
	 * Returns an immutable set of the distinct keys in this multimap, in the
	 * same order as they appear in this multimap.
	 */
	@Override
	 ...
	 * values in the multimap. Keys and values appear in the same order as in
	 * this multimap.
	 */
	@Override
	...
	/**
	 * Returns an immutable multiset containing all the keys in this multimap,
	 * in the same order and with the same frequencies as they appear in this
	 * multimap; to get only a single occurrence of each key, use
	 * {@link #keySet}.
	 */
	@Override
	 ...
	 * iterator traverses the values for the first key, the values for the
