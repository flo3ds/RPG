 ...
 * @param <E>
 *            the type of the {@link InternalEntry} entry implementation used
 *            internally
 * @param <S>
 *            the type of the {@link Segment} entry implementation used
 *            internally
 * @author Bob Lee
 * @author Charles Fry
...
class MapMakerInternalMap<K, V, E extends MapMakerInternalMap.InternalEntry<K, V, E>, S extends MapMakerInternalMap.Segment<K, V, E, S>>
	 ...
	 * either of the constructors with arguments. MUST be a power of two no
	 * greater than {@code 1<<30} to ensure that entries are indexable using
	 * ints.
	 */
	...
	/**
	 * Strategy for handling entries and segments in a type-safe and efficient
	 * manner.
	 */
	...
	/**
	 * Returns a fresh {@link MapMakerInternalMap} with {@link MapMaker.Dummy}
	 * values but otherwise as specified by the given {@code builder}. The
	 * returned {@link MapMakerInternalMap} will be optimized to saved memory.
	 * Since {@link MapMaker.Dummy} is a singleton, we don't need to store any
	 * values at all. Because of this optimization,
	 * {@code build.getValueStrength()} must be {@link Strength#STRONG}.
	 *
	 * <p>
	 * This method is intended to only be used by the internal implementation of
	 * {@link Interners}, since a map of dummy values is the exact use case
	 * there.
	 */
	static <K> MapMakerInternalMap<K, Dummy, ? extends InternalEntry<K, Dummy, ?>, ?> createWithDummyValues(
			MapMaker builder) {
		if (builder.getKeyStrength() == Strength.STRONG && builder.getValueStrength() == Strength.STRONG) {
			return new MapMakerInternalMap<K, Dummy, StrongKeyDummyValueEntry<K>, StrongKeyDummyValueSegment<K>>(
					builder, StrongKeyDummyValueEntry.Helper.<K>instance());
		}
		if (builder.getKeyStrength() == Strength.WEAK && builder.getValueStrength() == Strength.STRONG) {
			return new MapMakerInternalMap<K, Dummy, WeakKeyDummyValueEntry<K>, WeakKeyDummyValueSegment<K>>(builder,
					WeakKeyDummyValueEntry.Helper.<K>instance());
		}
	...
	/**
	 * A helper object for operating on {@link InternalEntry} instances in a
	 * type-safe and efficient manner.
	 *
	 * <p>
	 * For each of the four combinations of strong/weak key and strong/weak
	 * value, there are corresponding {@link InternalEntry}, {@link Segment},
	 * and {@link InternalEntryHelper} implementations.
	 *
	 * @param <K>
	 *            the type of the key in each entry
	 * @param <V>
	 *            the type of the value in each entry
	 * @param <E>
	 *            the type of the {@link InternalEntry} entry implementation
	 * @param <S>
	 *            the type of the {@link Segment} entry implementation
	 */
	interface InternalEntryHelper<K, V, E extends InternalEntry<K, V, E>, S extends Segment<K, V, E, S>> {
		...
		/**
		 * Returns a freshly created entry, typed at the {@code E} type, for the
		 * given {@code segment}.
		 */
		...
		/**
		 * Returns a freshly created entry, typed at the {@code E} type, for the
		 * given {@code segment}, that is a copy of the given {@code entry}.
		 */
		E copy(S segment, E entry, @Nullable E newNext);

		/**
		 * Sets the value of the given {@code entry} in the given
		 * {@code segment} to be the given {@code
		 * value}
		 */
	...
	/*
	 * Note: the following classes have a lot of duplicate code. It sucks, but
	 * it saves a lot of memory. If only Java had mixins!
	 */
	...
	/**
	 * Marker interface for {@link InternalEntry} implementations for strong
	 * values.
	 */
	...
	/**
	 * Marker interface for {@link InternalEntry} implementations for weak
	 * values.
	 */
	interface WeakValueEntry<K, V, E extends InternalEntry<K, V, E>> extends InternalEntry<K, V, E> {
		...
		/**
		 * Clears the weak value reference held by the entry. Should be used
		 * when the entry's value is overwritten.
		 */
		void clearValue();
	}

	@SuppressWarnings("unchecked") // impl never uses a parameter or returns any
									// non-null value
	...
	static final class StrongKeyStrongValueEntry<K, V>
		...
		StrongKeyStrongValueEntry<K, V> copy(StrongKeyStrongValueEntry<K, V> newNext) {
			StrongKeyStrongValueEntry<K, V> newEntry = new StrongKeyStrongValueEntry<K, V>(this.key, this.hash,
					newNext);
		...
		/**
		 * Concrete implementation of {@link InternalEntryHelper} for strong
		 * keys and strong values.
		 */
	...
	/**
	 * Concrete implementation of {@link InternalEntry} for strong keys and weak
	 * values.
	 */
	static final class StrongKeyWeakValueEntry<K, V> extends AbstractStrongKeyEntry<K, V, StrongKeyWeakValueEntry<K, V>>
		...
		void setValue(V value, ReferenceQueue<V> queueForValues) {
			...
			this.valueReference = new WeakValueReferenceImpl<K, V, StrongKeyWeakValueEntry<K, V>>(queueForValues, value,
					this);
			previous.clear();
		}
	 ...
	 * {@link Dummy} values.
	 */
	static final class StrongKeyDummyValueEntry<K> extends AbstractStrongKeyEntry<K, Dummy, StrongKeyDummyValueEntry<K>>
		...
		/**
		 * Concrete implementation of {@link InternalEntryHelper} for strong
		 * keys and {@link Dummy} values.
		 */
		static final class Helper<K>
				implements InternalEntryHelper<K, Dummy, StrongKeyDummyValueEntry<K>, StrongKeyDummyValueSegment<K>> {
			...
			public StrongKeyDummyValueSegment<K> newSegment(
					MapMakerInternalMap<K, Dummy, StrongKeyDummyValueEntry<K>, StrongKeyDummyValueSegment<K>> map,
					int initialCapacity, int maxSegmentSize) {
	 ...
	 * {@link Dummy} values.
	 */
	static final class WeakKeyDummyValueEntry<K> extends AbstractWeakKeyEntry<K, Dummy, WeakKeyDummyValueEntry<K>>
		...
		/**
		 * Concrete implementation of {@link InternalEntryHelper} for weak keys
		 * and {@link Dummy} values.
		 */
		static final class Helper<K>
				implements InternalEntryHelper<K, Dummy, WeakKeyDummyValueEntry<K>, WeakKeyDummyValueSegment<K>> {
			private static final Helper<?> INSTANCE = new Helper<Object>();
	...
	/**
	 * Concrete implementation of {@link InternalEntry} for weak keys and strong
	 * values.
	 */
	static final class WeakKeyStrongValueEntry<K, V> extends AbstractWeakKeyEntry<K, V, WeakKeyStrongValueEntry<K, V>>
		...
		WeakKeyStrongValueEntry<K, V> copy(ReferenceQueue<K> queueForKeys, WeakKeyStrongValueEntry<K, V> newNext) {
			WeakKeyStrongValueEntry<K, V> newEntry = new WeakKeyStrongValueEntry<K, V>(queueForKeys, getKey(),
					this.hash, newNext);
	...
	/**
	 * Concrete implementation of {@link InternalEntry} for weak keys and weak
	 * values.
	 */
	static final class WeakKeyWeakValueEntry<K, V> extends AbstractWeakKeyEntry<K, V, WeakKeyWeakValueEntry<K, V>>
		...
		WeakKeyWeakValueEntry<K, V> copy(ReferenceQueue<K> queueForKeys, ReferenceQueue<V> queueForValues,
			...
			WeakKeyWeakValueEntry<K, V> newEntry = new WeakKeyWeakValueEntry<K, V>(queueForKeys, getKey(), this.hash,
					newNext);
			newEntry.valueReference = valueReference.copyFor(queueForValues, newEntry);
			return newEntry;
		}
		...
		void setValue(V value, ReferenceQueue<V> queueForValues) {
			...
			this.valueReference = new WeakValueReferenceImpl<K, V, WeakKeyWeakValueEntry<K, V>>(queueForValues, value,
					this);
			previous.clear();
		}
	...
	/**
	 * A weakly referenced value that also has a reference to its containing
	 * entry.
	 */
	interface WeakValueReference<K, V, E extends InternalEntry<K, V, E>> {
		/**
		 * Returns the current value being referenced, or {@code null} if there
		 * is none (e.g. because either it got collected, or {@link #clear} was
		 * called, or it wasn't set in the first place).
		 */
		@Nullable
		V get();
		...
		/**
		 * Unsets the referenced value. Subsequent calls to {@link #get} will
		 * return {@code null}.
		 */
		void clear();
		 ...
		 * {@code entry} (and on the given {@code queue} with the same value as
		 * this {@link WeakValueReference}.
		 */
		WeakValueReference<K, V, E> copyFor(ReferenceQueue<V> queue, E entry);
	}

	/**
	 * A dummy implementation of {@link InternalEntry}, solely for use in the
	 * type signature of {@link #UNSET_WEAK_VALUE_REFERENCE} below.
	 */
	...
	/**
	 * A singleton {@link WeakValueReference} used to denote an unset value in a
	 * entry with weak values.
	 */
	...
	/**
	 * Gets the value from an entry. Returns {@code null} if the entry is
	 * invalid, partially-collected or computing.
	 */
	...
	abstract static class Segment<K, V, E extends InternalEntry<K, V, E>, S extends Segment<K, V, E, S>>
		...
		/**
		 * Returns {@code this} up-casted to the specific {@link Segment}
		 * implementation type {@code S}.
		 *
		 * <p>
		 * This method exists so that the {@link Segment} code can be generic in
		 * terms of {@code S}, the type of the concrete implementation.
		 */
		abstract S self();
		 ...
		 *
		 * <p>
		 * This method is provided as a convenience for tests. Otherwise they'd
		 * need to be knowledgable about all the implementation details of our
		...
		/**
		 * Unsafely creates of a fresh {@link WeakValueReference}, referencing
		 * the given {@code value}, for the given {@code entry}
		 */
		...
		/**
		 * Unsafely sets the weak value reference inside the given {@code entry}
		 * to be the given {@code
		 * valueReference}
		 */
		...
		/**
		 * Unsafely sets the given index of this segment's internal hash table
		 * to be the given entry.
		 */
		...
		/**
		 * Unsafely removes the given entry from the given chain in this
		 * segment's hash table.
		 */
		 ...
		 *
		 * <p>
		 * This method does not decrement count for the removed entry, but does
		 * decrement count for all partially collected entries which are
		 * skipped. As such callers which are modifying count must re-read it
		 * after calling removeFromChain.
		 *
		 * @param first
		...
		/**
		 * Clears a value that has not yet been set, and thus does not require
		 * count to be modified.
		 */
		@CanIgnoreReturnValue
		...
		/**
		 * Returns {@code true} if the value has been partially collected,
		 * meaning that the value is null.
		 */
		...
		/**
		 * Gets the value from an entry. Returns {@code null} if the entry is
		 * invalid or partially-collected.
		 */
		@Nullable
		V getLiveValue(E entry) {
			if (entry.getKey() == null) {
		 ...
		 * during writes, or from the cleanupExecutor. If cleanup is not
		 * observed after a sufficient number of reads, try cleaning up from the
		 * read thread.
		 */
	...
	/**
	 * Concrete implementation of {@link Segment} for strong keys and strong
	 * values.
	 */
	...
	/**
	 * Concrete implementation of {@link Segment} for strong keys and weak
	 * values.
	 */
	...
	/**
	 * Concrete implementation of {@link Segment} for weak keys and strong
	 * values.
	 */
	...
	/**
	 * Concrete implementation of {@link Segment} for weak keys and
	 * {@link Dummy} values.
	 */
	...
	/**
	 * Returns the internal entry for the specified key. The entry may be
	 * computing or partially collected. Does not impact recency ordering.
	 */
	...
	public boolean containsValue(@Nullable Object value) {
		...
		// This implementation is patterned after ConcurrentHashMap, but without
		// the locking. The only
		// way for it to return a false negative would be for the target value
		// to jump around in the map
		// such that none of the subsequent iterations observed it, despite the
		// fact that at every point
		// in time it was present somewhere int the map. This becomes
		// increasingly unlikely as
		// CONTAINS_VALUE_RETRIES increases, though without locking it is
	...
	abstract class HashIterator<T> implements Iterator<T> {
		...
		/**
		 * Finds the next entry in the current chain. Returns {@code true} if an
		 * entry was found.
		 */
		boolean nextInChain() {
			if (nextEntry != null) {
		...
		/**
		 * Finds the next entry in the current table. Returns {@code true} if an
		 * entry was found.
		 */
		boolean nextInTable() {
		...
		/**
		 * Advances to the given entry. Returns {@code true} if the entry was
		 * valid, {@code false} if it should be skipped.
		 */
	...
	final class Values extends AbstractCollection<V> {
		...
		// super.toArray() may misbehave if size() is inaccurate, at least on
	...
	private static <E> ArrayList<E> toArrayList(Collection<E> c) {
		// Avoid calling ArrayList(Collection), which may call back into
		// toArray.
