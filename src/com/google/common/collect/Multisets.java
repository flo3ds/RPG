...
public final class Multisets {
	...
	static class UnmodifiableMultiset<E> extends ForwardingMultiset<E> implements Serializable {
		...
		public Set<Multiset.Entry<E>> entrySet() {
			...
			return (es == null)
					// Safe because the returned set is made unmodifiable and
					// Entry
					// itself is readonly
	 ...
	 * serializable.
	 *
	 * @param sortedMultiset
	 *            the sorted multiset for which an unmodifiable view is to be
	 *            generated
	 * @return an unmodifiable view of the multiset
	 * @since 11.0
	 */
	 ...
	 *
	 * <p>
	 * The resulting multiset's iterators, and those of its {@code entrySet()}
	 * and {@code elementSet()}, do not support {@code remove()}. However, all
	 * other multiset methods supported by {@code unfiltered} are supported by
	 * the returned multiset. When given an element that doesn't satisfy the
	 * predicate, the multiset's {@code add()} and {@code addAll()} methods
	 * throw an {@link IllegalArgumentException}. When methods such as
	 * {@code removeAll()} and {@code clear()} are called on the filtered
	 * multiset, only elements that satisfy the filter will be removed from the
	 * underlying multiset.
	 *
	 * <p>
	 ...
	 * across every element in the underlying multiset and determine which
	 * elements satisfy the filter. When a live view is <i>not</i> needed, it
	 * may be faster to copy the returned multiset and use the copy.
	 *
	 * <p>
	 * <b>Warning:</b> {@code predicate} must be <i>consistent with equals</i>,
	...
	/**
	 * Returns an unmodifiable view of the union of two multisets. In the
	 * returned multiset, the count of each element is the <i>maximum</i> of its
	 * counts in the two backing multisets. The iteration order of the returned
	 * multiset matches that of the element set of {@code multiset1} followed by
	 * the members of the element set of {@code multiset2} that are not
	 * contained in {@code multiset1}, with repeated occurrences of the same
	 ...
	 * counts in the two backing multisets, with elements that would have a
	 * count of 0 not included. The iteration order of the returned multiset
	 ...
	 * multiset, the count of each element is the <i>sum</i> of its counts in
	 * the two backing multisets. The iteration order of the returned multiset
	 * matches that of the element set of {@code multiset1} followed by the
	 * members of the element set of {@code multiset2} that are not contained in
	 ...
	 * <i>zero-truncated subtraction</i> of its count in the second multiset
	 * from its count in the first multiset, with elements that would have a
	 * count of 0 not included. The iteration order of the returned multiset
	 * matches that of the element set of {@code multiset1}, with repeated
	 * occurrences of the same element appearing consecutively.
	 *
	...
	/**
	 * For each occurrence of an element {@code e} in
	 * {@code occurrencesToRemove}, removes one occurrence of {@code e} in
	 * {@code multisetToModify}.
	 *
	 * <p>
	 ...
	 * @return {@code true} if {@code multisetToModify} was changed as a result
	 *         of this operation
	 * @since 10.0 (missing in 18.0 when only the overload taking an
	...
	static boolean equalsImpl(Multiset<?> multiset, @Nullable Object object) {
		...
		if (object instanceof Multiset) {
			...
			/*
			 * We can't simply check whether the entry sets are equal, since
			 * that approach fails when a TreeMultiset has a comparator that
			 * returns 0 when passed unequal elements.
			 */
	...
	private static final class DecreasingCount implements Comparator<Entry<?>> {
		...
		public int compare(Entry<?> entry1, Entry<?> entry2) {
			return entry2.getCount() - entry1.getCount(); // subtracting two
															// nonnegative
															// integers
		}
	}
