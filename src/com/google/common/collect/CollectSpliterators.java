...
final class CollectSpliterators {
	...
	/**
	 * Returns a {@code Spliterator} over the elements of
	 * {@code fromSpliterator} mapped by {@code
	 * function}.
	 */
	...
	/**
	 * Returns a {@code Spliterator} that iterates over the elements of the
	 * spliterators generated by applying {@code function} to the elements of
	 * {@code fromSpliterator}.
	 */
	static <F, T> Spliterator<T> flatMap(Spliterator<F> fromSpliterator, Function<? super F, Spliterator<T>> function,
			int topCharacteristics, long topSize) {
		checkArgument((topCharacteristics & Spliterator.SUBSIZED) == 0,
		...
		class FlatMapSpliterator implements Spliterator<T> {
			...
			public Spliterator<T> trySplit() {
				...
				if (fromSplit != null) {
					...
					Spliterator<T> result = new FlatMapSpliterator(this.prefix, fromSplit, splitCharacteristics,
							estSplitSize);
					this.prefix = null;
					return result;
				} else if (prefix != null) {
