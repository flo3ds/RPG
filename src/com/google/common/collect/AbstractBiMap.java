...
abstract class AbstractBiMap<K, V> extends ForwardingMap<K, V> implements BiMap<K, V>, Serializable {
	...
	public void replaceAll(BiFunction<? super K, ? super V, ? extends V> function) {
		...
		while (itr.hasNext()) {
			...
			if (conflict != null) {
				broken = entry;
				// We're definitely going to throw, but we'll try to keep the
				// BiMap in an internally
	...
	private class EntrySet extends ForwardingSet<Entry<K, V>> {
		...
		public boolean remove(Object object) {
			...
			/*
			 * Remove the mapping in inverse before removing from esDelegate
			 * because if entry is part of esDelegate, entry might be
			 * invalidated after the mapping is removed from esDelegate.
			 */
