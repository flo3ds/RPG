...
public final class Tables {
	 ...
	 * {@code Table} created using the specified supplier, whose cells are
	 * generated by applying the provided mapping functions to the input
	 * elements. Cells are inserted into the generated {@code Table} in
	 * encounter order.
	 *
	 * <p>
	 * If multiple input elements map to the same row and column, an
	 * {@code IllegalStateException} is thrown when the collection operation is
	 * performed.
	 *
	 * @since 21.0
	 */
	 ...
	 *
	 * <p>
	 * If multiple input elements map to the same row and column, the specified
	 * merging function is used to combine the values. Like
	 ...
	 * NullPointerException} on null values returned from {@code valueFunction},
	 * and treats nulls returned from {@code mergeFunction} as removals of that
	 * row/column pair.
	 *
	 * @since 21.0
	 */
	 ...
	 *
	 * @param rowKey
	 *            the row key to be associated with the returned cell
	 * @param columnKey
	 ...
	 * get(rowKey, columnKey)} on the original table. Updating the original
	 * table changes the contents of the transposed table and vice versa.
	 *
	 * <p>
	 ...
	 * {@link HashBasedTable}, it's the other way around.
	 */
	 ...
	 * {@link Table#columnKeySet}, and {@link Table#columnMap} have iterators
	 * that don't support {@code remove()}. Otherwise, all optional operations
	 * are supported. Null row keys, columns keys, and values are not supported.
	 *
	 * <p>
	 ...
	 * {@code column(columnKey).get(rowKey)} still runs quickly, since the row
	 * key is provided. However, {@code column(columnKey).size()} takes longer,
	 ...
	 *
	 * <p>
	 * Note: the table assumes complete ownership over of {@code backingMap} and
	 * the maps returned by {@code factory}. Those objects should not be
	 ...
	 * @param factory
	 *            supplier of new, empty maps that will each hold all column key
	 *            / value mappings for a given row key
	 * @throws IllegalArgumentException
	 *             if {@code backingMap} is not empty
	 * @since 10.0
	 */
	@Beta
	public static <R, C, V> Table<R, C, V> newCustomTable(Map<R, Map<C, V>> backingMap,
		...
		// TODO(jlevy): Wrap factory to validate that the supplied maps are
		// empty?
	 ...
	 * input. The transformed table might contain null values, if the function
	 ...
	 * for the returned table to be a view, but it means that the function will
	 * be applied many times for bulk operations like
	 * {@link Table#containsValue} and {@code Table.toString()}. For this to
	 * perform well, {@code function} should be fast. To avoid lazy evaluation
	 * when the returned table doesn't need to be a view, copy the returned
	 * table into a new table of your choosing.
	 *
	 * @since 10.0
	 */
	@Beta
	...
	private static class TransformedTable<R, C, V1, V2> extends AbstractTable<R, C, V2> {
		...
		public V2 get(Object rowKey, Object columnKey) {
			// The function is passed a null input only when the table contains
			// a null
			// value.
	 ...
	 * modules to provide users with "read-only" access to internal tables.
	 * Query operations on the returned table "read through" to the specified
	 * table, and attempts to modify the returned table, whether direct or via
	...
	/**
	 * Returns an unmodifiable view of the specified row-sorted table. This
	 * method allows modules to provide users with "read-only" access to
	 * internal tables. Query operations on the returned table "read through" to
	 * the specified table, and attempts to modify the returned table, whether
	 ...
	 *
	 * @param table
	 *            the row-sorted table for which an unmodifiable view is to be
	 *            returned
	...
	public static <R, C, V> RowSortedTable<R, C, V> unmodifiableRowSortedTable(
		 ...
		 * Specifically, table.rowMap().comparator() could return a comparator
		 * that only works for the ? extends R.
	 ...
	 * access to the backing table is accomplished through the returned table.
	 *
	 * <p>
	 * It is imperative that the user manually synchronize on the returned table
	 ...
	 *
	 * @param table
	 *            the table to be wrapped in a synchronized view
