 ...
 *
 * <p>
 * <b>For Java 8 users:</b> Because this is an abstract class, not an interface,
 * you can't use a lambda expression to extend it:
 *
 * <pre>
 * {
 * 	&#64;code
 ...
 * </pre>
 *
 * Instead, you can pass a lambda expression to the {@code using} factory
 * method:
 *
 * <pre>
 * {
...
public abstract class TreeTraverser<T> {
	...
	/**
	 * Returns a tree traverser that uses the given function to navigate from a
	 * node to its children. This is useful if the function instance already
	 * exists, or so that you can supply a lambda expressions. If those
	 * circumstances don't apply, you probably don't need to use this; subclass
	 * {@code TreeTraverser} and implement its {@link #children} method
	 * directly.
	 *
	 * @since 20.0
	 */
	...
	/**
	 * Returns an unmodifiable iterable over the nodes in a tree structure,
	 * using pre-order traversal. That is, each node's subtrees are traversed
	 * after the node itself is returned.
	 *
	 * <p>
	 * No guarantees are made about the behavior of the traversal when nodes
	 * change while iteration is in progress or when the iterators generated by
	...
	/**
	 * Returns an unmodifiable iterable over the nodes in a tree structure,
	 * using post-order traversal. That is, each node's subtrees are traversed
	 * before the node itself is returned.
	 *
	 * <p>
	 * No guarantees are made about the behavior of the traversal when nodes
	 * change while iteration is in progress or when the iterators generated by
	...
	/**
	 * Returns an unmodifiable iterable over the nodes in a tree structure,
	 * using breadth-first traversal. That is, all the nodes of depth 0 are
	 * returned, then depth 1, then 2, and so on.
	 *
	 * <p>
	 * No guarantees are made about the behavior of the traversal when nodes
	 * change while iteration is in progress or when the iterators generated by
