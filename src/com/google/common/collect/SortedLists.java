 ...
 *
 * In this documentation, the terms <i>greatest</i>, <i>greater</i>,
 * <i>least</i>, and <i>lesser</i> are considered to refer to the comparator on
 * the elements, and the terms <i>first</i> and <i>last</i> are considered to
 * refer to the elements' ordering in a list.
 *
...
final class SortedLists {
	...
	/**
	 * A specification for which index to return if the list contains at least
	 * one element that compares as equal to the key.
	 */
	public enum KeyPresentBehavior {
		/**
		 * Return the index of any list element that compares as equal to the
		 * key. No guarantees are made as to which index is returned, if more
		 * than one element compares as equal to the key.
		 */
		ANY_PRESENT {
			@Override
		...
		/**
		 * Return the index of the last list element that compares as equal to
		 * the key.
		 */
		LAST_PRESENT {
			@Override
			<E> int resultIndex(Comparator<? super E> comparator, E key, List<? extends E> list, int foundIndex) {
				...
				// Everything between lower and upper inclusive compares at >=
				// 0.
				while (lower < upper) {
		...
		/**
		 * Return the index of the first list element that compares as equal to
		 * the key.
		 */
		FIRST_PRESENT {
			@Override
		...
		/**
		 * Return the index of the first list element that compares as greater
		 * than the key, or {@code
		...
		/**
		 * Return the index of the last list element that compares as less than
		 * the key, or {@code -1} if there is no such element.
		 */
		LAST_BEFORE {
			@Override
			public <E> int resultIndex(Comparator<? super E> comparator, E key, List<? extends E> list,
	...
	/**
	 * A specification for which index to return if the list contains no
	 * elements that compare as equal to the key.
	 */
	public enum KeyAbsentBehavior {
		/**
		 * Return the index of the next lower element in the list, or {@code -1}
		...
		/**
		 * Return {@code ~insertionIndex}, where {@code insertionIndex} is
		 * defined as the point at which the key would be inserted into the
		 * list: the index of the next higher element in the list, or
		 * {@code list.size()} if there is no such element.
		 *
		 * <p>
		 * Note that the return value will be {@code >= 0} if and only if there
		 * is an element of the list that compares as equal to the key.
		 *
		 * <p>
		 * This is equivalent to the behavior of
		 * {@link java.util.Collections#binarySearch(List, Object)} when the key
		 * isn't present, since {@code ~insertionIndex} is equal to
	...
	/**
	 * Searches the specified naturally ordered list for the specified object
	 ...
	 * {@link Collections#sort(List, Comparator) Collections.sort(List,
	 * Comparator)} method), prior to making this call. If it is not sorted, the
	 * results are undefined.
	 *
	 * <p>
	 * If there are elements in the list which compare as equal to the key, the
	 * choice of {@link KeyPresentBehavior} decides which index is returned. If
	 * no elements compare as equal to the key, the choice of
	 * {@link KeyAbsentBehavior} decides which index is returned.
	 *
	 * <p>
	 ...
	 * @param presentBehavior
	 *            the specification for what to do if at least one element of
	 *            the list compares as equal to the key.
	 * @param absentBehavior
	 *            the specification for what to do if no elements of the list
	 *            compare as equal to the key.
	 * @return the index determined by the {@code KeyPresentBehavior}, if the
	 *         key is in the list; otherwise the index determined by the
	 *         {@code KeyAbsentBehavior}.
	 */
