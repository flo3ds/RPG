...
/**
 * An assortment of mainly legacy static utility methods that operate on or
 * return objects of type {@code Iterable}. Except as noted, each method has a
 * corresponding {@link Iterator}-based method in the {@link Iterators} class.
 *
 * <p>
 * <b>Java 8 users:</b> several common uses for this class are now more
 * comprehensively addressed by the new {@link java.util.stream.Stream} library.
 * Read the method documentation below for comparisons. This class is not being
...
public final class Iterables {
	...
	public static <T> Iterable<T> unmodifiableIterable(final Iterable<? extends T> iterable) {
		...
		if (iterable instanceof UnmodifiableIterable || iterable instanceof ImmutableCollection) {
			@SuppressWarnings("unchecked") // Since it's unmodifiable, the
											// covariant cast is safe
	...
	private static final class UnmodifiableIterable<T> extends FluentIterable<T> {
		...
		@SuppressWarnings("unchecked") // safe upcast, assuming no one has a
										// crazy Spliterator subclass
		@Override
	...
	/**
	 * Returns {@code true} if {@code iterable} contains any element {@code o}
	 * for which {@Objects.equals(o, element)} would return {@code true}.
	 * Otherwise returns {@code false}, even in cases where
	 * {@link Collection#contains} might throw {@link NullPointerException} or
	 ...
	 *
	 * @param removeFrom
	 *            the iterable to (potentially) remove elements from
	 ...
	 *
	 * <p>
	 * Removals may or may not happen immediately as each element is tested
	 * against the predicate. The behavior of this method is not specified if
	 * {@code predicate} is dependent on {@code removeFrom}.
	 *
	 * <p>
	 * <b>Java 8 users:</b> if {@code removeFrom} is a {@link Collection}, use
	 * {@code removeFrom.removeIf(predicate)} instead.
	 *
	 * @param removeFrom
	 *            the iterable to (potentially) remove elements from
	 * @param predicate
	 *            a predicate that determines whether an element should be
	 *            removed
	 * @return {@code true} if any elements were removed from the iterable
	 *
	 * @throws UnsupportedOperationException
	 *             if the iterable does not support {@code remove()}.
	 * @since 2.0
	 */
	@CanIgnoreReturnValue
	...
	/**
	 * Removes and returns the first matching element, or returns {@code null}
	 * if there is none.
	 */
	@Nullable
	 ...
	 * collection.toString()} also gives the same result, but that behavior is
	 ...
	 *
	 * <p>
	 * <b>Java 8 users:</b> the {@code Stream} equivalent to this method is
	 * {@code
	...
	/**
	 * Returns the single element contained in {@code iterable}, or
	 * {@code defaultValue} if the iterable is empty.
	 *
	 * <p>
	 * <b>Java 8 users:</b> the {@code Stream} equivalent to this method is
	 * {@code
	 ...
	 * @return a newly-allocated array into which all the elements of the
	 ...
	 *
	 * <p>
	 * <b>Java 8 users:</b> In most cases, the {@code Stream} equivalent of this
	 * method is {@code
	 * stream.filter(element::equals).count()}. If {@code element} might be
	 * null, use {@code
	...
	/**
	 * Returns an iterable whose iterators cycle indefinitely over the elements
	 * of {@code iterable}.
	 *
	 * <p>
	 * That iterator supports {@code remove()} if {@code iterable.iterator()}
	 * does. After {@code
	 * remove()} is called, subsequent cycles omit the removed element, which is
	 * no longer in {@code
	 * iterable}. The iterator's {@code hasNext()} method returns {@code true}
	 * until {@code iterable} is empty.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * After {@code remove} is invoked on a generated iterator, the removed
	 * element will no longer appear in either that iterator or any other
	 * iterator created from the same source iterable. That is, this method
	 * behaves exactly as {@code Iterables.cycle(Lists.newArrayList(elements))}.
	 * The iterator's {@code hasNext} method returns {@code true} until all of
	 * the original elements have been removed.
	 *
	 * <p>
	 * <b>Warning:</b> Typical uses of the resulting iterator may produce an
	 * infinite loop. You should use an explicit {@code break} or be certain
	 ...
	 *
	 * <p>
	 * <b>Java 8 users:</b> If passing a single element {@code e}, the
	 * {@code Stream} equivalent of this method is
	 * {@code Stream.generate(() -> e)}. Otherwise, put the elements in a
	 * collection and use
	...
	/**
	 * Combines two iterables into a single iterable. The returned iterable has
	 * an iterator that traverses the elements in {@code a}, followed by the
	 * elements in {@code b}. The source iterators are not polled until
	 * necessary.
	 *
	 * <p>
	...
	/**
	 * Combines three iterables into a single iterable. The returned iterable
	 * has an iterator that traverses the elements in {@code a}, followed by the
	 ...
	 * the final iterable with null values if necessary. For example,
	 * partitioning an iterable containing {@code [a, b, c, d, e]} with a
	 * partition size of 3 yields {@code [[a, b, c], [d, e, null]]} -- an outer
	 * iterable containing two inner lists of three elements each, all in the
	 * original order.
	 *
	 * <p>
	 * Iterators returned by the returned iterable do not support the
	 ...
	 * the input predicate {@code retainIfTrue}. The returned iterable's
	 ...
	 * the type {@code
	 * desiredType}. The returned iterable's iterator does not support
	 * {@code remove()}.
	 *
	 * <p>
	 ...
	 * If it is possible that <i>no</i> element will match, use {@link #tryFind}
	 ...
	 *
	 * <p>
	 * The returned iterable's iterator supports {@code remove()} if {@code
	 ...
	 * <p>
	 * <b>{@code Stream} equivalent:</b>
	 * {@code stream.skip(position).findFirst().get()} (throws
	 ...
	 * <p>
	 * <b>{@code Stream} equivalent:</b>
	 * {@code stream.skip(position).findFirst().orElse(defaultValue)} (returns
	 * the default value if the index is out of bounds)
	 *
	 * @param position
	 *            position of the element to return
	 * @param defaultValue
	 *            the default value to return if {@code position} is greater
	 *            than or equal to the size of the iterable
	 ...
	 *
	 * <p>
	 * To get the only element in a single-element {@code Iterable}, consider
	 * using {@link #getOnlyElement(Iterable)} or
	 * {@link #getOnlyElement(Iterable, Object)} instead.
	 *
	 * <p>
	 * <b>{@code Stream} equivalent:</b>
	 * {@code stream.findFirst().orElse(defaultValue)}
	 *
	 * @param defaultValue
	...
	/**
	 * Returns the last element of {@code iterable}. If {@code iterable} is a
	 * {@link List} with {@link RandomAccess} support, then this operation is
	 ...
	 * {@link RandomAccess} support, then this operation is guaranteed to be
	 * {@code O(1)}.
	 *
	 * <p>
	 * <b>{@code Stream} equivalent:</b>
	 ...
	 *
	 * @param defaultValue
	 *            the value to return if {@code iterable} is empty
	 ...
	 * iterator skips the first {@code numberToSkip} elements that exist when
	 * the {@code Iterator} is created, not when {@code skip()} is called.
	 *
	 * <p>
	...
	public static <T> Iterable<T> skip(final Iterable<T> iterable, final int numberToSkip) {
		...
		return new FluentIterable<T>() {
			...
			public Iterator<T> iterator() {
				...
				/*
				 * We can't just return the iterator because an immediate call
				 * to its remove() method would remove one of the skipped
				 * elements instead of throwing an IllegalStateException.
				 */
	 ...
	 *
	 * <p>
	 * Note: If {@code iterable} is a {@link Queue}, the returned iterable will
	 ...
	 * @return a view of the supplied iterable that wraps each generated
	 *         iterator through {@link Iterators#consumingIterator(Iterator)};
	 *         for queues, an iterable that generates iterators that return and
	 *         consume the queue's elements in queue order
	 *
	 ...
	 *
	 * <p>
	 * Callers must ensure that the source {@code iterables} are in
