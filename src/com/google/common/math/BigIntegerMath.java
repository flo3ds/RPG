...
public final class BigIntegerMath {
	 ...
	 * equivalent to {@code BigInteger.valueOf(2).pow(log2(x, FLOOR))}.
	 *
	 * @throws IllegalArgumentException
	 *             if {@code x <= 0}
	 * @since 20.0
	 ...
	 * explicit half power of two. This can be any value, but higher values
	 * incur more class load time and linearly increasing memory consumption.
	 */
	@VisibleForTesting
	...
	public static int log10(BigInteger x, RoundingMode mode) {
		...
		/*
		 * We adjust approxLog10 and approxPow until they're equal to
		 * floor(log10(x)) and 10^floor(log10(x)).
		 */
		...
		if (approxCmp > 0) {
			 ...
			 * but in practice this branch should almost never be entered, and
		...
		case HALF_UP:
		case HALF_EVEN:
			// Since sqrt(10) is irrational, log10(x) - floorLog can never be
			// exactly 0.5
			BigInteger x2 = x.pow(2);
	...
	public static BigInteger sqrt(BigInteger x, RoundingMode mode) {
		...
		switch (mode) {
		case UNNECESSARY:
			checkRoundingUnnecessary(sqrtFloor.pow(2).equals(x)); // fall
																	// through
		case FLOOR:
		case DOWN:
			return sqrtFloor;
		...
		case HALF_EVEN:
			 ...
			 * is equivalent to testing whether or not x <= halfSquare.
			 */
	...
	private static BigInteger sqrtFloor(BigInteger x) {
		 ...
		 * extremely fast, and then we can get a double approximation of the
		 * square root. Then, we iteratively improve this guess with an
		 * application of Newton's method, which sets guess := (guess + (x /
		 * guess)) / 2. This iteration has the following two properties:
		 *
		 * a) every iteration (except potentially the first) has guess >=
		 * floor(sqrt(x)). This is because guess' is the arithmetic mean of
		 * guess and x / guess, sqrt(x) is the geometric mean, and the
		 * arithmetic mean is always higher than the geometric mean.
		 *
		 * b) this iteration converges to floor(sqrt(x)). In fact, the number of
		 * correct digits doubles with each iteration, so this algorithm takes
		 * O(log(digits)) iterations.
		 *
		 * We start out with a double-precision approximation, which may be
		 * higher or lower than the true value. Therefore, we perform at least
		 * one Newton iteration to get a guess that's definitely >=
		 * floor(sqrt(x)), and then continue the iteration until we reach a
		 * fixed point.
		 */
		BigInteger sqrt0;
		...
		if (log2 < Double.MAX_EXPONENT) {
			 ...
			 * sqrtFloor(x) will be 2^(shift/2) * sqrtApproxWithDoubles(x /
			 * 2^shift).
			 */
	 ...
	 *
	 * <p>
	 * <b>Warning:</b> the result takes <i>O(n log n)</i> space, so use
	 * cautiously.
	 *
	 * <p>
	 * This uses an efficient binary recursive algorithm to compute the
	 * factorial with balanced multiplies. It also removes all the 2s from the
	 * intermediate products (shifting them back in at the end).
	 *
	 * @throws IllegalArgumentException
	 *             if {@code n < 0}
	 */
	public static BigInteger factorial(int n) {
		...
		for (long num = startingNumber; num <= n; num++) {
			...
			// If it won't fit in a long, then we store off the intermediate
			// product.
	 ...
	 * coefficient of {@code n} and {@code k}, that is,
	 * {@code n! / (k! (n - k)!)}.
	 *
	 * <p>
	...
	public static BigInteger binomial(int n, int k) {
		...
		for (int i = 1; i < k; i++) {
			...
			if (numeratorBits + bits >= Long.SIZE - 1) {
				// The numerator is as big as it can get without risking
				// overflow.
