...
public final class LongMath {
	 ...
	 * This is equivalent to {@code checkedPow(2, log2(x, CEILING))}.
	 *
	 ...
	 * equivalent to {@code checkedPow(2, log2(x, FLOOR))}.
	 *
	 * @throws IllegalArgumentException
	 *             if {@code x <= 0}
	 * @since 20.0
	...
	public static long floorPowerOfTwo(long x) {
		...
		// Long.highestOneBit was buggy on GWT. We've fixed it, but I'm not
		// certain when the fix will
		// be released.
	 ...
	 * {@code Long.bitCount(Long.MIN_VALUE) == 1}, but {@link Long#MIN_VALUE} is
	 * not a power of two.
	 */
	...
	/**
	 * Returns 1 if {@code x < y} as unsigned longs, and 0 otherwise. Assumes
	 * that x - y fits into a signed long. The implementation is branch-free,
	 * and benchmarks suggest it is measurably faster than the straightforward
	 * ternary expression.
	 */
	@VisibleForTesting
	 ...
	 * it will be equal to {@code BigInteger.valueOf(b).pow(k).longValue()}.
	 * This implementation runs in {@code O(log k)} time.
	 *
	 * @throws IllegalArgumentException
	 *             if {@code k < 0}
	 */
	...
	public static long sqrt(long x, RoundingMode mode) {
		 ...
		 *
		 * k * k <= x < (k + 1) * (k + 1) (double) (k * k) <= (double) x <=
		 * (double) ((k + 1) * (k + 1)) since casting to double is
		 * nondecreasing. Note that the right-hand inequality is no longer
		 * strict. Math.sqrt(k * k) <= Math.sqrt(x) <= Math.sqrt((k + 1) * (k +
		 * 1)) since Math.sqrt is monotonic. (long) Math.sqrt(k * k) <= (long)
		 * Math.sqrt(x) <= (long) Math.sqrt((k + 1) * (k + 1)) since casting to
		 * long is monotonic k <= (long) Math.sqrt(x) <= k + 1 since (long)
		...
		// Note (2013-2-26): benchmarks indicate that, inscrutably enough, using
		// if statements is
		...
		case HALF_EVEN:
			 ...
			 * Therefore, it's safe to treat x - halfSquare as a signed long, so
	...
	public static long gcd(long a, long b) {
		 ...
		 * what do you return on gcd(0, Long.MIN_VALUE)? BigInteger.gcd would
		 * return positive 2^63, but positive 2^63 isn't an int.
		 */
		 ...
		 * http://en.wikipedia.org/wiki/Binary_GCD_algorithm. This is >60%
		...
		while (a != b) { // both a, b are odd
			...
			a >>= Long.numberOfTrailingZeros(a); // divide out all 2s, since 2
													// doesn't divide b
		}
	 ...
	 * underflow in which case {@code Long.MAX_VALUE} or {@code Long.MIN_VALUE}
	...
	public static long saturatedAdd(long a, long b) {
		...
		if ((a ^ b) < 0 | (a ^ naiveSum) >= 0) {
			// If a and b have different signs or a has the same sign as the
			// result then there was no
			// overflow, return.
			return naiveSum;
		}
	...
	/**
	 * Returns the difference of {@code a} and {@code b} unless it would
	 * overflow or underflow in which case {@code Long.MAX_VALUE} or
	 * {@code Long.MIN_VALUE} is returned, respectively.
	 *
	 * @since 20.0
	 */
	@Beta
	public static long saturatedSubtract(long a, long b) {
		...
		if ((a ^ b) >= 0 | (a ^ naiveDifference) >= 0) {
			// If a and b have the same signs or a has the same sign as the
			// result then there was no
			// overflow, return.
	...
	/**
	 * Returns the product of {@code a} and {@code b} unless it would overflow
	 * or underflow in which case {@code Long.MAX_VALUE} or
	 * {@code Long.MIN_VALUE} is returned, respectively.
	 *
	 * @since 20.0
	 */
	...
	public static long saturatedMultiply(long a, long b) {
		...
		// the return value if we will overflow (which we calculate by
		// overflowing a long :) )
	...
	/**
	 * Returns the {@code b} to the {@code k}th power, unless it would overflow
	 * or underflow in which case {@code Long.MAX_VALUE} or
	 * {@code Long.MIN_VALUE} is returned, respectively.
	 *
	 * @since 20.0
	...
	public static long saturatedPow(long b, int k) {
		...
		long accum = 1;
		// if b is negative and k is odd then the limit is MIN otherwise the
		// limit is MAX
	 ...
	 * coefficient of {@code n} and {@code k}, or {@link Long#MAX_VALUE} if the
	 * result does not fit in a {@code long}.
	 *
	 * @throws IllegalArgumentException
	...
	public static long binomial(int n, int k) {
		...
		default:
			if (n < factorials.length) {
				...
				/*
				 * We want to do this in long math for speed, but want to avoid
				 * overflow. We adapt the technique previously used by
				 * BigIntegerMath: maintain separate numerator and denominator
				 * accumulators, multiplying the fraction into result when near
				 * overflow.
				 */
				for (int i = 2; i <= k; i++, n--) {
	...
	static long multiplyFraction(long x, long numerator, long denominator) {
		...
		// We know gcd(x, denominator) = 1, and x * numerator / denominator is
		// exact,
	...
	static final int[] biggestSimpleBinomials = { Integer.MAX_VALUE, Integer.MAX_VALUE, Integer.MAX_VALUE, 2642246,
			...
			62, 62, 61, 61, 61 };
	// These values were generated by using checkedMultiply to see when the
	...
	/*
	 * This bitmask is used as an optimization for cheaply testing for
	 * divisiblity by 2, 3, or 5. Each bit is set to 1 for all remainders that
	 * indicate divisibility by 2, 3, or 5, so 1, 7, 11, 13, 17, 19, 23, 29 are
	 * set to 0. 30 and up don't matter because they won't be hit.
	 */
	 ...
	 * <a href="http://mathworld.wolfram.com/PrimeNumber.html">prime number</a>:
	 * an integer <i>greater than one</i> that cannot be factored into a product
	 * of <i>smaller</i> positive integers. Returns {@code false} if {@code n}
	 * is zero, one, or a composite number (one which <i>can</i> be factored
	 ...
	 *
	 * NOTE: We could get slightly better bases that would be treated as
	 * unsigned, but benchmarks showed negligible performance improvements.
	 */
	...
	private enum MillerRabinTester {
		...
		SMALL {
			...
			long mulMod(long a, long b, long m) {
				/*
				 * NOTE(lowasser, 2015-Feb-12): Benchmarks suggest that changing
				 * this to UnsignedLongs.remainder and increasing the threshold
				 * to 2^32 doesn't pay for itself, and adding another enum
				 * constant hurts performance further -- I suspect because
		...
		LARGE {
			/**
			 * Returns (a + b) mod m. Precondition: {@code 0 <= a},
			 * {@code b < m < 2^63}.
			 */
			...
			private long times2ToThe32Mod(long a, long m) {
				...
				do {
					...
					// shift is either the number of powers of 2 left to
					// multiply a by, or the biggest shift
			...
			long mulMod(long a, long b, long m) {
				...
				/*
				 * a * b == aHi * bHi * 2^64 + (aHi * bLo + aLo * bHi) * 2^32 +
				 * aLo * bLo. == (aHi * bHi * 2^32 + aHi * bLo + aLo * bHi) *
				 * 2^32 + aLo * bLo
				 *
				 * We carry out this computation in modular arithmetic. Since
				 * times2ToThe32Mod accepts any unsigned long, we don't have to
				 * do a mod on every operation, only when intermediate results
				 * can exceed 2^63.
				 */
				long result = times2ToThe32Mod(aHi * bHi /* < 2^62 */, m); // <
																			// m
																			// <
																			// 2^63
			...
			long squareMod(long a, long m) {
				...
				/*
				 * a^2 == aHi^2 * 2^64 + aHi * aLo * 2^33 + aLo^2 == (aHi^2 *
				 * 2^32 + aHi * aLo * 2) * 2^32 + aLo^2 We carry out this
				 * computation in modular arithmetic. Since times2ToThe32Mod
				 * accepts any unsigned long, we don't have to do a mod on every
				 * operation, only when intermediate results can exceed 2^63.
				 */
		...
		static boolean test(long base, long n) {
			// Since base will be considered % n, it's okay if base >
			// FLOOR_SQRT_MAX_LONG,
		...
		/**
		 * Returns true if n is a strong probable prime relative to the
		 * specified base.
		 */
