...
public abstract class ByteSource {
	...
	/**
	 * Returns a {@link CharSource} view of this byte source that decodes bytes
	 * read from this source as characters using the given {@link Charset}.
	 *
	 * <p>
	 * If {@link CharSource#asByteSource} is called on the returned source with
	 * the same charset, the default implementation of this method will ensure
	 * that the original {@code ByteSource} is returned, rather than round-trip
	 ...
	 * The returned stream is not required to be a {@link BufferedInputStream}
	 * in order to allow implementations to simply delegate to
	 * {@link #openStream()} when the stream returned by that method does not
	 * benefit from additional buffering (for example, a
	 * {@code ByteArrayInputStream}). This method returns a new, independent
	 ...
	 * {@code length} bytes long starting at the given {@code offset}. If
	 * {@code offset} is greater than the size of this source, the returned
	 * source will be empty. If {@code offset + length} is greater than the size
	 * of this source, the returned source will contain the slice starting at
	 * {@code offset} and ending at the end of this source.
	 *
	...
	/**
	 * Returns whether the source has zero bytes. The default implementation
	 * first checks {@link #sizeIfKnown}, returning true if it's known to be
	 * zero and false if it's known to be non-zero. If the size is not known, it
	 * falls back to opening a stream and checking for EOF.
	 *
	 * <p>
	 * Note that, in cases where {@code sizeIfKnown} returns zero, it is
	 * <i>possible</i> that bytes are actually available for reading. (For
	 * example, some special files may return a size of 0 despite actually
	 * having content when read.) This means that a source may return
	 ...
	 * such as a file, may return a non-absent value. Note that in such cases,
	 * it is <i>possible</i> that this method will return a different number of
	 * bytes than would be returned by reading all of the bytes (for example,
	 * some special files may return a size of 0 despite actually having content
	 * when read).
	 *
	 * <p>
	 * Additionally, for mutable sources such as files, a subsequent read may
	 * return a different number of bytes if the contents are changed.
	 *
	 * @since 19.0
	 */
	@Beta
	...
	/**
	 * Returns the size of this source in bytes, even if doing so requires
	 * opening and traversing an entire stream. To avoid a potentially expensive
	 * operation, see {@link #sizeIfKnown}.
	 *
	 * <p>
	 * The default implementation calls {@link #sizeIfKnown} and returns the
	 * value if present. If absent, it will fall back to a heavyweight operation
	 * that will open a stream, read (or {@link InputStream#skip(long) skip}, if
	 * possible) to the end of the stream and return the total number of bytes
	 * that were read.
	 *
	 * <p>
	 * Note that for some sources that implement {@link #sizeIfKnown} to provide
	 * a more efficient implementation, it is <i>possible</i> that this method
	 * will return a different number of bytes than would be returned by reading
	 * all of the bytes (for example, some special files may return a size of 0
	 * despite actually having content when read).
	 *
	 * <p>
	 ...
	 * Returns SKIP_FAILED if the first call to skip threw, in which case skip
	 * may just not be supported.
	 */
	 ...
	 * {@code OutputStream}. Does not close {@code output}.
	 *
	...
	/**
	 * Reads the contents of this byte source using the given {@code processor}
	 * to process bytes as they are read. Stops when all bytes have been read or
	 * the consumer returns {@code false}. Returns the result produced by the
	 * processor.
	 *
	 * @throws IOException
	...
	/**
	 * Checks that the contents of this byte source are equal to the contents of
	 * the given byte source.
	 *
	 * @throws IOException
	 *             if an I/O error occurs while reading from this source or
	 *             {@code other}
	...
	/**
	 * Returns a view of the given byte array as a {@link ByteSource}. To view
	 * only a specific range in the array, use
	...
	private static class ByteArrayByteSource extends ByteSource {
		...
		// NOTE: Preconditions are enforced by slice, the only non-trivial
		// caller.
		...
		@SuppressWarnings("CheckReturnValue") // it doesn't matter what
												// processBytes returns here
		@Override
		public <T> T read(ByteProcessor<T> processor) throws IOException {
