...
public final class ByteStreams {
	 ...
	 *
	 * <ol>
	 * <li>Use sendfile(2) or equivalent. Requires that both the input channel
	 * and the output channel have their own file descriptors. Generally this
	 * only happens when both channels are files or sockets. This performs zero
	 * copies - the bytes never enter userspace.
	 * <li>Use mmap(2) or equivalent. Requires that either the input channel or
	 * the output channel have file descriptors. Bytes are copied from the file
	 * into a kernel buffer, then directly into the other buffer (userspace).
	 * Note that if the file is very large, a naive implementation will
	 * effectively put the whole file in memory. On many systems with paging and
	 * virtual memory, this is not a problem - because it is mapped read-only,
	 * the kernel can always page it to disk "for free". However, on systems
	 * where killing processes happens all the time in normal conditions (i.e.,
	 * android) the OS must make a tradeoff between paging memory and killing
	 * other processes - so allocating a gigantic buffer and then sequentially
	 * accessing it could result in other processes dying. This is solvable via
	 * madvise(2), but that obviously doesn't exist in java.
	 * <li>Ordinary copy. Kernel copies bytes into a kernel buffer, from a
	 * kernel buffer into a userspace buffer (byte[] or ByteBuffer), then copies
	 * them from that buffer into the destination channel.
	 * </ol>
	 *
	 * This value is intended to be large enough to make the overhead of system
	 * calls negligible, without being so large that it causes problems for
	 * systems with atypical memory management if approaches 2 or 3 are used.
	 */
	...
	public static byte[] toByteArray(InputStream in) throws IOException {
		// Presize the ByteArrayOutputStream since we know how large it will
		// need
		// to be, unless that value is less than the default
	...
	/**
	 * Reads and discards data from the given {@code InputStream} until the end
	 * of the stream is reached. Returns the total number of bytes read. Does
	...
	public static ByteArrayDataOutput newDataOutput(int size) {
		// When called at high frequency, boxing size generates too much
		// garbage,
	 ...
	 * returned in the byte array may not be exactly what was written via calls
	 * to {@code ByteArrayDataOutput}).
	 *
	 * @since 17.0
	 */
