 ...
 * 		// do stuff
 * 	} catch (Throwable e) {
 * 		// ensure that any checked exception types other than IOException
 * 		// that could be thrown are
 ...
 * block. The throwable from the try block will be thrown.
 * <li>If no exceptions or errors were thrown in the try block, the <i>first</i>
 * exception thrown by an attempt to close a resource will be thrown.
 ...
 * that <i>will</i> be thrown using {@code Throwable.addSuppressed(Throwable)}.
...
public final class Closer implements Closeable {
	...
	Closer(Suppressor suppressor) {
		this.suppressor = checkNotNull(suppressor); // checkNotNull to satisfy
													// null tests
	}
	...
	/**
	 * Stores the given throwable and rethrows it. It will be rethrown as is if
	 * it is an {@code IOException}, {@code RuntimeException} or {@code Error}.
	 * Otherwise, it will be rethrown wrapped in a {@code RuntimeException}.
	 * <b>Note:</b> Be sure to declare all of the checked exception types your
	 * try block can throw when calling an overload of this method so as to
	 * avoid losing the original exception type.
	 *
	 * <p>
	...
	/**
	 * Stores the given throwable and rethrows it. It will be rethrown as is if
	 * it is an {@code IOException}, {@code RuntimeException}, {@code Error} or
	 * a checked exception of the given type. Otherwise, it will be rethrown
	 * wrapped in a {@code RuntimeException}. <b>Note:</b> Be sure to declare
	 * all of the checked exception types your try block can throw when calling
	 * an overload of this method so as to avoid losing the original exception
	 * type.
	 *
	 * <p>
	 * This method always throws, and as such should be called as
	 * {@code throw closer.rethrow(e, ...);} to ensure the compiler knows that
	 * it will throw.
	 *
	 * @return this method does not return; it always throws
	 * @throws IOException
	 ...
	 * Stores the given throwable and rethrows it. It will be rethrown as is if
	 * it is an {@code IOException}, {@code RuntimeException}, {@code Error} or
	 * a checked exception of either of the given types. Otherwise, it will be
	 ...
	 *             when the given throwable is an IOException
	 * @throws X1
	 *             when the given throwable is of the declared type X1
	 * @throws X2
	 ...
	 * {@code Closer}. If an exception was thrown in the try block and passed to
	 * one of the {@code exceptionThrown} methods, any exceptions thrown when
	 * attempting to close a closeable will be suppressed. Otherwise, the
	 * <i>first</i> exception to be thrown from an attempt to close a closeable
	 * will be thrown and any additional exceptions that are thrown after that
	 * will be suppressed.
	 */
	@Override
	...
	interface Suppressor {
		/**
		 * Suppresses the given exception ({@code suppressed}) which was thrown
		 * when attempting to close the given closeable. {@code thrown} is the
		 * exception that is actually being thrown from the method.
		 * Implementations of this method should not throw under any
		 * circumstances.
		 */
	...
	static final class SuppressingSuppressor implements Suppressor {
		...
		public void suppress(Closeable closeable, Throwable thrown, Throwable suppressed) {
			...
			try {
				...
				// if, somehow, IllegalAccessException or another exception is
				// thrown, fall back to logging
