	...
	abstract class CollectionFutureRunningState extends RunningState {
		...
		final void collectOneValue(boolean allMustSucceed, int index, @Nullable V returnValue) {
			...
			if (localValues != null) {
				...
				// Some other future failed or has been cancelled, causing this
				// one to also be cancelled or
				// have an exception set. This should only happen if
				// allMustSucceed is true or if the output
