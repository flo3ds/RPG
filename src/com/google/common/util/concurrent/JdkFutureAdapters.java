 ...
 * {@link Future} instances. Note that, whenever possible, it is strongly
 * preferred to modify those libraries to return {@code
...
public final class JdkFutureAdapters {
	/**
	 * Assigns a thread to the given {@link Future} to provide
	 ...
	 * {@link ListenableFuture#addListener} by taking a thread from an internal,
	 * unbounded pool at the first call to {@code addListener} and holding it
	 * until the future is {@linkplain Future#isDone() done}.
	 *
	 * <p>
	 * Prefer to create {@code ListenableFuture} instances with
	 * {@link SettableFuture},
	 ...
	 * <b>Warning:</b> If the input future does not already implement
	 * {@code ListenableFuture}, the returned future will emulate
	 * {@link ListenableFuture#addListener} by submitting a task to the given
	 * executor at the first call to {@code addListener}. The task must be
	 * started by the executor promptly, or else the returned
	 * {@code ListenableFuture} may fail to work. The task's execution consists
	 * of blocking until the input future is {@linkplain Future#isDone() done},
	 * so each call to this method may claim and hold a thread for an arbitrary
	 * length of time. Use of bounded executors or other executors that may fail
	 * to execute a task promptly may result in deadlocks.
	 *
	 * <p>
	 * Prefer to create {@code ListenableFuture} instances with
	 * {@link SettableFuture},
	...
	private static class ListenableFutureAdapter<V> extends ForwardingFuture<V> implements ListenableFuture<V> {
		...
		// This allows us to only start up a thread waiting on the delegate
		// future when the first
		// listener is added.
			...
			if (hasListeners.compareAndSet(false, true)) {
				...
				adapterExecutor.execute(new Runnable() {
					...
					public void run() {
						try {
							/*
							 * Threads from our private pool are never
							 * interrupted. Threads from a user-supplied
							 * executor might be, but... what can we do? This is
							 * another reason to return a proper
							 * ListenableFuture instead of using
							 * listenInPoolThread.
							 */
