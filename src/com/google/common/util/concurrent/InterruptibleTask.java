...
abstract class InterruptibleTask implements Runnable {
	// These two fields are used to interrupt running tasks. The thread
	// executing the task publishes
	// itself to the 'runner' field and the thread interrupting sets
	// 'doneInterrupting' when it has
	...
	public final void run() {
		...
		try {
			...
			if (wasInterrupted()) {
				// We were interrupted, it is possible that the interrupted bit
				// hasn't been set yet. Wait
				// for the interrupting thread to set 'doneInterrupting' to
				// true. See interruptTask().
				// We want to wait so that we don't interrupt the _next_ thing
				// run on the thread.
				// Note: We don't reset the interrupted bit, just wait for it to
				// be set.
				// If this is a thread pool thread, the thread pool will reset
				// it for us. Otherwise, the
				// interrupted bit may have been intended for something else, so
				// don't clear it.
	...
	final void interruptTask() {
		// interruptTask is guaranteed to be called at most once, and if runner
		// is non-null when that
		// happens, then it must have been the first thread that entered run().
		// So there is no risk that
