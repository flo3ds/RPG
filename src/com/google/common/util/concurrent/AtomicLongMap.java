...
/**
 * A map containing {@code long} values that can be atomically updated. While
 * writes to a traditional {@code Map} rely on {@code put(K, V)}, the typical
 * mechanism for writing to this map is {@code addAndGet(K, long)}, which adds a
 * {@code long} to the value currently associated with {@code K}. If a key has
 * not yet been associated with a value, its implicit value is zero.
 *
 * <p>
 * Most methods in this class treat absent values and zero values identically,
 * as individually documented. Exceptions to this are {@link #containsKey},
 * {@link #size}, {@link #isEmpty}, {@link #asMap}, and {@link #toString}.
 *
 * <p>
 * Instances of this class may be used by multiple threads concurrently. All
 * operations are atomic unless otherwise noted.
 *
 * <p>
 * <b>Note:</b> If your values are always positive and less than 2^31, you may
 * wish to use a {@link com.google.common.collect.Multiset} such as
 * {@link com.google.common.collect.ConcurrentHashMultiset} instead.
 *
 * <b>Warning:</b> Unlike {@code Multiset}, entries whose values are zero are
 * not automatically removed from the map. Instead they must be removed manually
 * with {@link #removeAllZeros}.
 *
 * @author Charles Fry
 * @since 11.0
 */
...
public final class AtomicLongMap<K> implements Serializable {
	...
	/**
	 * Updates the value currently associated with {@code key} with the
	 * specified function, and returns the new value. If there is not currently
	 * a value associated with {@code key}, the function is applied to
	 * {@code 0L}.
	 *
	 * @since 21.0
	 */
	...
	/**
	 * Updates the value currently associated with {@code key} with the
	 * specified function, and returns the old value. If there is not currently
	 * a value associated with {@code key}, the function is applied to
	 * {@code 0L}.
	 *
	 * @since 21.0
	...
	/**
	 * Updates the value currently associated with {@code key} by combining it
	 * with {@code x} via the specified accumulator function, returning the new
	 * value. The previous value associated with {@code key} (or zero, if there
	 * is none) is passed as the first argument to {@code accumulatorFunction},
	 * and {@code x} is passed as the second argument.
	 *
	 * @since 21.0
	 */
	@CanIgnoreReturnValue
	...
	/**
	 * Updates the value currently associated with {@code key} by combining it
	 * with {@code x} via the specified accumulator function, returning the old
	 * value. The previous value associated with {@code key} (or zero, if there
	 * is none) is passed as the first argument to {@code accumulatorFunction},
	 * and {@code x} is passed as the second argument.
	 *
