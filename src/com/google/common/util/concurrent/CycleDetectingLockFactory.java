...
/**
 * The {@code CycleDetectingLockFactory} creates {@link ReentrantLock} instances
 * and {@link ReentrantReadWriteLock} instances that detect potential deadlock
 * by checking for cycles in lock acquisition order.
 *
 * <p>
 ...
 * In a simple example involving two locks and two threads, deadlock occurs when
 * one thread acquires Lock A, and then Lock B, while another thread acquires
 * Lock B, and then Lock A:
 *
 * <pre>
 ...
 *
 * <p>
 * While {@code ReadWriteLock} instances have different properties and can form
 * cycles without potential deadlock, this class treats {@code ReadWriteLock}
 * instances as equivalent to traditional exclusive locks. Although this
 * increases the false positives that the locks detect (i.e. cycles that will
 * not actually result in deadlock), it simplifies the algorithm and
 * implementation considerably. The assumption is that a user of this factory
 * wishes to eliminate any cyclic acquisition ordering.
 *
 * <p>
 ...
 *
 * <p>
 * In order to allow proper garbage collection of unused locks, the edges of the
 * lock graph are weak references.
 *
 * <p>
...
public class CycleDetectingLockFactory {
	...
	public interface Policy {
		 ...
		 * {@code tryLock()}. Although {@code tryLock()} technically recovers
		 * from deadlock by eventually timing out, this behavior is chosen based
		 * on the assumption that it is the application's wish to prohibit any
		 * cyclical lock acquisitions.
		 */
	...
	public enum Policies implements Policy {
		/**
		 * When potential deadlock is detected, this policy results in the
		 * throwing of the {@code PotentialDeadlockException} indicating the
		 * potential deadlock, which includes stack traces illustrating the
		 * cycle in lock acquisition order.
		 */
		THROW {
			@Override
		...
		/**
		 * When potential deadlock is detected, this policy results in the
		 * logging of a {@link Level#SEVERE} message indicating the potential
		 ...
		 * unmodified lock implementations provided by the JDK, and is provided
		 * to allow applications to easily parameterize when cycle detection is
		 * enabled.
		 *
		 * <p>
		 * Note that locks created by a factory with this policy will
		 * <em>not</em> participate the cycle detection performed by locks
		 * created by other factories.
		 */
		DISABLED {
			@Override
			public void handlePotentialDeadlock(PotentialDeadlockException e) {
	...
	public static <E extends Enum<E>> WithExplicitOrdering<E> newInstanceWithExplicitOrdering(Class<E> enumClass,
			Policy policy) {
		// createNodes maps each enumClass to a Map with the corresponding enum
		// key
		// type.
	 ...
	 * with nodes according to the natural ordering of the associated Enum
	 * values.
	 */
	@VisibleForTesting
	 ...
	 * {@code CycleDetectingLockFactory} explicitly ordered locks participate in
	 * general cycle detection with all other cycle detecting locks, and a
	 * lock's behavior when detecting a cyclic lock acquisition is defined by
	 * the {@code Policy} of the factory that created it.
	 *
	 * <p>
	 * Note, however, that although multiple locks can be created for a given
	 * Enum value, whether it be through separate factory instances or through
	 * multiple calls to the same factory, attempting to acquire multiple locks
	 * with the same Enum value (within the same thread) will result in an
	...
	public static final class WithExplicitOrdering<E extends Enum<E>> extends CycleDetectingLockFactory {
		...
		/**
		 * Creates a {@link ReentrantLock} with the given fairness policy and
		 ...
		 *             If the factory has already created a {@code Lock} with
		 *             the specified rank.
		 */
		...
		/**
		 * Creates a {@link ReentrantReadWriteLock} with the given fairness
		 * policy and rank. The values returned by
		 * {@link Enum#getDeclaringClass()} and {@link Enum#name()} are used to
		 * describe the lock in warning or exception output.
		 *
		 * @throws IllegalStateException
		 *             If the factory has already created a {@code Lock} with
		 *             the specified rank.
	...
	/**
	 * A Throwable used to record a stack trace that illustrates an example of a
	 * specific lock acquisition ordering. The top of the stack trace is
		...
		ExampleStackTrace(LockGraphNode node1, LockGraphNode node2) {
			...
			for (int i = 0, n = origStackTrace.length; i < n; i++) {
				if (WithExplicitOrdering.class.getName().equals(origStackTrace[i].getClassName())) {
					// For pre-populated disallowedPriorLocks edges, omit the
					// stack trace.
	 ...
	 * includes a causal chain of {@code ExampleStackTrace} instances to
	 * illustrate the cycle, e.g.
	 *
	 * <pre>
	...
	public static final class PotentialDeadlockException extends ExampleStackTrace {
		...
		/**
		 * Appends the chain of messages from the {@code conflictingStackTrace}
		 * to the original {@code message}.
		 */
		@Override
	...
	private static class LockGraphNode {
		 ...
		 * lock, each associated with an example stack trace. Locks are weakly
		 * keyed to allow proper garbage collection when they are no longer
		 * referenced.
		 */
		final Map<LockGraphNode, ExampleStackTrace> allowedPriorLocks = new MapMaker().weakKeys().makeMap();

		/**
		 * The map tracking lock nodes that can cause a lock acquisition cycle
		 * if acquired before this node.
		 */
		...
		/**
		 * Checks the acquisition-ordering between {@code this}, which is about
		 * to be acquired, and the specified {@code acquiredLock}.
		 *
		 * <p>
		 * When this method returns, the {@code acquiredLock} should be in
		 * either the {@code preAcquireLocks} map, for the case in which it is
		 * safe to acquire {@code this} after the {@code acquiredLock}, or in
		 * the {@code disallowedPriorLocks} map, in which case it is not safe.
		 */
		void checkAcquiredLock(Policy policy, LockGraphNode acquiredLock) {
			// checkAcquiredLock() should never be invoked by a lock that has
			// already
			// been acquired. For unordered locks, aboutToAcquire() ensures this
			// by
			// checking isAcquiredByCurrentThread(). For ordered locks, however,
			// this
			// can happen because multiple locks may share the same
			// LockGraphNode. In
			// this situation, throw an IllegalStateException as defined by
			// contract
			...
			if (allowedPriorLocks.containsKey(acquiredLock)) {
				// The acquisition ordering from "acquiredLock" to "this" has
				// already
				// been verified as safe. In a properly written application,
				// this is
				// the common case.
				return;
			}
			...
			if (previousDeadlockException != null) {
				...
				// Create a new PotentialDeadlockException with the same causal
				// chain
			...
			// Otherwise, it's the first time seeing this lock relationship.
			// Look for
			...
			if (path == null) {
				...
				//
				// Note that there is a race condition here which can result in
				// missing
				// a cyclic edge: it's possible for two threads to simultaneous
				// find
				// "safe" edges which together form a cycle. Preventing this
				// race
				// condition efficiently without _introducing_ deadlock is
				// probably
				// tricky. For now, just accept the race condition---missing a
				// warning
				// now and then is still better than having no deadlock
				// detection.
		 ...
		 * node's {@code allowedPriorLocks} to find a path between {@code this}
		 * and the specified {@code lock}.
		 *
		 * @return If a path was found, a chained {@link ExampleStackTrace}
		 *         illustrating the path to the {@code lock}, or {@code null} if
		 *         no path was found.
		 */
		@Nullable
		private ExampleStackTrace findPathTo(LockGraphNode node, Set<LockGraphNode> seen) {
			...
			for (Map.Entry<LockGraphNode, ExampleStackTrace> entry : allowedPriorLocks.entrySet()) {
				...
				if (found != null) {
					// One of this node's allowedPriorLocks found a path.
					// Prepend an
					// ExampleStackTrace(preAcquiredLock, this) to the returned
					// chain of
					// ExampleStackTraces.
	...
	/**
	 * CycleDetectingLock implementations must call this method before
	...
	final class CycleDetectingReentrantReadWriteLock extends ReentrantReadWriteLock implements CycleDetectingLock {
		...
		// ReentrantReadWriteLock superclass. They are simply wrappers around
		// the
		// internal Sync object, so this is safe since the shadowed locks are
		// never
		// exposed or used.
