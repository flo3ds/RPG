...
final class FuturesGetChecked {
	 ...
	 * {@link FuturesGetChecked#getChecked(Future, Class)}, possibly using
	 * caching.
	 *
	 * <p>
	 * Uses reflection to gracefully fall back to when certain implementations
	 * aren't available.
	 */
	@VisibleForTesting
	static class GetCheckedTypeValidatorHolder {
		...
		@IgnoreJRERequirement // getChecked falls back to another implementation
								// if necessary
		@J2ObjCIncompatible // ClassValue
		enum ClassValueValidator implements GetCheckedTypeValidator {
			...
			/*
			 * Static final fields are presumed to be fastest, based on our
			 * experience with UnsignedBytesBenchmark. TODO(cpovirk): benchmark
			 * this
			 */
			...
			public void validateClass(Class<? extends Exception> exceptionClass) {
				isValidClass.get(exceptionClass); // throws if invalid; returns
													// safely (and caches) if
													// valid
			}
		}
		...
		enum WeakSetValidator implements GetCheckedTypeValidator {
			 ...
			 */
			/*
			 * A CopyOnWriteArraySet<WeakReference> is faster than a
			 * newSetFromMap of a MapMaker map with weakKeys() and
			 * concurrencyLevel(1), even up to at least 12 cached exception
			 * types.
			 */
			...
			public void validateClass(Class<? extends Exception> exceptionClass) {
				...
				/*
				 * It's very unlikely that any loaded Futures class will see
				 * getChecked called with more than a handful of exceptions. But
				 * it seems prudent to set a cap on how many we'll cache. This
				 * avoids out-of-control memory consumption, and it keeps the
				 * cache from growing so large that doing the lookup is
				 * noticeably slower than redoing the work would be.
				 *
				 * Ideally we'd have a real eviction policy, but until we see a
				 * problem in practice, I hope that this will suffice. I have
				 * not even benchmarked with different size limits.
				 */
		...
		/**
		 * Returns the ClassValue-using validator, or falls back to the "weak
		 * Set" implementation if unable to do so.
		 */
	...
	// TODO(cpovirk): change parameter order to match other helper methods
	// (Class, Throwable)?
