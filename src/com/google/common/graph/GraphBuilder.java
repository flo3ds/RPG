...
/**
 * A builder for constructing instances of {@link MutableGraph} with
...
public final class GraphBuilder<N> extends AbstractGraphBuilder<N> {
	...
	/**
	 * Returns a {@link GraphBuilder} initialized with all properties queryable
	 * from {@code graph}.
	 *
	 * <p>
	 * The "queryable" properties are those that are exposed through the
	 * {@link Graph} interface, such as {@link Graph#isDirected()}. Other
	 * properties, such as {@link #expectedNodeCount(int)}, are not set in the
	 * new builder.
	 */
	public static <N> GraphBuilder<N> from(Graph<N> graph) {
		return new GraphBuilder<Object>(graph.isDirected()).allowsSelfLoops(graph.allowsSelfLoops())
