...
/**
 * A {@link Network} whose elements and structural relationships will never
 * change. Instances of this class may be obtained with
 * {@link #copyOf(Network)}.
 *
 * <p>
 ...
 * of the {@code Immutable*} types</a> for more information on the properties
 * and guarantees provided by this class.
 *
 * @author James Sexton
...
@SuppressWarnings("Immutable") // Extends ConfigurableNetwork but uses
								// ImmutableMaps.
public final class ImmutableNetwork<N, E> extends ConfigurableNetwork<N, E> {
	...
	public ImmutableGraph<N> asGraph() {
		return new ImmutableGraph<N>(super.asGraph()); // safe because the view
														// is effectively
														// immutable
	}

	private static <N, E> Map<N, NetworkConnections<N, E>> getNodeConnections(Network<N, E> network) {
		// ImmutableMap.Builder maintains the order of the elements as inserted,
		// so the map will have
		// whatever ordering the network's nodes do, so ImmutableSortedMap is
		// unnecessary even if the
	...
	private static <N, E> Map<E, N> getEdgeToReferenceNode(Network<N, E> network) {
		// ImmutableMap.Builder maintains the order of the elements as inserted,
		// so the map will have
		// whatever ordering the network's edges do, so ImmutableSortedMap is
		// unnecessary even if the
