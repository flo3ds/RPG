 ...
 * {
 * 	&#64;code
 * 	MutableValueGraph<String, Double> graph = ValueGraphBuilder.undirected().allowsSelfLoops(true).build();
...
public final class ValueGraphBuilder<N, V> extends AbstractGraphBuilder<N> {
	 ...
	 * {@link ValueGraph} interface, such as {@link ValueGraph#isDirected()}.
	 * Other properties, such as {@link #expectedNodeCount(int)}, are not set in
	 * the new builder.
	 */
	public static <N> ValueGraphBuilder<N, Object> from(ValueGraph<N, ?> graph) {
		return new ValueGraphBuilder<N, Object>(graph.isDirected()).allowsSelfLoops(graph.allowsSelfLoops())
