...
/**
 * A {@link Graph} whose elements and structural relationships will never
 * change. Instances of this class may be obtained with {@link #copyOf(Graph)}.
 *
 * <p>
...
public class ImmutableGraph<N> extends ForwardingGraph<N> {
	...
	public static <N> ImmutableGraph<N> copyOf(Graph<N> graph) {
		return (graph instanceof ImmutableGraph) ? (ImmutableGraph<N>) graph
				: new ImmutableGraph<N>(new ConfigurableValueGraph<N, Presence>(GraphBuilder.from(graph),
	...
	private static <N> ImmutableMap<N, GraphConnections<N, Presence>> getNodeConnections(Graph<N> graph) {
		...
		// whatever ordering the graph's nodes do, so ImmutableSortedMap is
	...
	private static <N> GraphConnections<N, Presence> connectionsOf(Graph<N> graph, N node) {
		...
		return graph.isDirected()
				? DirectedGraphConnections.ofImmutable(graph.predecessors(node),
