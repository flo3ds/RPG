 ...
 * caching provided by {@link MapIteratorCache}, this structure caches values
 * for the two most recently retrieved keys.
 *
 * @author James Sexton
 */
class MapRetrievalCache<K, V> extends MapIteratorCache<K, V> {
	...
	@SuppressWarnings("unchecked") // Safe because we only cast if key is found
									// in map.
	@Override
	...
	protected V getIfCached(@Nullable Object key) {
		...
		// immutable, this,
		// in combination with immutable cache entries, will ensure a
		// thread-safe cache.
		CacheEntry<K, V> entry;

		// Check cache. We use == on purpose because it's cheaper and a cache
		// miss is ok.
		entry = cacheEntry1;
	...
	private void addToCache(CacheEntry<K, V> entry) {
		// Slide new entry into first cache position. Drop previous entry in
		// second cache position.
