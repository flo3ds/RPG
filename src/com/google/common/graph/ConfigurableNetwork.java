...
/**
 * Configurable implementation of {@link Network} that supports the options
 * supplied by {@link NetworkBuilder}.
 *
 * <p>
 * This class maintains a map of nodes to {@link NetworkConnections}. This class
 * also maintains a map of edges to reference nodes. The reference node is
 * defined to be the edge's source node on directed graphs, and an arbitrary
 * endpoint of the edge on undirected graphs.
 *
 * <p>
...
class ConfigurableNetwork<N, E> extends AbstractNetwork<N, E> {
	...
	// We could make this a Map<E, EndpointPair<N>>. It would make
	// incidentNodes(edge) slightly
	// faster, but also make Networks consume 5 to 20+% (increasing with average
	// degree) more memory.
	protected final MapIteratorCache<E, N> edgeToReferenceNode; // referenceNode
																// == source if
																// directed
	...
	ConfigurableNetwork(NetworkBuilder<? super N, ? super E> builder) {
		this(builder,
				builder.nodeOrder
						.<N, NetworkConnections<N, E>>createMap(builder.expectedNodeCount.or(DEFAULT_NODE_COUNT)),
	 ...
	 */
	ConfigurableNetwork(NetworkBuilder<? super N, ? super E> builder, Map<N, NetworkConnections<N, E>> nodeConnections,
	...
	public Set<E> edgesConnecting(N nodeU, N nodeV) {
		...
		if (!allowsSelfLoops && nodeU == nodeV) { // just an optimization, only
