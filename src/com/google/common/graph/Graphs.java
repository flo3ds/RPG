...
/**
 * Static utility methods for {@link Graph}, {@link ValueGraph}, and
 * {@link Network} instances.
 *
 * @author James Sexton
...
public final class Graphs {
	...
	/**
	 * Returns true if {@code graph} has at least one cycle. A cycle is defined
	 * as a non-empty subset of edges in a graph arranged to form a path (a
	 * sequence of adjacent outgoing edges) starting and ending with the same
	 * node.
	 *
	 * <p>
	 * This method will detect any non-empty cycle, including self-loops (a
	 * cycle of length 1).
	 */
	public static <N> boolean hasCycle(Graph<N> graph) {
		...
		if (!graph.isDirected() && numEdges >= graph.nodes().size()) {
			return true; // Optimization for the undirected case: at least one
							// cycle must exist.
		}
	...
	/**
	 * Returns true if {@code network} has at least one cycle. A cycle is
	 * defined as a non-empty subset of edges in a graph arranged to form a path
	 * (a sequence of adjacent outgoing edges) starting and ending with the same
	 * node.
	 *
	 * <p>
	 * This method will detect any non-empty cycle, including self-loops (a
	...
	public static boolean hasCycle(Network<?, ?> network) {
		// In a directed graph, parallel edges cannot introduce a cycle in an
		// acyclic graph.
		// However, in an undirected graph, any parallel edge induces a cycle in
		// the graph.
	...
	/**
	 * Performs a traversal of the nodes reachable from {@code node}. If we ever
	 * reach a node we've already visited (following only outgoing edges and
	 * without reusing edges), we know there's a cycle in the graph.
	 */
	private static <N> boolean subgraphHasCycle(Graph<N> graph, Map<Object, NodeVisitState> visitedNodes, N node,
	...
	/**
	 * Determines whether an edge has already been used during traversal. In the
	 * directed case a cycle is always detected before reusing an edge, so no
	 * special logic is required. In the undirected case, we must take care not
	 * to "backtrack" over an edge (i.e. going from A to B and then going from B
	 * to A).
	 */
	private static boolean canTraverseWithoutReusingEdge(Graph<?> graph, Object nextNode,
		...
		// This falls into the undirected A->B->A case. The Graph interface does
		// not support parallel
		// edges, so this traversal would require reusing the undirected AB
		// edge.
		return false;
	}

	/**
	 * Returns the transitive closure of {@code graph}. The transitive closure
	 * of a graph is another graph with an edge connecting node A to node B if
	 * node B is {@link #reachableNodes(Graph, Object) reachable} from node A.
	 *
	 * <p>
	 * This is a "snapshot" based on the current topology of {@code graph},
	 * rather than a live view of the transitive closure of {@code graph}. In
	 * other words, the returned {@link Graph} will not be updated after
	 * modifications to {@code graph}.
	 */
	...
	public static <N> Graph<N> transitiveClosure(Graph<N> graph) {
		...
		// Every node is, at a minimum, reachable from itself. Since the
		// resulting transitive closure
		// will have no isolated nodes, we can skip adding nodes explicitly and
		// let putEdge() do it.

		if (graph.isDirected()) {
			// Note: works for both directed and undirected graphs, but we only
			// use in the directed case.
			for (N node : graph.nodes()) {
			...
			// An optimization for the undirected case: for every node B
	...
	/**
	 * Returns the set of nodes that are reachable from {@code node}. Node B is
	 * defined as reachable from node A if there exists a path (a sequence of
	 * adjacent outgoing edges) starting at node A and ending at node B. Note
	 * that a node is always reachable from itself via a zero-length path.
	 *
	 * <p>
	 * This is a "snapshot" based on the current topology of {@code graph},
	 * rather than a live view of the set of nodes reachable from {@code node}.
	 * In other words, the returned {@link Set} will not be updated after
	 * modifications to {@code graph}.
	 *
	 * @throws IllegalArgumentException
	 *             if {@code node} is not present in {@code graph}
	 */
	...
	/**
	 * @deprecated Use {@link Graph#equals(Object)} instead. This method will be
	...
	/**
	 * @deprecated Use {@link Network#equals(Object)} instead. This method will
	 *             be removed in late 2017.
	 */
	// TODO(user): Delete this method.
	@Deprecated
	...
	/**
	 * Returns a view of {@code graph} with the direction (if any) of every edge
	 * reversed. All other properties remain intact, and further updates to
	 * {@code graph} will be reflected in the view.
	 */
	public static <N> Graph<N> transpose(Graph<N> graph) {
		if (!graph.isDirected()) {
			return graph; // the transpose of an undirected graph is an
							// identical graph
		}
	...
	private static class TransposedGraph<N> extends AbstractGraph<N> {
		...
		/**
		 * Defer to {@link AbstractGraph#edges()} (based on
		 * {@link #successors(Object)}) for full edges() implementation.
		 */
		@Override
	...
	/**
	 * Returns a view of {@code network} with the direction (if any) of every
	 * edge reversed. All other properties remain intact, and further updates to
	 * {@code network} will be reflected in the view.
	 */
	public static <N, E> Network<N, E> transpose(Network<N, E> network) {
		if (!network.isDirected()) {
			return network; // the transpose of an undirected network is an
							// identical network
		}
	...
	/**
	 * Returns the subgraph of {@code graph} induced by {@code nodes}. This
	 * subgraph is a new graph that contains all of the nodes in {@code nodes},
	 * and all of the {@link Graph#edges() edges} from {@code graph} for which
	 * both nodes are contained by {@code nodes}.
	 *
	 * @throws IllegalArgumentException
	 *             if any element in {@code nodes} is not a node in the graph
	 */
	 ...
	 * Returns the subgraph of {@code graph} induced by {@code nodes}. This
	 * subgraph is a new graph that contains all of the nodes in {@code nodes},
	 * and all of the {@link Graph#edges() edges} (and associated edge values)
	...
	/**
	 * Returns the subgraph of {@code network} induced by {@code nodes}. This
	 * subgraph is a new graph that contains all of the nodes in {@code nodes},
	 * and all of the {@link Network#edges() edges} from {@code network} for
	 * which the {@link Network#incidentNodes(Object) incident nodes} are both
	 * contained by {@code nodes}.
	 *
	 * @throws IllegalArgumentException
	 *             if any element in {@code nodes} is not a node in the graph
	 */
	...
	/**
	 * Creates a mutable copy of {@code graph} with the same nodes, edges, and
	 * edge values.
	 */
	...
	/**
	 * An enum representing the state of a node during DFS. {@code PENDING}
	 * means that the node is on the stack of the DFS, while {@code COMPLETE}
	 * means that the node and all its successors have been already explored.
	 * Any node that has not been explored will not have a state at all.
	 */
