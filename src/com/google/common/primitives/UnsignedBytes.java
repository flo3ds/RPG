...
public final class UnsignedBytes {
	 ...
	 * @throws IllegalArgumentException
	 *             if {@code radix} is not between {@link Character#MIN_RADIX}
	 ...
	 *             if the string does not contain a valid unsigned {@code byte}
	 *             value
	 * @throws NullPointerException
	 *             if {@code string} is null (in contrast to
	 ...
	 *
	 * @param string
	 *            the string containing the unsigned {@code byte} representation
	 *            to be parsed.
	 * @param radix
	 *            the radix to use while parsing {@code string}
	 * @throws NumberFormatException
	 *             if the string does not contain a valid unsigned {@code byte}
	...
	/**
	 * Returns a comparator that compares two {@code byte} arrays <a href=
	 * "http://en.wikipedia.org/wiki/Lexicographical_order">lexicographically</a>.
	 * That is, it compares, using {@link #compare(byte, byte)}), the first pair
	 ...
	 * {@code [] < [0x01] < [0x01, 0x7F] < [0x01, 0x80] < [0x02]}. Values are
	 * treated as unsigned.
	 *
	 * <p>
	...
	static class LexicographicalComparatorHolder {
		...
		enum UnsafeComparator implements Comparator<byte[]> {
			 ...
			 *
			 * In UnsignedBytesBenchmark, accessing the following objects via
			 ...
			 * object remains inside the loop and BYTE_ARRAY_BASE_OFFSET doesn't
			 * get constant-folded.
			 *
			...
			static {
				// fall back to the safer pure java implementation unless we're
				// in
			...
			/**
			 * Returns a sun.misc.Unsafe. Suitable for use in a 3rd party
			 * package. Replace with a simple call to Unsafe.getUnsafe when
			 * integrating into a jdk.
			 *
			...
			public int compare(byte[] left, byte[] right) {
				...
				/*
				 * Compare 8 bytes at a time. Benchmarking on x86 shows a stride
				 * of 8 bytes is no slower than 4 bytes even on 32-bit. On the
				 * other hand, it is substantially faster on 64-bit.
				 */
				for (i = 0; i < strideLimit; i += stride) {
					...
					if (lw != rw) {
						...
						/*
						 * We want to compare only the first index where
						 * left[index] != right[index]. This corresponds to the
						 * least significant nonzero byte in lw ^ rw, since lw
						 * and rw are little-endian.
						 * Long.numberOfTrailingZeros(diff) tells us the least
						 * significant nonzero bit, and zeroing out the first
						 * three bits of L.nTZ gives us the shift to get that
						 * least significant nonzero byte.
						 */
