...
public final class Floats {
	 ...
	 * primitive types, whose compare methods were not added to the JDK until
	 * JDK 7.
	 *
	 * @param a
	 ...
	 * in {@code array}. Note that this always returns {@code false} when {@code
	 * target} is {@code NaN}.
	 *
	 * @param array
	 ...
	 *
	 * @param value
	 *            the {@code float} value to constrain
	 * @param min
	 ...
	 *
	 * <p>
	 * Note that {@link Float#toString(float)} formats {@code float} differently
	 * in GWT. In the previous example, it returns the string {@code
	 * "1-2-3"}.
	 *
	 * @param separator
	...
	/**
	 * Returns a comparator that compares two {@code float} arrays <a href=
	 * "http://en.wikipedia.org/wiki/Lexicographical_order">lexicographically</a>.
	 * That is, it compares, using {@link #compare(float, float)}), the first
	 * pair of values that follow any common prefix, or when one array is a
	 ...
	 * to a {@code float} value in the manner of {@link Number#floatValue}.
	 *
	 * <p>
	 ...
	 * instead of throwing an exception if parsing fails. Valid inputs are
	 * exactly those accepted by {@link Float#valueOf(String)}, except that
	 ...
	 *
	 * @param string
	 *            the string representation of a {@code float} value
	...
	public static Float tryParse(String string) {
		if (Doubles.FLOATING_POINT_PATTERN.matcher(string).matches()) {
			...
			try {
				...
				// Float.parseFloat has changed specs several times, so fall
				// through
				// gracefully
			}
		}
