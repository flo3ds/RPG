...
public final class Chars {
	...
	/**
	 * Returns the {@code char} value that is equal to {@code value}, if
	 * possible.
	 *
	 * @param value
	 *            any value in the range of the {@code char} type
	 * @return the {@code char} value that equals {@code value}
	 * @throws IllegalArgumentException
	 *             if {@code value} is greater than {@link Character#MAX_VALUE}
	 ...
	 *         {@code char} type, {@link Character#MAX_VALUE} if it is too
	 *         large, or {@link Character#MIN_VALUE} if it is too small
	 */
	 ...
	 *
	 * @param array
	 *            an array of {@code char} values, possibly empty
	 * @param target
	 *            a primitive {@code char} value
	 ...
	 *         {@code array[i] == target}, or {@code -1} if no such index
	 *         exists.
	 */
	 ...
	 *
	 * @param array
	 *            a <i>nonempty</i> array of {@code char} values
	...
	/**
	 * Returns the value nearest to {@code value} which is within the closed
	 * range {@code [min..max]}.
	 *
	 * <p>
	 * If {@code value} is within the range {@code [min..max]}, {@code value} is
	 * returned unchanged. If {@code value} is less than {@code min},
	 * {@code min} is returned, and if {@code value} is greater than
	 * {@code max}, {@code max} is returned.
	 *
	 * @param value
	 *            the {@code char} value to constrain
	 * @param min
	 *            the lower bound (inclusive) of the range to constrain
	 *            {@code value} to
	 * @param max
	 *            the upper bound (inclusive) of the range to constrain
	 *            {@code value} to
	 * @throws IllegalArgumentException
	 *             if {@code min > max}
	 * @since 21.0
	 ...
	 * For example, {@code concat(new char[] {a, b}, new char[] {}, new char[]
	 * {c}} returns the array {@code {a, b, c}}.
	 *
	 * @param arrays
	 *            zero or more {@code char} arrays
	 ...
	 * array; equivalent to
	 * {@code ByteBuffer.allocate(2).putChar(value).array()}. For example, the
	 * input value {@code '\\u5432'} would yield the byte array {@code {0x54,
	 * 0x32}}.
	 *
	 * <p>
	...
	/**
	 * Returns the {@code char} value whose big-endian representation is stored
	 * in the first 2 bytes of {@code bytes}; equivalent to
	 * {@code ByteBuffer.wrap(bytes).getChar()}. For example, the input byte
	 * array {@code {0x54, 0x32}} would yield the {@code char} value
	 * {@code '\\u5432'}.
	 *
	 * <p>
	 ...
	 *
	 * @throws IllegalArgumentException
	 *             if {@code bytes} has fewer than 2 elements
	 */
	@GwtIncompatible // doesn't work
	...
	/**
	 * Returns a comparator that compares two {@code char} arrays <a href=
	 * "http://en.wikipedia.org/wiki/Lexicographical_order">lexicographically</a>.
	 * That is, it compares, using {@link #compare(char, char)}), the first pair
	 * of values that follow any common prefix, or when one array is a prefix of
	 * the other, treats the shorter array as the lesser. For example,
	 * {@code [] < ['a'] < ['a', 'b'] < ['b']}.
	 *
	 * <p>
	 ...
	 *
	 * @param collection
	 *            a collection of {@code Character} objects
	 ...
	 * {@code Character} objects written to or read from it. For example,
	 * whether {@code list.get(0) == list.get(0)} is true for the returned list
	 * is unspecified.
	 *
	 * @param backingArray
