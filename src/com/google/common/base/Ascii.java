...
public final class Ascii {
	 ...
	 * devices.) Where appropriate, this character may have the meaning "New
	 * Line" (NL), a format effector which controls the movement of the printing
	 * point to the first printing position on the next printing line. Use of
	 * this convention requires agreement between sender and recipient of data.
	 *
	 * @since 8.0
	 */
	...
	public static boolean isLowerCase(char c) {
		// Note: This was benchmarked against the alternate expression "(char)(c
		// - 'a') < 26" (Nov '13)
	...
	/**
	 * Truncates the given character sequence to the given maximum length. If
	 * the length of the sequence is greater than {@code maxLength}, the
	 * returned string will be exactly {@code maxLength} chars in length and
	 * will end with the given {@code truncationIndicator}. Otherwise, the
	 * sequence will be returned as a string with no changes to the content.
	 *
	 * <p>
	 * Examples:
	 *
	 ...
	 *
	 * <p>
	 * <b>Note:</b> This method <i>may</i> work with certain non-ASCII text but
	 * is not safe for use with arbitrary Unicode text. It is mostly intended
	 * for use with text that is known to be safe for use with it (such as
	 * all-ASCII text) and for simple debugging text. When using this method,
	 ...
	 * <li>if truncating for display to users, there are other considerations
	 ...
	 *             if {@code maxLength} is less than the length of
	...
	public static String truncate(CharSequence seq, int maxLength, String truncationIndicator) {
		...
		// in this worst case, this allows a maxLength equal to the length of
		// the truncationIndicator,
		// meaning that a string will be truncated to just the truncation
		// indicator itself
		...
		if (seq.length() <= maxLength) {
			...
			// if the length of the toString() result was > maxLength for some
			// reason, truncate that
			seq = string;
		}
	 ...
	 * {@code s1} and {@code s2} are equal, ignoring the case of any ASCII
	 * alphabetic characters between {@code 'a'} and {@code 'z'} or {@code 'A'}
	 * and {@code 'Z'} inclusive.
	 *
	 * <p>
	 * This method is significantly faster than {@link String#equalsIgnoreCase}
	 * and should be used in preference if at least one of the parameters is
	 * known to contain only ASCII characters.
	 *
	 * <p>
	 ...
	 * strings are used, the author probably wanted the behavior provided by
	 * this method rather than the subtle and sometimes surprising behavior of
	 * {@code toUpperCase()} and {@code toLowerCase()}.
	 *
	 * @since 16.0
	 */
	public static boolean equalsIgnoreCase(CharSequence s1, CharSequence s2) {
		// Calling length() is the null pointer check (so do it before we can
		// exit early).
		...
		for (int i = 0; i < length; i++) {
			...
			// This was also benchmarked using '&' to avoid branching (but
	 ...
	 * regardless of case. Ie, 'a'/'A' returns 0 and 'z'/'Z' returns 25.
	 * Non-alpha characters return a value of 26 or greater.
	 */
	private static int getAlphaIndex(char c) {
		// Fold upper-case ASCII to lower-case and make zero-indexed and
		// unsigned (by casting to char).
