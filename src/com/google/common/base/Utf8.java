...
public final class Utf8 {
	 ...
	 * {@code sequence}. For a string, this method is equivalent to
	 * {@code string.getBytes(UTF_8).length}, but is more efficient in both time
	 * and space.
	 *
	 * @throws IllegalArgumentException
	 *             if {@code sequence} contains ill-formed UTF-16 (unpaired
	 *             surrogates)
	 */
	public static int encodedLength(CharSequence sequence) {
		...
		if (utf8Length < utf16Length) {
			// Necessary and sufficient condition for overflow because of
	 ...
	 * than simply whether the bytes can be decoded. For example, some versions
	 * of the JDK decoder will accept "non-shortest form" byte sequences, but
	 * encoding never reproduces these. Such byte sequences are <i>not</i>
	 * considered well-formed.
	 *
	 * <p>
	 * This method returns {@code true} if and only if
	 * {@code Arrays.equals(bytes, new
	 * String(bytes, UTF_8).getBytes(UTF_8))} does, but is more efficient in
	 * both time and space.
	 */
	 ...
	 * sequence, as defined by {@link #isWellFormed(byte[])}. Note that this can
	 * be false even when {@code
	...
	private static boolean isWellFormedSlowPath(byte[] bytes, int off, int end) {
		...
		while (true) {
			...
			if (byte1 < (byte) 0xE0) {
				...
				// Simultaneously check for illegal trailing-byte in leading
				// position
				...
				if (byte2 > (byte) 0xBF
						// Overlong? 5 most significant bits must not all be
						// zero.
				...
				int byte2 = bytes[index++];
				if (byte2 > (byte) 0xBF
						// Check that 1 <= plane <= 16. Tricky optimized form
						// of:
						// if (byte1 > (byte) 0xF4
