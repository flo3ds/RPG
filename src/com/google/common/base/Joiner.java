...
public class Joiner {
	 ...
	 * {@code multimap.entries()} to a {@code MapJoiner} method that accepts
	 * entries as input, and receive output of the form
	 * {@code key1=A&key1=B&key2=C}.
	 * <li>To output a single entry for each key, pass {@code multimap.asMap()}
	 * to a {@code MapJoiner} method that accepts a map as input, and receive
	 * output of the form {@code
	 *     key1=[A, B]&key2=C}.
	 * </ul>
	 *
	 * @since 2.0
	...
	public static final class MapJoiner {
		...
		private MapJoiner(Joiner joiner, String keyValueSeparator) {
			this.joiner = joiner; // only "this" is ever passed, so don't
									// checkNotNull
		...
		/**
		 * Appends the string representation of each entry in {@code entries},
		 * using the previously configured separator and key-value separator, to
		 * {@code appendable}.
		 *
		 * @since 10.0
		 */
		 ...
		 * Appends the string representation of each entry in {@code entries},
		 * using the previously configured separator and key-value separator, to
		 * {@code builder}. Identical to
		 * {@link #appendTo(Appendable, Iterable)}, except that it does not
		...
		/**
		 * Returns a string containing the string representation of each entry
		 * in {@code entries}, using the previously configured separator and
		 * key-value separator.
		 *
		 * @since 10.0
		 */
