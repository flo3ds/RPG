 ...
 *
 * <p>
 * This class can't depend on other Guava code. If we were to load this class in
 * the same class loader as the rest of Guava, this thread would keep an
 * indirect strong reference to the class loader and prevent it from being
 * garbage collected. This poses a problem for environments where you want to
 * throw away the class loader. For example, dynamically reloading a web
 * application or unloading an OSGi bundle.
 *
 * <p>
...
public class Finalizer implements Runnable {
	 ...
	 * @param queue
	 *            a reference queue that the thread will poll.
	 * @param frqReference
	 *            a phantom reference to the FinalizableReferenceQueue, which
	 *            will be queued either when the FinalizableReferenceQueue is no
	 *            longer referenced anywhere, or when its close() method is
	 *            called.
	 */
	public static void startFinalizer(Class<?> finalizableReferenceClass, ReferenceQueue<Object> queue,
		 ...
		 *
		 * 2) To detect when FinalizableReference's class loader has to be
		 * garbage collected, at which point, Finalizer can stop running
		 */
	...
	private Method getFinalizeReferentMethod() {
		...
		if (finalizableReferenceClass == null) {
			/*
			 * FinalizableReference's class loader was reclaimed. While there's
			 * a chance that other finalizable references could be enqueued
			 * subsequently (at which point the class loader would be
			 * resurrected by virtue of us having a strong reference to it), we
			 * should pretty much just shut down and make sure we don't keep it
