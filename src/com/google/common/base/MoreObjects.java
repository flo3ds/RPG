...
/**
 * Helper functions that operate on any {@code Object}, and are not already
 * provided in {@link java.util.Objects}.
 *
 * <p>
 * See the Guava User Guide on <a href=
 * "https://github.com/google/guava/wiki/CommonObjectUtilitiesExplained">writing
...
public final class MoreObjects {
	 ...
	 *
	 * <p>
	 * To find the first non-null element in an iterable, use {@code
	 * Iterables.find(iterable, Predicates.notNull())}. For varargs, use {@code
	 * Iterables.find(Arrays.asList(a, b, c, ...), Predicates.notNull())},
	 * static importing as necessary.
	 *
	 * <p>
	 * <b>Note:</b> if {@code first} is represented as an {@link Optional}, this
	 * can be accomplished with {@link Optional#or(Object) first.or(second)}.
	 * That approach also allows for lazy evaluation of the fallback instance,
	 * using {@link Optional#or(Supplier) first.or(supplier)}.
	 *
	 * @return {@code first} if it is non-null; otherwise {@code second} if it
	 *         is non-null
	 ...
	 *
	 * <p>
	 * This is helpful for implementing {@link Object#toString()}. Specification
	 * by example:
	 *
	 * <pre>
	 *    {@code
	 ...
	 *            {@code this}), used only for its class name
	 ...
	 * {@link #toStringHelper(Object)}, but using the simple name of
	 * {@code clazz} instead of using an instance's {@link Object#getClass()}.
	 *
	 * <p>
	 * Note that in GWT, class names are often obfuscated.
	 *
	...
	/**
	 * Creates an instance of {@link ToStringHelper} in the same manner as
	 * {@link #toStringHelper(Object)}, but using {@code className} instead of
	 * using an instance's {@link Object#getClass()}.
	 *
	 * @param className
	...
	public static final class ToStringHelper {
		...
		/**
		 * Configures the {@link ToStringHelper} so {@link #toString()} will
		 * ignore properties with null value. The order of calling this method,
		 * relative to the {@code add()}/{@code addValue()} methods, is not
		 * significant.
		 *
		 * @since 18.0 (since 12.0 as
		 *        {@code Objects.ToStringHelper.omitNullValues()}).
		 */
		 ...
		 * is used, unless {@link #omitNullValues()} is called, in which case
		 ...
		 *
		 * <p>
		 * It is strongly encouraged to use {@link #add(String, boolean)}
		 * instead and give value a readable name.
		 *
		 * @since 18.0 (since 11.0 as
		 *        {@code Objects.ToStringHelper.addValue()}).
		 */
		@CanIgnoreReturnValue
		 ...
		 * the same object; but properties cannot be removed, so this only
		 * allows limited reuse of the helper instance. The helper allows
		 * duplication of properties (multiple name/value pairs with the same
		 * name can be added).
		 */
		@Override
