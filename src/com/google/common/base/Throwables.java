 ...
 *
 * <p>
 * See the Guava User Guide entry on <a href=
...
public final class Throwables {
	 ...
	 * </pre>
	 *
	 * @deprecated Use {@link #throwIfUnchecked}, which has the same behavior
	 *             but rejects {@code
	 *     null}. This method is scheduled to be removed in July 2018.
	 */
	@Deprecated
	 ...
	 * wraps it in a {@code RuntimeException} and then propagates.
	 *
	 * <p>
	 * This method always throws an exception. The {@code RuntimeException}
	 * return type allows client code to signal to the compiler that statements
	 * after the call are unreachable. Example usage:
	 *
	 * <pre>
	 * T doSomething() {
	 * 	try {
	 ...
	 * @deprecated Use {@code throw e} or {@code throw new RuntimeException(e)}
	 *             directly, or use a combination of {@link #throwIfUnchecked}
	 *             and {@code throw new RuntimeException(e)}. For background on
	 *             the deprecation, read <a href="https://goo.gl/Ivn2kc">Why we
	 *             deprecated {@code Throwables.propagate}</a>. This method is
	 ...
	 * list will be {@code
	 * throwable} followed by its cause hierarchy. Note that this is a snapshot
	 * of the cause chain and will not reflect any subsequent changes to the
	 * cause chain.
	 *
	 * <p>
	 ...
	 *
	 * @param throwable
	 *            the non-null {@code Throwable} to extract causes from
	 ...
	 * example, {@code
	 * (IOException) e.getCause()} throws a {@link ClassCastException} that
	 * discards the original exception {@code e} if the cause is not an
	 * {@link IOException}, but {@code
	 * Throwables.getCauseAs(e, IOException.class)} keeps {@code e} as the
	 * {@link ClassCastException}'s cause.
	 *
	 * @throws ClassCastException
	 *             if the cause cannot be cast to the expected type. The {@code
	...
	/**
	 * Returns the stack trace of {@code throwable}, possibly providing slower
	 * iteration over the full trace but faster iteration over parts of the
	 * trace. Here, "slower" and "faster" are defined in comparison to the
	 * normal way to access the stack trace, {@link Throwable#getStackTrace()
	 * throwable.getStackTrace()}. Note, however, that this method's special
	 * implementation is not available for all platforms and configurations. If
	 * that implementation is unavailable, this method falls back to
	 * {@code getStackTrace}. Callers that require the special implementation
	 * can check its availability with {@link #lazyStackTraceIsLazy()}.
	 *
	 * <p>
	 * The expected (but not guaranteed) performance of the special
	 * implementation differs from {@code getStackTrace} in one main way: The
	 * {@code lazyStackTrace} call itself returns quickly by delaying the
	 * per-stack-frame work until each element is accessed. Roughly speaking:
	 *
	 * <ul>
	 ...
	 * {@code 1/stackSize} time to retrieve each element of the returned list
	 * (probably slightly more than {@code
	 *       1/stackSize}).
	 * </ul>
	 *
	 * <p>
	 * Note: The special implementation does not respect calls to
	 * {@link Throwable#setStackTrace throwable.setStackTrace}. Instead, it
	 * always reflects the original stack trace from the exception's creation.
	 *
	 * @since 19.0
	 */
	// TODO(cpovirk): Say something about the possibility that List access could
	// fail at runtime?
	@Beta
	@GwtIncompatible // lazyStackTraceIsLazy, jlaStackTrace
	// TODO(cpovirk): Consider making this available under GWT (slow
	// implementation only).
	...
	/**
	 * Returns whether {@link #lazyStackTrace} will use the special
	 * implementation described in its documentation.
	 *
	 * @since 19.0
	 */
	@Beta
	...
	private static List<StackTraceElement> jlaStackTrace(final Throwable t) {
		checkNotNull(t);
		/*
		 * TODO(cpovirk): Consider optimizing iterator() to catch IOOBE instead
		 * of doing bounds checks.
		 *
		 * TODO(cpovirk): Consider the UnsignedBytes pattern if it performs
		 * faster and doesn't cause AOSP grief.
		 */
	...
	/**
	 * The "getStackTraceElementMethod" method, only available on some JDKs so
	 * we use reflection to find it when available. When this is null, use the
	 * slow way.
	 */
	@GwtIncompatible // java.lang.reflect
	@Nullable
	...
	/**
	 * The "getStackTraceDepth" method, only available on some JDKs so we use
	 * reflection to find it when available. When this is null, use the slow
	 * way.
	 */
	@GwtIncompatible // java.lang.reflect
	@Nullable
	...
	/**
	 * Returns the JavaLangAccess class that is present in all Sun JDKs. It is
	 * not whitelisted for AppEngine, and not present in non-Sun JDKs.
	 */
	@GwtIncompatible // java.lang.reflect
	@Nullable
	private static Object getJLA() {
		try {
			/*
			 * We load sun.misc.* classes using reflection since Android doesn't
			 * support these classes and would result in compilation failure if
			 * we directly refer to these classes.
			 */
			...
			/*
			 * This is not one of AppEngine's whitelisted classes, so even in
			 * Sun JDKs, this can fail with a NoClassDefFoundError. Other apps
			 * might deny access to sun.misc packages.
			 */
			return null;
		}
	}
	...
	/**
	 * Returns the Method that can be used to resolve an individual
	 * StackTraceElement, or null if that method cannot be found (it is only to
	 * be found in fairly recent JDKs).
	 */
	@GwtIncompatible // java.lang.reflect
	@Nullable
	...
	/**
	 * Returns the Method that can be used to return the size of a stack, or
	 * null if that method cannot be found (it is only to be found in fairly
	 * recent JDKs).
	 */
	@GwtIncompatible // java.lang.reflect
	@Nullable
	...
	private static Method getJlaMethod(String name, Class<?>... parameterTypes) throws ThreadDeath {
		try {
			...
			/*
			 * Either the JavaLangAccess class itself is not found, or the
			 * method is not supported on the JVM.
			 */
			return null;
		}
	}
