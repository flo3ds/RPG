...
public final class Suppliers {
	 ...
	 * and supplier. In other words, the new supplier's value will be computed
	 * by retrieving the value from {@code supplier}, and then applying
	 ...
	 *
	 * <p>
	 * The returned supplier is thread-safe. The delegate's {@code get()} method
	 * will be invoked at most once. The supplier's serialized form does not
	 * contain the cached value, which will be recalculated when {@code get()}
	 * is called on the reserialized instance.
	 *
	 * <p>
	 * If {@code delegate} is an instance created by an earlier call to
	 * {@code memoize}, it is returned directly.
	 */
	 ...
	 *
	 * <p>
	 * The returned supplier is thread-safe. The supplier's serialized form does
	 * not contain the cached value, which will be recalculated when {@code
	 * get()} is called on the reserialized instance.
	 *
	 * @param duration
	 *            the length of time after a value is created that it should
	...
	static class ExpiringMemoizingSupplier<T> implements Supplier<T>, Serializable {
			...
			if (nanos == 0 || now - nanos >= 0) {
				synchronized (this) {
					if (nanos == expirationNanos) { // recheck for lost race
						...
						// In the very unlikely event that nanos is 0, set it to
						// 1;
		...
		@Override
		public String toString() {
			// This is a little strange if the unit the user provided was not
			// NANOS,
	 ...
	 *
	 * <p>
	 * <b>Java 8 users:</b> use the method reference {@code Supplier::get}
	 * instead.
	 *
	 * @since 8.0
	 */
