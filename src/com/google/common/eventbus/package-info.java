 ...
 * "https://github.com/google/guava/wiki/EventBusExplained">{@code EventBus}</a>.
 *
 ...
 * defined with the event &mdash; such as CustomerChangeEventListener.
 ...
 * producer's {@code registerCustomerChangeEventListener} method. These methods
 * are rarely defined in common interfaces, so in addition to knowing every
 * possible producer, you must also know its type.
 ...
 * EventBus. You'll need to make sure that your object shares an EventBus
 ...
 * listeners of any supertype, allowing listeners for interface types or
 * "wildcard listeners" for Object.
 * </ul>
 *
 * <p>
 ...
 * {@link com.google.common.eventbus.DeadEvent}. The EventBus will notify you of
 * any events that were posted but not delivered. (Handy for debugging.)
 * </ul>
 *
 * <h3>For Producers</h3>
 *
 ...
 * <ul>
 * <li><strong>...in traditional Java events:</strong> write code to manage a
 * list of listeners to your object, including synchronization, or use a utility
 * class like EventListenerList.
 ...
 * dispatch events to each event listener, including error isolation and (if
 * desired) asynchronicity.
 ...
 *
 * <dt>Listener
 * <dd>An object that wishes to receive events, by exposing <em>subscriber
 * methods</em>.
 *
 * <dt>Subscriber method
 * <dd>A public method that the EventBus should use to deliver <em>posted</em>
 * events. Subscriber methods are marked by the
 * {@link com.google.common.eventbus.Subscribe} annotation.
 *
 * <dt>Posting an event
 * <dd>Making the event available to any <em>listeners</em> through the
 * EventBus.
 *
 * </dl>
 *
 ...
 * nothing stopping you from doing it that way. Simply have your container (such
 * as Guice) create the EventBus as a singleton at global scope (or stash it in
 * a static field, if you're into that sort of thing).
 *
 * <p>
 * In short, the EventBus is not a singleton because we'd rather not make that
 * decision for you. Use it how you like.
 *
 * <h3>Why use an annotation to mark subscriber methods, rather than requiring
 * the listener to implement an interface?</h3>
 *
 * <p>
 ...
 * so), while leaving you free to place event subscriber methods wherever you
 * wish and give them intention-revealing names.
 *
 * <p>
 * Traditional Java Events use a listener interface which typically sports only
 * a handful of methods -- typically one. This has a number of disadvantages:
 * <ul>
 * <li>Any one class can only implement a single response to a given event.
 * <li>Listener interface methods may conflict.
 * <li>The method must be named after the event (e.g.
 * {@code handleChangeEvent}), rather than its purpose (e.g.
 * {@code recordChangeInJournal}).
 ...
 * traditional Java Events, where even if {@code actionPerformed} and
 * {@code keyPressed} aren't very meaningful names, at least you can implement
 * both methods!
 *
 ...
 *
 * <p>
 * {@code Object} is used here for a good reason: the Event Bus library places
 * no restrictions on the types of either your event listeners (as in
 ...
 * methods for that event, and renaming the type will affect all subscriber
 * methods within view of your IDE (and any code that creates the event).
 *
 * <p>
 ...
 *
 * <p>
 * The Event Bus was designed to integrate with containers and module systems,
 * with Guice as the prototypical example. In these cases, it's convenient to
 * have the container/factory/environment pass <i>every</i> created object to an
 * EventBus's {@code register(Object)} method.
 *
 * <p>
 * This way, any object created by the container/factory/environment can hook
 * into the system's event model simply by exposing subscriber methods.
 *
 ...
 * registered is checked for the <i>well-formedness</i> of its subscriber
 * methods. Specifically, any methods marked with {@code @Subscribe} must take
 * only a single argument.
 *
 * <p>
 * Any violations of this rule will cause an {@code IllegalArgumentException} to
 * be thrown.
 *
 * <p>
 ...
 * {@code @Subscribe} annotation, or that the listening component is not
 * loaded).
 *
 * <p>
 * (Note that this is <i>not necessarily</i> indicative of a problem. There are
 * many cases where an application will deliberately ignore a posted event,
 ...
 *
 * <p>
 * Because subscriber methods on your listener classes are normal methods, you
 * can simply call them from your test code to simulate the EventBus.
 */
@CheckReturnValue
