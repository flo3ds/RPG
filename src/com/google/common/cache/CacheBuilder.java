...
/**
 * <p>
 * A builder of {@link LoadingCache} and {@link Cache} instances having any
 ...
 *
 * <p>
 * These features are all optional; caches can be created using all or none of
 * them. By default cache instances created by {@code CacheBuilder} will not
 * perform any type of eviction.
 *
 * <p>
 * Usage example:
 * 
 ...
 * 			.expireAfterWrite(10, TimeUnit.MINUTES).removalListener(MY_LISTENER)
 ...
 * characteristics to {@link ConcurrentHashMap}. It implements all optional
 * operations of the {@link LoadingCache} and {@link Cache} interfaces. The
 * {@code asMap} view (and its collection views) have <i>weakly consistent
 * iterators</i>. This means that they are safe for concurrent use, but if other
 * threads modify the cache after the iterator is created, it is undefined which
 * of these changes, if any, are reflected in that iterator. These iterators
 * never throw {@link ConcurrentModificationException}.
 *
 * <p>
 * <b>Note:</b> by default, the returned cache uses equality comparisons (the
 * {@link Object#equals equals} method) to determine equality for keys or
 * values. However, if {@link #weakKeys} was specified, the cache uses identity
 * ({@code ==}) comparisons instead for keys. Likewise, if {@link #weakValues}
 * or {@link #softValues} was specified, the cache uses identity comparisons for
 * values.
 *
 * <p>
 ...
 * {@linkplain #expireAfterAccess expireAfterAccess}, {@linkplain #weakKeys
 * weakKeys}, {@linkplain #weakValues weakValues}, or {@linkplain #softValues
 * softValues} are requested.
 *
 * <p>
 * If {@linkplain #maximumSize(long) maximumSize} or
 * {@linkplain #maximumWeight(long) maximumWeight} is requested entries may be
 * evicted on each cache modification.
 *
 * <p>
 * If {@linkplain #expireAfterWrite expireAfterWrite} or
 * {@linkplain #expireAfterAccess expireAfterAccess} is requested entries may be
 * evicted on each cache modification, on occasional cache accesses, or on calls
 * to {@link Cache#cleanUp}. Expired entries may be counted by
 ...
 * {@linkplain #softValues softValues} are requested, it is possible for a key
 * or value present in the cache to be reclaimed by the garbage collector.
 * Entries with reclaimed keys or values may be removed from the cache on each
 * cache modification, on occasional cache accesses, or on calls to
 * {@link Cache#cleanUp}; such entries may be counted in {@link Cache#size}, but
 * will never be visible to read or write operations.
 *
 * <p>
 * Certain cache configurations will result in the accrual of periodic
 * maintenance tasks which will be performed during write operations, or during
 * occasional read operations in the absence of writes. The
 * {@link Cache#cleanUp} method of the returned cache will also perform
 * maintenance, but calling it should not be necessary with a high throughput
 ...
 *
 * <p>
 * The caches produced by {@code CacheBuilder} are serializable, and the
 * deserialized caches retain all the configuration properties of the original
 * cache. Note that the serialized form does <i>not</i> include cache contents,
 * but only configuration.
 *
 * <p>
 * See the Guava User Guide article on
 * <a href="https://github.com/google/guava/wiki/CachesExplained">caching</a>
 * for a higher-level explanation.
 *
 * @param <K>
 *            the base key type for all caches created by this builder
 * @param <V>
...
public final class CacheBuilder<K, V> {
	 ...
	 * including strong keys, strong values, and no automatic eviction of any
	 * kind.
	 */
	...
	/**
	 * Constructs a new {@code CacheBuilder} instance with the settings
	 * specified in {@code spec}.
	 *
	 * @since 12.0
	 */
	...
	/**
	 * Constructs a new {@code CacheBuilder} instance with the settings
	 * specified in {@code spec}. This is especially useful for command-line
	 * configuration of a {@code CacheBuilder}.
	 *
	 * @param spec
	 *            a String in the format specified by {@link CacheBuilderSpec}
	 * @since 12.0
	 */
	@GwtIncompatible // To be supported
	 ...
	 *
	 * <p>
	 * By default, the cache uses {@link Equivalence#identity} to determine key
	 * equality when {@link #weakKeys} is specified, and
	 ...
	 *
	 * <p>
	 * By default, the cache uses {@link Equivalence#identity} to determine
	 * value equality when {@link #weakValues} or {@link #softValues} is
	 ...
	 * thread to modify the cache at a time, but since read operations and cache
	 * loading computations can proceed concurrently, this still yields higher
	 * concurrency than full synchronization.
	 *
	 * <p>
	 * Defaults to 4. <b>Note:</b>The default may change in the future. If you
	 * care about this value, you should always choose it explicitly.
	 *
	 * <p>
	 * The current implementation uses the concurrency level to create a fixed
	 * number of hashtable segments, each governed by its own write lock. The
	 * segment lock is taken once for each explicit write, and twice for each
	 * cache loading computation (once prior to loading the new value, and once
	 * after loading completes). Much internal cache management is performed at
	 * the segment granularity. For example, access queues and write queues are
	 * kept per segment when they are required by the selected eviction
	 * algorithm. As such, when writing unit tests it is not uncommon to specify
	 * {@code concurrencyLevel(1)} in order to achieve more deterministic
	 * eviction behavior.
	 *
	 * <p>
	 * Note that future implementations may abandon segment locking in favor of
	 * more advanced concurrency controls.
	 *
	 * @return this {@code CacheBuilder} instance (for chaining)
	 * @throws IllegalArgumentException
	 ...
	 *
	 * <p>
	 * Note that the cache <b>may evict an entry before this limit is
	 * exceeded</b>. For example, in the current implementation, when
	 * {@code concurrencyLevel} is greater than {@code 1}, each resulting
	 * segment inside the cache <i>independently</i> limits its own size to
	 * approximately {@code maximumSize / concurrencyLevel}.
	 *
	 * <p>
	 * When eviction is necessary, the cache evicts entries that are less likely
	 * to be used again. For example, the cache may evict an entry because it
	 * hasn't been used recently or very often.
	 *
	 * <p>
	 * If {@code maximumSize} is zero, elements will be evicted immediately
	 * after being loaded into cache. This can be useful in testing, or to
	 ...
	 * @return this {@code CacheBuilder} instance (for chaining)
	 * @throws IllegalArgumentException
	 *             if {@code maximumSize} is negative
	 * @throws IllegalStateException
	 ...
	 * determined using the {@link Weigher} specified with {@link #weigher}, and
	 * use of this method requires a corresponding call to {@link #weigher}
	 ...
	 * exceeded</b>. For example, in the current implementation, when
	 * {@code concurrencyLevel} is greater than {@code 1}, each resulting
	 * segment inside the cache <i>independently</i> limits its own weight to
	 * approximately {@code maximumWeight / concurrencyLevel}.
	 *
	 * <p>
	 * When eviction is necessary, the cache evicts entries that are less likely
	 * to be used again. For example, the cache may evict an entry because it
	 ...
	 *
	 * <p>
	 * If {@code maximumWeight} is zero, elements will be evicted immediately
	 * after being loaded into cache. This can be useful in testing, or to
	 * disable caching temporarily.
	 *
	 * <p>
	 * Note that weight is only used to determine whether the cache is over
	 ...
	 * @return this {@code CacheBuilder} instance (for chaining)
	 * @throws IllegalArgumentException
	 *             if {@code maximumWeight} is negative
	 * @throws IllegalStateException
	...
	/**
	 * Specifies the weigher to use in determining the weight of entries. Entry
	 * weight is taken into consideration by {@link #maximumWeight(long)} when
	 * determining which entries to evict, and use of this method requires a
	 * corresponding call to {@link #maximumWeight(long)} prior to calling
	 * {@link #build}. Weights are measured and recorded when entries are
	 * inserted into the cache, and are thus effectively static during the
	 ...
	 * are incompatible with the types accepted by the weigher already provided;
	 * the {@code CacheBuilder} type cannot do this. For best results, simply
	 * use the standard method-chaining idiom, as illustrated in the
	 ...
	 * incompatible with the weigher, you will likely experience a
	...
	// Make a safe contravariant cast now so we don't have to do it over and
	// over.
	@SuppressWarnings("unchecked")
	 ...
	 * {@link Cache#size}, but will never be visible to read or write
	 * operations; such entries are cleaned up as part of the routine
	 ...
	 * {@linkplain #maximumSize(long) maximum size} instead of using soft
	 * references. You should only use this method if you are well familiar with
	 * the practical consequences of soft references.
	 *
	 * <p>
	 * <b>Note:</b> when this method is used, the resulting cache will use
	 * identity ({@code ==}) comparison to determine equality of values.
	 ...
	 *
	 * <p>
	 * When {@code duration} is zero, this method hands off to
	 * {@link #maximumSize(long) maximumSize}{@code (0)}, ignoring any
	 * otherwise-specified maximum size or weight. This can be useful in
	 * testing, or to disable caching temporarily without a code change.
	 *
	 * <p>
	 * Expired entries may be counted in {@link Cache#size}, but will never be
	 * visible to read or write operations. Expired entries are cleaned up as
	 * part of the routine maintenance described in the class javadoc.
	 *
	 * @param duration
	 ...
	 * {@code Cache.asMap().get(Object)} and {@code Cache.asMap().put(K, V)}),
	 ...
	 * replacement of its value. The semantics of refreshes are specified in
	 * {@link LoadingCache#refresh}, and are performed by calling
	 * {@link CacheLoader#reload}.
	 *
	 * <p>
	 * As the default implementation of {@link CacheLoader#reload} is
	 * synchronous, it is recommended that users of this method override
	 * {@link CacheLoader#reload} with an asynchronous implementation; otherwise
	 * refreshes will be performed during unrelated cache read and write
	 * operations.
	 *
	 * <p>
	 * Currently automatic refreshes are performed when the first stale request
	 * for an entry occurs. The request triggering refresh will make a blocking
	 * call to {@link CacheLoader#reload} and immediately return the new value
	 * if the returned future is complete, and the old value otherwise.
	 *
	 * <p>
	 * <b>Note:</b> <i>all exceptions thrown during refresh will be logged and
	 * then swallowed</i>.
	 *
	 * @param duration
	 *            the length of time after an entry is created that it should be
	 ...
	 * entry is removed for any {@linkplain RemovalCause reason}. Each cache
	 * created by this builder will invoke this listener as part of the routine
	 * maintenance described in the class documentation above.
	 *
	 * <p>
	 * <b>Warning:</b> after invoking this method, do not continue to use
	 * <i>this</i> cache builder reference; instead use the reference this
	 * method <i>returns</i>. At runtime, these point to the same instance, but
	 * only the returned reference has the correct generic type information so
	 * as to ensure type safety. For best results, use the standard
	 * method-chaining idiom illustrated in the class documentation above,
	 * configuring a builder and building your cache in a single statement.
	 * Failure to heed this advice can result in a {@link ClassCastException}
	 * being thrown by a cache operation at some <i>undefined</i> point in the
	 * future.
	 *
	 * <p>
	 * <b>Warning:</b> any exception thrown by {@code listener} will <i>not</i>
	 * be propagated to the {@code Cache} user, only logged via a
	 * {@link Logger}.
	 *
	 * @return the cache builder reference that should be used instead of
	 *         {@code this} for any remaining configuration and cache building
	 * @return this {@code CacheBuilder} instance (for chaining)
	 * @throws IllegalStateException
	...
	/**
	 * Enable the accumulation of {@link CacheStats} during the operation of the
	 * cache. Without this {@link Cache#stats} will return zero for all
	 * statistics. Note that recording stats requires bookkeeping to be
	 * performed with each operation, and thus imposes a performance penalty on
	 * cache operation.
	 *
	 * @return this {@code CacheBuilder} instance (for chaining)
	 ...
	 * key or atomically computes or retrieves it using the supplied
	 * {@code CacheLoader}. If another thread is currently loading the value for
	 * this key, simply waits for that thread to finish and returns its loaded
	 * value. Note that multiple threads can concurrently load values for
	 * distinct keys.
	 *
	 * <p>
	 * This method does not alter the state of this {@code CacheBuilder}
	 ...
	 *
	 * <p>
	 * Consider {@link #build(CacheLoader)} instead, if it is feasible to
	...
	/**
	 * Returns a string representation for this CacheBuilder instance. The exact
