...
class LocalCache<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
	...
	/**
	 * Accumulates global cache statistics. Note that there are also
	 * per-segments stats counters which must be aggregated to obtain a global
	 * stats view.
	 */
	...
	LocalCache(CacheBuilder<? super K, ? super V> builder, @Nullable CacheLoader<? super K, V> loader) {
		...
		// Find the lowest power-of-two segmentCount that exceeds
		// concurrencyLevel, unless
		// maximumSize/Weight is specified in which case ensure that each
		// segment gets at least 10
		// entries. The special casing for size-based eviction is only necessary
		// because that eviction
		// happens per segment instead of globally, so too many segments
		// compared to the maximum size
	...
	enum Strength {
		/*
		 * TODO(kevinb): If we strongly reference the value and aren't loading,
		 * we needn't wrap the value. This could save ~8 bytes per entry.
		 */
	...
	interface ValueReference<K, V> {
		...
		/**
		 * Waits for a value that may still be loading. Unlike get(), this
		 * method can block (in the case of FutureValueReference).
		 *
		...
		/**
		 * Returns the weight of this entry. This is assumed to be static
		 * between calls to setValue.
		 */
		int getWeight();

		/**
		 * Returns the entry associated with this value reference, or
		 * {@code null} if this value reference is independent of any entry.
		 */
		@Nullable
		...
		/**
		 * Notify pending loads that a new value was set. This is only relevant
		 * to loading value references.
		 */
		void notifyNewValue(@Nullable V newValue);

		/**
		 * Returns true if a new value is currently loading, regardless of
		 * whether or not there is an existing value. It is assumed that the
		 * return value of this method is constant for any given ValueReference
		 * instance.
		 */
		boolean isLoading();

		/**
		 * Returns true if this reference contains an active value, meaning one
		 * that is still considered present in the cache. Active values consist
		 * of live values, which are returned by cache lookups, and dead values,
		 * which have been evicted but awaiting removal. Non-active values
		 * consist strictly of loading values, though during refresh a value may
		 * be both active and loading.
		 */
		boolean isActive();
	}
	...
	interface ReferenceEntry<K, V> {
		...
		/*
		 * Implemented by entries that use write order. Write entries are
		 * maintained in a doubly-linked list. New entries are added at the tail
		 * of the list at write time and stale entries are expired from the head
		 * of the list.
		 */
	...
	static class WeakEntry<K, V> extends WeakReference<K> implements ReferenceEntry<K, V> {
		...
		/*
		 * It'd be nice to get these for free from AbstractReferenceEntry, but
		 * we're already extending WeakReference<K>.
		 */
	 ...
	 * partially-collected, loading, or expired. Unlike
	 * {@link Segment#getLiveValue} this method does not attempt to cleanup
	 * stale entries. As such it should only be called outside of a segment
	...
	static class Segment<K, V> extends ReentrantLock {
		 ...
		 * If modCounts change during a traversal of segments loading size or
		...
		/**
		 * The value reference queue contains value references whose values have
		 * been garbage collected, and which need to be cleaned up internally.
		 */
		final ReferenceQueue<V> valueReferenceQueue;

		/**
		 * The recency queue is used to record which entries were accessed for
		 * updating the access list's ordering. It is drained as a batch
		 * operation when either the DRAIN_THRESHOLD is crossed or a write
		 ...
		 * Returns the new entry, or {@code null} if {@code original} was
		...
		V get(K key, int hash, CacheLoader<? super K, V> loader) throws ExecutionException {
			...
			try {
				if (count != 0) { // read-volatile
					// don't call getLiveEntry, which would ignore loading
					// values
		...
		V lockedGetOrLoad(K key, int hash, CacheLoader<? super K, V> loader) throws ExecutionException {
					...
					if (e.getHash() == hash && entryKey != null && map.keyEquivalence.equivalent(key, entryKey)) {
						...
						if (valueReference.isLoading()) {
							...
							if (value == null) {
								...
								// This is a duplicate check, as preWriteCleanup
								// already purged expired
								// entries, but let's accomodate an incorrect
								// expiration queue.
								enqueueNotification(entryKey, hash, value, valueReference.getWeight(),
			...
			if (createNewEntry) {
				try {
					...
					// recursive load is
					// detected. This may be circumvented when an entry is
					// copied, but will fail fast most
					// of the time.
					synchronized (e) {
		...
		V compute(K key, int hash, BiFunction<? super K, ? super V, ? extends V> function) {
			...
			try {
				...
				// note valueReference can be an existing value or even itself
				// another loading value if
		...
		/**
		 * Waits uninterruptibly for {@code newValue} to be loaded, and then
		...
		/**
		 * Refreshes the value associated with {@code key}, unless another
		 * thread is already doing so. Returns the newly refreshed value
		 * associated with {@code key} if it was refreshed inline, or
		 * {@code null} if another thread is performing the refresh or if an
		...
		V refresh(K key, int hash, CacheLoader<? super K, V> loader, boolean checkTime) {
			...
			if (result.isDone()) {
				try {
					...
					// don't let refresh exceptions propagate; error was already
					// logged
				}
			}
			return null;
		...
		/**
		 * Returns a newly inserted {@code LoadingValueReference}, or null if
		 * the live value reference is already loading.
		 */
		@Nullable
				...
				for (e = first; e != null; e = e.getNext()) {
					...
					if (e.getHash() == hash && entryKey != null && map.keyEquivalence.equivalent(key, entryKey)) {
						...
						if (valueReference.isLoading() || (checkTime && (now - e.getWriteTime() < map.refreshNanos))) {
							...
							// if checkTime, we want to check *after* acquiring
							// the lock if refresh still needs
							// to be scheduled
							return null;
						}
		...
		/**
		 * Drain the key and value reference queues, cleaning up internal
		 * entries containing garbage collected keys or values.
		 */
		@GuardedBy("this")
		 ...
		 * manipulates the eviction queues. Unlocked reads should use
		 * {@link #recordRead}.
		 */
		@GuardedBy("this")
		...
		/**
		 * Performs eviction if the segment is over capacity. Avoids flushing
		 * the entire cache if the newest entry exceeds the maximum weight all
		 * on its own.
		 *
		 * @param newest
					...
					if (e.getHash() == hash && entryKey != null && map.keyEquivalence.equivalent(key, entryKey)) {
						...
						if (entryValue == null) {
							...
							if (valueReference.isActive()) {
								...
								newCount = this.count; // count remains
														// unchanged
							} else {
								setValue(e, key, value, now);
								newCount = this.count + 1;
			...
			try {
				...
				for (ReferenceEntry<K, V> e = first; e != null; e = e.getNext()) {
					...
					if (e.getHash() == hash && entryKey != null && map.keyEquivalence.equivalent(key, entryKey)) {
						...
						if (entryValue == null) {
							if (valueReference.isActive()) {
								// If the value disappeared, this entry is
								// partially collected.
								int newCount = this.count - 1;
								++modCount;
			...
			try {
				...
				for (ReferenceEntry<K, V> e = first; e != null; e = e.getNext()) {
					...
					if (e.getHash() == hash && entryKey != null && map.keyEquivalence.equivalent(key, entryKey)) {
						ValueReference<K, V> valueReference = e.getValueReference();
						V entryValue = valueReference.get();
						// replace the old LoadingValueReference if it's live,
						// otherwise
			...
			if (count != 0) { // read-volatile
				...
				try {
					...
					for (int i = 0; i < table.length(); ++i) {
						for (ReferenceEntry<K, V> e = table.get(i); e != null; e = e.getNext()) {
							// Loading references aren't actually in the map
							// yet.
	...
	static class LoadingValueReference<K, V> implements ValueReference<K, V> {
		...
		// TODO(fry): rename get, then extend AbstractFuture instead of
		// containing SettableFuture
		...
		public void notifyNewValue(@Nullable V newValue) {
			if (newValue != null) {
				...
				// The pending load was removed. Delay notifications until
				// loading completes.
				oldValue = unset();
			}

			// TODO(fry): could also cancel loading if we had a handle on its
			// future
		}
		...
		public ListenableFuture<V> loadFuture(K key, CacheLoader<? super K, V> loader) {
			try {
				...
				return transform(newValue, new com.google.common.base.Function<V, V>() {
					@Override
	 ...
	 *
	 * <p>
	 * Note that this entire implementation makes the assumption that all
	 * elements which are in the map are also in this queue, and that all
	 * elements not in the queue are not in the map.
	 *
	 * <p>
	 * The benefits of creating our own queue are that (1) we can replace
	 * elements in the middle of the queue as part of copyWriteEntry, and (2)
	 * the contains method is highly optimized for the current model.
	 */
	...
	long longSize() {
		...
		long sum = 0;
		for (int i = 0; i < segments.length; ++i) {
			sum += Math.max(0, segments[i].count); // see
	 ...
	 * loading, expired, or partially collected.
	 */
	...
	/**
	 * Serializes the configuration of a LocalCache, reconstituting it as a
	 * Cache using CacheBuilder upon deserialization. An instance of this class
	 * is fit for use by the writeReplace of LocalManualCache.
	 *
	 * Unfortunately, readResolve() doesn't get called when a circular
	 * dependency is present, so the proxy must be able to behave as the cache
	 * itself.
	 */
	 ...
	 * LoadingCache using CacheBuilder upon deserialization. An instance of this
	 * class is fit for use by the writeReplace of LocalLoadingCache.
	 *
	 * Unfortunately, readResolve() doesn't get called when a circular
	 * dependency is present, so the proxy must be able to behave as the cache
	 * itself.
