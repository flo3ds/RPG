...
public class XmlEscapers {
	...
	// For each xxxEscaper() method, please add links to external reference
	// pages
	// that are considered authoritative for the behavior of that escaper.

	/**
	 * Returns an {@link Escaper} instance that escapes special characters in a
	 * string so it can safely be included in an XML document as element
	 * content. See section
	 * <a href="http://www.w3.org/TR/2008/REC-xml-20081126/#syntax">2.4</a> of
	 ...
	 *
	 * <p>
	 * This escaper substitutes {@code 0xFFFD} for non-whitespace control
	 * characters and the character values {@code 0xFFFE} and {@code 0xFFFF}
	 * which are not permitted in XML. For more detail see section
	 ...
	 * character references (NCR). Any non-ASCII characters appearing in the
	 * input will be preserved in the output. Specifically "\r" (carriage
	 * return) is preserved in the output, which may result in it being silently
	...
	/**
	 * Returns an {@link Escaper} instance that escapes special characters in a
	 * string so it can safely be included in XML document as an attribute
	 * value. See section <a href=
	 * "http://www.w3.org/TR/2008/REC-xml-20081126/#AVNormalize">3.3.3</a> of
	 * the XML specification.
	 *
	 * <p>
	 * This escaper substitutes {@code 0xFFFD} for non-whitespace control
	 ...
	 * <p>
	 * This escaper does not escape non-ASCII characters to their numeric
	 * character references (NCR). However, horizontal tab {@code '\t'}, line
	...
	static {
		...
		/*
		 * Except for \n, \t, and \r, all ASCII control characters are replaced
		 * with the Unicode replacement character.
		 *
		 * Implementation note: An alternative to the following would be to make
		 * a map that simply replaces the allowed ASCII whitespace characters
		 * with themselves and to set the minimum safe character to 0x20.
		 * However this would slow down the escaping of simple strings that
		 * contain \t, \n, or \r.
		 */
