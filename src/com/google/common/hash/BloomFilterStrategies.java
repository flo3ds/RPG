...
enum BloomFilterStrategies implements BloomFilter.Strategy {
	...
	static final class LockFreeBitArray {
		...
		boolean set(long bitIndex) {
			...
			long mask = 1L << bitIndex; // only cares about low 6 bits of
										// bitIndex
		 ...
		 * method is executing, the final long[] will be a "rolling snapshot" of
		 * the state of the bit array. This is usually good enough, but should
		 * be kept in mind.
		 */
		 ...
		 * bitCount is a (very) close *estimate* of the actual number of bits
		 * set. It's not possible to do better than an estimate without locking.
		 * Note that the number, if not exactly accurate, is *always*
		 ...
		 *
		 * <p>
		 * NOTE: Because of the use of atomics, if the other LockFreeBitArray is
		 * being mutated while this operation is executing, not all of those new
		 * 1's may be set in the final state of this LockFreeBitArray. The ONLY
		 * guarantee provided is that all the bits that were set in the other
		 * LockFreeBitArray at the start of this method will be set in this
		 * LockFreeBitArray at the end of this method.
		 */
