...
abstract class AbstractStreamingHasher extends AbstractHasher {
	...
	/**
	 * Constructor for use by subclasses. This hasher instance will process
	 * chunks of the specified size.
	 *
	 * @param chunkSize
	 *            the number of bytes available per {@link #process(ByteBuffer)}
	...
	/**
	 * Constructor for use by subclasses. This hasher instance will process
	 * chunks of the specified size, using an internal buffer of
	 * {@code bufferSize} size, which must be a multiple of {@code chunkSize}.
	 *
	 * @param chunkSize
	 *            the number of bytes available per {@link #process(ByteBuffer)}
	 *            invocation; must be at least 4
	 * @param bufferSize
	 *            the size of the internal buffer. Must be a multiple of
	 *            chunkSize
	 */
	...
	/**
	 * Processes the available bytes of the buffer (at most {@code chunk}
	 * bytes).
	 */
	...
	private Hasher putBytesInternal(ByteBuffer readBuffer) {
		...
		munch(); // buffer becomes empty here, since chunkSize divides
					// bufferSize
	 ...
	 *
	 * While intuitively, using CharsetEncoder to encode the CharSequence
	 * directly to the buffer (or even to an intermediate buffer) should be
	 * considerably more efficient than potentially copying the CharSequence to
	 * a String and then calling getBytes(Charset) on that String, in reality
	 * there are optimizations that make the getBytes(Charset) approach
	 * considerably faster, at least for commonly used charsets like UTF-8.
	 */
	 ...
	 * hasher. This is called after all chunks are handled with {@link #process}
	 * and any leftover bytes that did not make a complete chunk are handled
	 * with {@link #processRemaining}.
	 */
	...
	private void munchIfFull() {
		if (buffer.remaining() < 8) {
			// buffer is full; not enough room for a primitive. We have at least
			// one full chunk.
			munch();
		}
	}
	...
	private void munch() {
		...
		while (buffer.remaining() >= chunkSize) {
			// we could limit the buffer to ensure process() does not read more
			// than
			// chunkSize number of bytes, but we trust the implementations
			process(buffer);
		}
		buffer.compact(); // preserve any remaining data that do not make a full
							// chunk
	}
}
