...
public abstract class HashCode {
	 ...
	 * least-significant bytes and {@code 0x00} as the remaining
	...
	/**
	 * Returns the value of this hash code as a byte array. The caller may
	 * modify the byte array; changes to it will <i>not</i> be reflected in this
	 ...
	 *             if there is not enough room in {@code dest}
	 */
	 ...
	 * {@code HashCode} if it is a byte-based hashcode. Otherwise it returns
	 * {@link HashCode#asBytes}. Do <i>not</i> mutate this array or else you
	 * will break the immutability contract of {@code HashCode}.
	 */
	 ...
	 * copied to preserve the immutability contract of {@code HashCode}. The
	...
	private static final class BytesHashCode extends HashCode implements Serializable {
		...
		@Override
		boolean equalsSameBits(HashCode that) {
			// We don't use MessageDigest.isEqual() here because its contract
			// does not guarantee
	 ...
	 *
	 * <p>
	 * This method accepts the exact format generated by {@link #toString}. If
	 * you require more lenient {@code base 16} decoding, please use
	...
	/**
	 * Returns {@code true} if {@code object} is a {@link HashCode} instance
	 * with the identical byte representation to this hash code.
	 *
	 * <p>
	 * <b>Security note:</b> this method uses a constant-time (not
	 * short-circuiting) implementation to protect against
	 ...
	 * instances into a {@code
	 * HashSet}) but is otherwise probably not what you want to use.
	 */
	@Override
	public final int hashCode() {
		// If we have at least 4 bytes (32 bits), just take the first 4 bytes.
		// Since this is
		// already a (presumably) high-quality hash code, any four bytes of it
		// will do.
		if (bits() >= 32) {
			return asInt();
		}
	 ...
	 *
	 * <p>
	 * Note that if the output is considered to be a single hexadecimal number,
