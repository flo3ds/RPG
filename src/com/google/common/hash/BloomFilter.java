 ...
 *
 * <p>
 * As of Guava 22.0, this class is thread-safe and lock-free. It internally uses
 * atomics and compare-and-swap to ensure correctness when multiple threads are
 * used to access it.
 *
 * @param <T>
 *            the type of instances that the {@code BloomFilter} accepts
...
public final class BloomFilter<T> implements Predicate<T>, Serializable {
	...
	public double expectedFpp() {
		// You down with FPP? (Yeah you know me!) Who's down with FPP? (Every
		// last homie!)
	 ...
	 * been added to this Bloom filter. This approximation is reasonably
	...
	public long approximateElementCount() {
		...
		/**
		 * Each insertion is expected to reduce the # of clear bits by a factor
		 * of `numHashFunctions/bitSize`. So, after n insertions, expected
		 * bitCount is `bitSize * (1 - (1 - numHashFunctions/bitSize)^n)`.
		 * Solving that for n, and approximating `ln x` as `x - 1` when x is
		 * close to 1 (why?), gives the following formula.
		 */
	 ...
	 * bitwise OR of the underlying data. The mutations happen to <b>this</b>
	 * instance. Callers must ensure the Bloom filters are appropriately sized
	 * to avoid saturating them.
	 *
	 * @param that
	 *            The Bloom filter to combine this Bloom filter with. It is not
	 *            mutated.
	 * @throws IllegalArgumentException
	 *             if {@code isCompatible(that) == false}
	 * @since 15.0
	 */
	...
	/**
	 * Creates a {@link BloomFilter}{@code <T>} with the expected number of
	 ...
	 *
	 * @param funnel
	 *            the funnel of T's that the constructed {@code BloomFilter<T>}
	 *            will use
	...
	public static <T> BloomFilter<T> create(Funnel<? super T> funnel, long expectedInsertions) {
		return create(funnel, expectedInsertions, 0.03); // FYI, for 3%, we
															// always get 5 hash
															// functions
	}
	 ...
	 * (not Java serialization). This has been measured to save at least 400
	 * bytes compared to regular serialization.
	 *
	 * <p>
	 * Use {@linkplain #readFrom(InputStream, Funnel)} to reconstruct the
	 * written BloomFilter.
	 */
	public void writeTo(OutputStream out) throws IOException {
		...
		dout.writeByte(UnsignedBytes.checkedCast(numHashFunctions)); // note:
																		// checked
																		// at
																		// the
																		// c'tor
	 ...
	 * {@linkplain #writeTo(OutputStream)}, into a {@code BloomFilter<T>}.
	 *
	 * The {@code Funnel} to be used is not encoded in the stream, so it must be
	 * provided here. <b>Warning:</b> the funnel provided <b>must</b> behave
	 * identically to the one used to populate the original Bloom filter!
	 *
	 * @throws IOException
	 *             if the InputStream throws an {@code IOException}, or if its
	...
	public static <T> BloomFilter<T> readFrom(InputStream in, Funnel<? super T> funnel) throws IOException {
		...
		try {
			...
			// currently this assumes there is no negative ordinal; will have to
			// be updated if we
			// add non-stateless strategies (for which we've reserved negative
			// ordinals; see
			// Strategy.ordinal()).
