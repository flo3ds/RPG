 ...
 * supporting dynamic striping on 64bit values. The class extends Number so that
 * concrete subclasses must publicly do so.
 */
@GwtIncompatible
abstract class Striped64 extends Number {
	 ...
	 *
	 * Table entries are of class Cell; a variant of AtomicLong padded to reduce
	 * cache contention on most processors. Padding is overkill for most Atomics
	 * because they are usually irregularly scattered in memory and thus don't
	 * interfere much with each other. But Atomic objects residing in arrays
	 * will tend to be placed adjacent to each other, and so will most often
	 * share cache lines (with a huge negative performance impact) without this
	 * precaution.
	 *
	 ...
	 * operation (see method retryUpdate). Upon a collision, if the table size
	 ...
	 * proceed by "double hashing", using a secondary hash (Marsaglia XorShift)
	 * to try to find a free slot.
	 *
	 ...
	 * collisions. When we reach capacity, we search for this mapping by
	 * randomly varying the hash codes of colliding threads. Because search is
	 * random, and collisions only become known via CAS failures, convergence
	 ...
	 * value field is placed between pads, hoping that the JVM doesn't reorder
	 * them.
	 *
	...
	/**
	 * ThreadLocal holding a single-slot int array holding hash code. Unlike the
	 * JDK8 version of this class, we use a suboptimal int[] representation to
	 * avoid introducing a new type that can impede class-unloading when
	 * ThreadLocals are not removed.
	 */
	...
	/**
	 * Computes the function of current and new value. Subclasses should
	 * open-code this update function for most uses, but the virtualized form is
	 * needed within retryUpdate.
	 *
	 * @param currentValue
