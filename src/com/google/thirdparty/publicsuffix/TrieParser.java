...
final class TrieParser {
	 ...
	 *
	 * @param stack
	 *            The prefixes that precede the characters represented by this
	 *            node. Each entry of the stack is in reverse order.
	 * @param encoded
	 *            The serialized trie.
	 * @param builder
	 *            A map builder to which all entries will be added.
	...
	private static int doParseTrieToBuilder(List<CharSequence> stack, CharSequence encoded,
		...
		if (c == '!' || c == '?' || c == ':' || c == ',') {
			// '!' represents an interior node that represents an ICANN entry in
			// the map.
			// '?' represents a leaf node, which represents an ICANN entry in
			// map.
			// ':' represents an interior node that represents a private entry
			// in the map
			// ',' represents a leaf node, which represents a private entry in
			// the map.
		...
		if (c != '?' && c != ',') {
			while (idx < encodedLen) {
				...
				if (encoded.charAt(idx) == '?' || encoded.charAt(idx) == ',') {
					// An extra '?' or ',' after a child node indicates the end
					// of all children of this node.
					idx++;
					break;
				}
			}
