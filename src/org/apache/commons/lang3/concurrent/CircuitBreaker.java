 ...
 * unreliable services or unexpected load. It typically monitors a specific
 * resource. As long as this resource works as expected, it stays in state
 * <em>closed</em>, meaning that the resource can be used. If problems are
 * encountered when using the resource, the circuit breaker can switch into
 * state <em>open</em>; then access to this resource is prohibited. Depending on
 * a concrete implementation, it is possible that the circuit breaker switches
 * back to state <em>closed</em> when the resource becomes available again.
 * </p>
 * <p>
 * This interface defines a generic protocol of a circuit breaker component. It
 * should be sufficiently generic to be applied to multiple different use cases.
 * </p>
 *
 * @param <T>
