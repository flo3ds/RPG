 ...
 * {@link FastDateFormat#getInstance(String, TimeZone, Locale)} or another
 * variation of the factory methods of {@link FastDateFormat}.
 * </p>
 *
 * <p>
 ...
 * <code>SimpleDateFormat</code> in most parsing situations. This class is
 ...
 * it be as Sun has closed the <a href=
 * "http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4228335">bug</a>/RFE.
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * The class operates in lenient mode, so for example a time of 90 minutes is
 * treated as 1 hour 30 minutes.
 * </p>
 *
 * <p>
 * Timing tests indicate this class is as about as fast as SimpleDateFormat in
 * single thread applications and about 25% faster in multi-thread applications.
 * </p>
 *
 * @since 3.2
 * @see FastDatePrinter
...
public class FastDateParser implements DateParser, Serializable {
	 ...
	 * another variation of the factory methods of {@link FastDateFormat} to get
	 * a cached FastDateParser instance.
	 *
	 * @param pattern
	 *            non-null {@link java.text.SimpleDateFormat} compatible pattern
	 * @param timeZone
	 ...
	 *            non-null locale
	 * @param centuryStart
	 *            The start of the century for 2 digit year parsing
	 *
	 * @since 3.5
	 */
	protected FastDateParser(final String pattern, final TimeZone timeZone, final Locale locale,
	 ...
	 * constructor and from readObject (de-serialization)
	 *
	 * @param definingCalendar
	 *            the {@link java.util.Calendar} instance used to initialize
	 *            this FastDateParser
	 */
	 ...
	 * However, it sets the error index to the position before the failed field
	 * unlike the method
	 * {@link java.text.SimpleDateFormat#parse(String, ParsePosition)} which
	 * sets the error index to after the failed field.
	 * <p>
	 ...
	 * updated. If the input buffer has been fully parsed, then the index will
	 ...
	 * Calendar with parsed fields. Upon success, the ParsePosition index is
	 * updated to indicate how much of the source text was consumed. Not all
	 * source text needs to be consumed. Upon parse failure, ParsePosition error
	 * index is updated to the offset of the source text which does not match
	 * the supplied format.
	 *
	 * @param source
	 *            The text to parse.
	 * @param pos
	 *            On input, the position in the source to start parsing, on
	 ...
	 *             when Calendar has been set to be not lenient, and a parsed
	 ...
	 * @param locale
	 *            The locale of display names
	 * @param field
	 *            The field of interest
	 * @param regex
	 *            The regular expression to build
	 ...
	 * 
	 * @param f
	 *            A sub-sequence of the SimpleDateFormat pattern
	 * @param definingCalendar
	 *            The calendar to obtain the short and long values
		...
		TimeZoneStrategy(final Locale locale) {
			...
			for (final String[] zoneNames : zones) {
				...
				for (int i = 1; i < zoneNames.length; ++i) {
					switch (i) {
					case 3: // offset 3 is long daylight savings (or summertime)
							// name
	...
	private static class ISO8601TimeZoneStrategy extends PatternStrategy {
		 ...
		 *
		 * @param tokenLen
		 *            a token indicating the length of the TimeZone String to be
		 *            formatted.
		 * @return a ISO8601TimeZoneStrategy that can format TimeZone String of
		 *         length {@code tokenLen}. If no such strategy exists, an
