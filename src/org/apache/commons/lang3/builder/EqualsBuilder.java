 ...
 * follows rules laid out in <a href=
 * "http://www.oracle.com/technetwork/java/effectivejava-136174.html">Effective
 ...
 * Non-primitive fields are compared using <code>equals()</code>.
 * </p>
 *
 * <p>
 ...
 * used by the <code>reflectionEquals</code> methods.
 * </p>
 *
 * @since 1.0
 */
public class EqualsBuilder implements Builder<Boolean> {
	...
	/*
	 * NOTE: we cannot store the actual objects in a HashSet, as that would use
	 ...
	 * object.
	 *
	 * Now HashSet uses equals() to determine if two elements with the same hash
	 * code really are equal, so we also need to ensure that the replacement
	 * objects are only equal if the original objects are identical.
	 *
	 * The original implementation (2.4 and before) used the
	 * System.identityHashCode() method - however this is not guaranteed to
	 ...
	 * Used by the reflection methods to avoid infinite loops. Objects might be
	 * swapped therefore a check is needed if the object pair is registered in
	 * given or swapped order.
	 * </p>
	 *
	 * @param lhs
	 *            <code>this</code> object to lookup in registry
	 * @param rhs
	...
	/**
	 * <p>
	 * Registers the given object pair. Used by the reflection methods to avoid
	 * infinite loops.
	 * </p>
	 *
	 * @param lhs
	 *            <code>this</code> object to register
	 * @param rhs
	 ...
	 * compared using <code>equals()</code>.
	 * </p>
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * If the testRecursive parameter is set to <code>true</code>, non primitive
	 ...
	 *            <code>null</code>
	 * @param testRecursive
	 *            whether to call reflection equals on non primitive fields
	 *            recursively.
	 * @param excludeFields
	 *            array of field names to exclude from testing
	 * @return <code>true</code> if the two Objects have tested equals.
	 ...
	 */
	public static boolean reflectionEquals(final Object lhs, final Object rhs, final boolean testTransients,
			final Class<?> reflectUpToClass, final boolean testRecursive, final String... excludeFields) {
		if (lhs == rhs) {
			return true;
		}
		...
		return new EqualsBuilder().setExcludeFields(excludeFields).setReflectUpToClass(reflectUpToClass)
				.setTestTransients(testTransients).setTestRecursive(testRecursive).reflectionAppend(lhs, rhs)
				.isEquals();
	}

	/**
	 * <p>
	 * Tests if two <code>objects</code> by using reflection.
	 * </p>
	 *
	 * <p>
	 * It uses <code>AccessibleObject.setAccessible</code> to gain access to
	 ...
	 *
	 * <p>
	 * If the testTransients field is set to <code>true</code>, transient
	 * members will be tested, otherwise they are ignored, as they are likely
	 * derived fields, and not part of the value of the <code>Object</code>.
	 * </p>
	 *
	 * <p>
	 * Static fields will not be included. Superclass fields will be appended up
	 * to and including the specified superclass in field
	 * <code>reflectUpToClass</code>. A null superclass is treated as
	 * java.lang.Object.
	 * </p>
	 *
	 * <p>
	 * Field names listed in field <code>excludeFields</code> will be ignored.
	 * </p>
	 *
	 * @param lhs
		...
		try {
			...
			for (int i = 0; i < fields.length && isEquals; i++) {
				...
				if (!ArrayUtils.contains(excludeFields, f.getName()) && !f.getName().contains("$")
						...
						&& !Modifier.isStatic(f.getModifiers()) && !f.isAnnotationPresent(EqualsExclude.class)) {
					try {
	...
	/**
	 * <p>
	 * Test if two <code>Object</code>s are equal using either
	 * #{@link #reflectionAppend(Object, Object)}, if object are non primitives
	 * (or wrapper of primitives) or if field <code>testRecursive</code> is set
	 * to <code>false</code>. Otherwise, using their <code>equals</code> method.
	 * </p>
	 *
	 * @param lhs
	 *            the left hand object
	...
	/**
	 * <p>
	 * Test if an <code>Object</code> is equal to an array.
	 * </p>
	 *
	 * @param lhs
	...
	private void appendArray(final Object lhs, final Object rhs) {
		// First we compare different dimensions, for example: a boolean[][] to
		// a boolean[]
	 ...
	 * </p>
	 *
	 * @return <code>true</code> if all of the fields that have been checked are
